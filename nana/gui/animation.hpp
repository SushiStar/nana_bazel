...
namespace nana {
class animation;
/// Holds the frames and frame builders. Have reference semantics for
/// efficiency.
class frameset {
...
public:
  ...
  using framebuilder =
      std::function<bool(std::size_t pos, paint::graphics &, nana::size &)>;

public:
  frameset();
  void push_back(paint::image); ///< Inserts frames at the end.
  void push_back(framebuilder fb,
                 std::size_t length); ///< Inserts a framebuilder and the number
                                      ///< of frames that it generates.
private:
  struct impl;
...
public:
  ...
  void looped(
      bool enable); ///< Enables or disables the animation repeating playback.
  ...
  /**
   * If the size of rectangle is not equal to the size of frame, it stretches
   * the frame for the size of rectangle.
   * @param wd Output window.
   * @param r Generator of the rectangle. The generator gets called every time
   * rendering occurs.
   */
