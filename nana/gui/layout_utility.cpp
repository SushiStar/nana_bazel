...
bool overlapped(const rectangle &r1, const rectangle &r2) {
  if (r1.y + (long long)(r1.height) <= r2.y)
    return false;
  if (r1.y >= r2.y + (long long)(r2.height))
    return false;

  if (r1.x + (long long)(r1.width) <= r2.x)
    return false;
  if (r1.x >= r2.x + (long long)(r2.width))
    return false;
...
bool overlap(const rectangle &ir, const size &valid_input_area,
             const rectangle &dr, const size &valid_dst_area, rectangle &op_ir,
             rectangle &op_dr) {
...
bool intersection(const rectangle &r, point pos_beg, point pos_end,
                  point &good_pos_beg, point &good_pos_end) {
  ...
  bool good_beg = (0 <= pos_beg.x && pos_beg.x < right && 0 <= pos_beg.y &&
                   pos_beg.y < bottom);
  bool good_end = (0 <= pos_end.x && pos_end.x < right && 0 <= pos_end.y &&
                   pos_end.y < bottom);
...
void zoom(const rectangle &ref, const rectangle &scaled,
          const rectangle &ref_dst, rectangle &r) {
  ...
  r.width =
      static_cast<unsigned>(scaled.width / double(ref.width) * ref_dst.width);
  r.height = static_cast<unsigned>(scaled.height / double(ref.height) *
                                   ref_dst.height);
}
             ...
             const rectangle &r2) // Rectangle 2
{
  if (r1.x < r2.x || r1.right() > r2.right())
    return false;
  if (r1.y < r2.y || r1.bottom() > r2.bottom())
    return false;
  return true;
}
} // namespace nana
