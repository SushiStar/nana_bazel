...
#ifdef NANA_X11
#include "nana/detail/posix/platform_spec.hpp"
#endif
...
namespace restrict {
  ...
  inline detail::window_manager &wd_manager() { return bedrock.wd_manager(); }
  } // namespace
} // namespace restrict
...
make_scheme(::nana::detail::scheme_factory_interface &&factory) {
  return restrict ::bedrock.scheme().create(
      static_cast<::nana::detail::scheme_factory_interface &&>(factory));
}

bool emit_event(event_code evt_code, window wd, const ::nana::event_arg &arg) {
  return restrict ::bedrock.emit(
      evt_code, wd, arg, true, restrict ::bedrock.get_thread_context(), false);
}

bool emit_internal_event(event_code evt_code, window wd,
                         const ::nana::event_arg &arg) {
  return restrict ::bedrock.emit(evt_code, wd, arg, true,
                                 restrict ::bedrock.get_thread_context(), true);
}

void enum_widgets_function_base::enum_widgets(window wd, bool recursive) {
  internal_scope_guard lock;
  if (is_window(wd)) {
    // Use a copy, because enumerating function may close a child window and the
    // original children container would be changed, in the situation, the
    // walking thorugh directly to the wd->children would cause error.
...
void effects_edge_nimbus(window wd, effects::edge_nimbus en) {
  ...
  if (is_window(wd)) {
    ...
    if (effects::edge_nimbus::none != en) {
      ...
      wd->effect.edge_nimbus = static_cast<effects::edge_nimbus>(
          static_cast<unsigned>(en) |
          static_cast<unsigned>(wd->effect.edge_nimbus));
    } else {
...
void effects_bground(window wd,
                     const effects::bground_factory_interface &factory,
                     double fade_rate) {
  if (fade_rate < 0.0 || fade_rate > 1.0)
    throw std::invalid_argument(
        "effects_bground: value range of fade_rate must be [0, 1].");
...
bground_mode effects_bground_mode(window wd) {
  ...
  if (is_window(wd) && wd->effect.bground)
    return (wd->effect.bground_fade_rate <= 0.009 ? bground_mode::basic
                                                  : bground_mode::blend);
...
void attach_drawer(widget &wdg, drawer_trigger &dr) {
  ...
  if (is_window(wd)) {
    ...
    wd->drawer.graphics.rectangle(true,
                                  wd->annex.scheme->background.get_color());
    wd->drawer.attached(wdg, dr);
    wd->drawer.refresh(); // Always redraw no matter it is visible or invisible.
                          // This can make the graphics data correctly.
  }
}
...
window create_window(window owner, bool nested, const rectangle &r,
                     const appearance &ap, widget *wdg) {
...
bool copy_transparent_background(window wd, paint::graphics &graph) {
  ...
  wd->other.glass_buffer.paste(rectangle{wd->other.glass_buffer.size()}, graph,
                               0, 0);
  return true;
}

bool copy_transparent_background(window wd, const rectangle &src_r,
                                 paint::graphics &graph, const point &dst_pt) {
  internal_scope_guard lock;
...
void lazy_refresh() { restrict ::bedrock.thread_context_lazy_refresh(); }

void draw_shortkey_underline(paint::graphics &graph, const std::string &text,
                             wchar_t shortkey, std::size_t shortkey_position,
                             const point &text_pos, const color &line_color) {
  if (shortkey) {
#ifdef _nana_std_has_string_view
    auto off_x = (shortkey_position ? graph
                                          .text_extent_size(std::string_view{
                                              text.c_str(), shortkey_position})
                                          .width
                                    : 0);
    auto key_px = static_cast<int>(
        graph.text_extent_size(std::wstring_view{&shortkey, 1}).width);
#else
    auto off_x =
        (shortkey_position
             ? graph.text_extent_size(text.c_str(), shortkey_position).width
             : 0);
...
//@brief:	This function searches whether the text contains a '&' and
// removes the character for transforming. 			If the text
// contains more than one '&'
// character, the others are ignored. e.g 			text =
// "&&a&bcd&ef", the result should be "&abcdef", shortkey = 'b', and pos = 2.
...
std::string transform_shortkey_text(std::string text, wchar_t &shortkey,
  ...
  while (true) {
    ...
    if (pos != std::wstring::npos) {
      ...
      if (shortkey == 0 && pos < text.length()) {
        ...
        if (shortkey == '&') // This indicates the text contains "&&", it means
                             // the symbol have to be ignored.
          shortkey = 0;
        else if (skpos)
          *skpos = pos;
      }
...
::nana::point cursor_position() { return interface_type::cursor_position(); }
...
::nana::rectangle make_center(unsigned width, unsigned height) {
  ...
  return {
      ...
      static_cast<int>(height > screen.height ? 0
...
void window_icon(window wd, const paint::image &small_icon,
                 const paint::image &big_icon) {
  if (nullptr != wd)
...
bool is_window(window wd) { return restrict ::wd_manager().available(wd); }
...
window root(native_window_type wd) { return restrict ::wd_manager().root(wd); }
...
void close_window(window wd) { restrict ::wd_manager().close(wd); }
...
window get_owner_window(window wd) {
  ...
  if (is_window(wd) && (wd->other.category == category::flags::root)) {
    auto owner =
        interface_type::get_window(wd->root, window_relationship::owner);
    if (owner)
...
nana::point window_position(window wd) {
  ...
  if (is_window(wd)) {
    return ((wd->other.category == category::flags::root)
                ? interface_type::window_position(wd->root)
                : wd->pos_owner);
  }
  return nana::point{};
}
...
bool set_window_z_order(window wd, window wd_after,
  ...
  if (is_window(wd) && (category::flags::root == wd->other.category)) {
    if (wd_after) {
      if (is_window(wd_after) &&
          (wd_after->other.category == category::flags::root)) {
        native_after = wd_after->root;
        action_if_no_wd_after = z_order_action::none;
      } else
        return false;
    }
    interface_type::set_window_z_order(wd->root, native_after,
                                       action_if_no_wd_after);
    return true;
  }
  return false;
}
...
void draw_through(window wd, std::function<void()> draw_fn) {
  ...
  if (::nana::category::flags::root != wd->other.category)
    throw std::invalid_argument(
        "draw_through: the window is not a root widget");
...
void window_outline_size(window wd, const size &sz) {
  ...
  if (is_window(wd)) {
    if (category::flags::root == wd->other.category) {
      ...
      if (inner_size.width <
          static_cast<unsigned>(fm_extents.left + fm_extents.right))
        inner_size.width = 0;
      else
        inner_size.width -=
            static_cast<unsigned>(fm_extents.left + fm_extents.right);

      if (inner_size.height <
          static_cast<unsigned>(fm_extents.top + fm_extents.bottom))
        inner_size.height = 0;
      else
        inner_size.height -=
            static_cast<unsigned>(fm_extents.top + fm_extents.bottom);
...
bool track_window_size(window wd, const nana::size &sz, bool true_for_max) {
  ...
  if (!sz.empty()) {
    if (true_for_max) {
      ...
      if (wd->min_track_size.width > sz.width ||
          wd->min_track_size.height > sz.height)
        return false;
    } else {
      // Make sure that the new size is less than max size
      if ((wd->max_track_size.width || wd->max_track_size.height) &&
          (wd->max_track_size.width < sz.width ||
           wd->max_track_size.height < sz.height))
        return false;
    }

    ts = interface_type::check_track_size(sz, wd->extra_width, wd->extra_height,
                                          true_for_max);
  } else
...
window capture_window() { return restrict ::wd_manager().capture_window(); }
...
void modal_window(window wd) {
  ...
  // modal has to guarantee that does not lock the mutex of window_manager
  // before invoking the pump_event, otherwise, the modal will prevent the other
  // thread access the window.
...
color bgcolor(window wd, const color &clr) {
  ...
  if (is_window(wd)) {
    ...
    if (prev != clr) {
      ...
      // If the bground mode of this window is basic, it should remake the
      // background
      if (wd->effect.bground &&
          wd->effect.bground_fade_rate <
              0.01) // fade rate < 0.01 means it is basic mode
        wd->flags.make_bground_declared = true;
...
class caret_proxy : public caret_interface {
public:
  caret_proxy(basic_window *wd) : window_{wd} {}

  void disable_throw() noexcept override { throw_ = false; }
...
void tabstop(window wd) { restrict ::wd_manager().enable_tabstop(wd); }
...
nana::element_state element_state(window wd) {
  ...
  if (is_window(wd)) {
    const bool is_focused =
        (wd->root_widget->other.attribute.root->focus == wd);
    switch (wd->flags.action) {
    case nana::mouse_action::normal:
    case nana::mouse_action::normal_captured:
      return (is_focused ? nana::element_state::focus_normal
                         : nana::element_state::normal);
    case nana::mouse_action::hovered:
      return (is_focused ? nana::element_state::focus_hovered
                         : nana::element_state::hovered);
...
std::optional<std::pair<size, size>>
content_extent(window wd, unsigned limited_px, bool limit_width) {
  ...
  if (is_window(wd) && wd->annex.content_measurer) {
    ...
    auto extent =
        wd->annex.content_measurer->measure(*graph, limited_px, limit_width);
    if (extent)
      return std::make_pair(extent.value(),
                            extent.value() +
                                wd->annex.content_measurer->extension());
  }
