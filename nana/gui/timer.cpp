 ...
 *	@description:
 *		A timer can repeatedly call a piece of code. The duration
 *between calls is specified in milliseconds. Timer is different from other
 *graphics controls, it has no graphics interface.
...
#include "nana/common/std_mutex.hpp"
#else
#include <mutex>
#endif
...
public:
  ...
  detail::timer_core *create(unsigned ms, Factory &&factory) {
    ...
    try {
      ...
      auto p = factory();
      auto tmid = p;
      ::nana::detail::platform_spec::instance().set_timer(
          tmid, ms, &timer_driver::_m_timer_proc);
#endif
...
private:
#if defined(NANA_WINDOWS)
  static void __stdcall _m_timer_proc(HWND hwnd, UINT uMsg, UINT_PTR id,
                                      DWORD dwTime);
#else
...
public:
#if defined(NANA_WINDOWS)
  timer_core(timer *sender, timer_identifier tmid,
             std::shared_ptr<basic_event<arg_elapse>> evt_elapse)
      : sender_(sender), timer_(tmid), evt_elapse_(evt_elapse) {}
#else
  timer_core(timer *sender, std::shared_ptr<basic_event<arg_elapse>> evt_elapse)
      : sender_(sender), timer_(this), evt_elapse_(evt_elapse) {}
#endif

  timer_identifier id() const { return timer_; }
...
#else
    ::nana::detail::platform_spec::instance().set_timer(
        timer_, ms, &timer_driver::_m_timer_proc);
#endif
  }
...
#if defined(NANA_WINDOWS)
void __stdcall timer_driver::_m_timer_proc(HWND /*hwnd*/, UINT /*uMsg*/,
                                           UINT_PTR handle, DWORD /*dwTime*/)
#else
...
timer::timer(std::chrono::milliseconds ms) : timer() { this->interval(ms); }
...
void timer::start() {
  if (impl_->tm_core)
    return;
#if defined(NANA_WINDOWS)
  impl_->tm_core = timer_driver::instance().create(
      impl_->interval, [this](timer_identifier id) {
...
bool timer::started() const { return (nullptr != impl_->tm_core); }
...
unsigned timer::_m_interval() const { return impl_->interval; }
// end class timer
} // end namespace nana
