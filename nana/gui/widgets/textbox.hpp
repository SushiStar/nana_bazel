...
struct arg_textbox : public event_arg {
  textbox &widget;
  const std::vector<upoint>
      &text_position; ///< position of characters that the first character of
                      ///< line which are displayed
...
namespace textbox {
struct textbox_events : public general_events {
  basic_event<arg_textbox> first_change;
  basic_event<arg_textbox> text_changed;
  basic_event<arg_textbox> text_exposed;
};

class event_agent : public widgets::skeletons::textbase_event_agent_interface,
...
class textbox
    : public widget_object<category::widget_tag, drawerbase::textbox::drawer,
                           drawerbase::textbox::textbox_events,
                           ::nana::widgets::skeletons::text_editor_scheme> {
public:
  using colored_area_type = widgets::skeletons::colored_area_type;
  using colored_area_access_interface =
      widgets::skeletons::colored_area_access_interface;
  ...
  /// \brief The construct that creates a widget.
  /// @param window  A handle to the parent window of the widget being created.
  /// @param rectangle  the size and position of the widget in its parent window
  /// coordinate.
  /// @param visible  specifying the visible after creating.
  textbox(window, const rectangle & = rectangle(), bool visible = true);

  ///  \brief Loads a text file. When attempt to load a unicode encoded text
  ///  file, be sure the file have a BOM header.
  ...
  /// Enables/disables the textbox to indent a line. Idents a new line when it
  /// is created by pressing enter.
  /// @param generator generates text for identing a line. If it is empty,
  /// textbox indents the line according to last line.
  textbox &indention(bool, std::function<std::string()> generator = {});

  /// Discards the old text and set a new text. It also clears the
  /// filename/edited flags and undo command. A workaround for reset, explicit
  /// default constructor syntax, because VC2013 incorrectly treats {} as {0}.
  /*
   * @param text A new text replaces the old text.
   * @param end_caret Indicates whether to position the caret to the end of
   * text.
   * @return a reference of *this.
   */
  textbox &
  reset(const std::string &text = std::string(),
        bool end_caret = true); ///< discard the old text and set a new text
  ...
  /// Read the text from a specified line with a set offset. It returns true for
  /// success.
  bool getline(std::size_t line_index, std::size_t offset,
  ...
  /// Read the text from a specified line with a set offset. Returns an empty
  /// optional for
  ///  failure.
  std::optional<std::string> getline(std::size_t line_index,
  ...
  /// Appends an string. If `at_caret` is `true`, the string is inserted at the
  ...
  /// character is set. This is used for hiding some special text, such as
  /// password.
  ...
  void copy() const; ///< Copies the selected text into shared memory, such as
  ...
  void set_highlight(const std::string &name, const ::nana::color &fgcolor,
                     const ::nana::color &bgcolor);
  void erase_highlight(const std::string &name);
  void set_keywords(const std::string &name, bool case_sensitive,
  ...
  /// E.g. Whether caret moves to left of selected content or moves to left of
  /// last position when left arrow key is pressed.
  /// @param move_to_end determines whether to move caret to left of
  /// selected_content or to left of last position.
  void select_behavior(bool move_to_end);

  /// Sets the undo/redo queue length
  /**
   * @param len The length of the queue. If this parameter is zero, the
  ...
  /**
   * The number of display lines may be not equal to the number of text lines
   * when the textbox is line wrapped mode.
