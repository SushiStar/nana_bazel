...
template <typename ValueType> struct arg_categorize : public event_arg {
...
template <typename T> struct categorize_events : public general_events {
...
template <typename T> class event_agent : public event_agent_interface {
public:
  event_agent(::nana::categorize<T> &wdg) : widget_(wdg) {}
...
public:
  ...
  virtual ~renderer() = 0;
  virtual void background(graph_reference, window wd, const nana::rectangle &,
                          const ui_element &) = 0;
  virtual void root_arrow(graph_reference, const nana::rectangle &,
                          mouse_action) = 0;
  virtual void item(graph_reference, const nana::rectangle &, std::size_t index,
                    const ::std::string &name, unsigned textheight,
                    bool has_child, mouse_action) = 0;
...
public:
  ...
  // splitstr
  //@brief: Sets the splitstr. If the parameter will be ignored if it is an
  //empty string.
  ...
  template <typename T> void create_event_agent(::nana::categorize<T> &wdg) {
...
/// The categorize widget can be used for representing a path of a directory or
/// the order of a hierarchy.
template <typename T>
class categorize
    : public widget_object<category::widget_tag,
                           drawerbase::categorize::trigger,
                           drawerbase::categorize::categorize_events<T>> {
public:
  using native_string_type = widget::native_string_type;
  using value_type = T; ///< The type of objects stored
  using renderer = drawerbase::categorize::renderer; ///< The interface for
                                                     ///< user-defined renderer.

  categorize() { this->get_drawer_trigger().create_event_agent(*this); }
  ...
  /// and after inserting, the new category is replaced of the current category
  /// as a new current one.
  ...
  std::string splitstr() const { return this->get_drawer_trigger().splitstr(); }

  /// Retrieves a reference of the current category's value type object. If
  /// current category is empty, it throws a exception of std::runtime_error.
...
private:
  ...
  void _m_caption(native_string_type &&str) override {
    ...
    API::dev::window_caption(*this,
                             to_nstring(this->get_drawer_trigger().path()));
  }
};
} // end namespace nana
#endif
