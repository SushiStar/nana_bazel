...
struct arg_datechooser : public event_arg {
  date_chooser *const widget;

  arg_datechooser(date_chooser *wdg) : widget(wdg) {}
};

namespace drawerbase {
namespace date_chooser {
struct date_chooser_events : public general_events {
...
class date_chooser
    : public widget_object<category::widget_tag,
                           drawerbase::date_chooser::trigger,
                           drawerbase::date_chooser::date_chooser_events> {
public:
  date_chooser();
  date_chooser(window, bool visible);
  date_chooser(window, const nana::rectangle &r = rectangle(),
               bool visible = true);

  bool chose() const;
  nana::date read() const;
  void weekstr(unsigned index,
               ::std::string); ///< Set the week strings which will be displayed
                               ///< for day, index is in the range of [0, 6]
};
} // end namespace nana
