...
public:
   ...
   * @param maximum Sets the width of column to the maximum if the width of
   * content is larger than maximum
   */
...
class const_virtual_pointer {
  ...
  template <typename T> struct real_pointer : public intern {
    const T *ptr;

    real_pointer(const T *p) noexcept : ptr(p) {}
  };
...
public:
  ...
  ~const_virtual_pointer() noexcept { delete intern_; }
...
template <typename Value> struct container_translator {
...
template <typename STLContainer>
class standalone_container : public basic_container<STLContainer> {
  using value_type = typename STLContainer::value_type;
  using value_translator =
      typename container_translator<value_type>::value_translator;
  using cell_translator =
      typename container_translator<value_type>::cell_translator;

public:
  standalone_container(STLContainer &&cont, value_translator vtrans,
                       cell_translator ctrans)
      : container_(std::move(cont)), translator_({vtrans, ctrans}) {}

  standalone_container(const STLContainer &cont, value_translator vtrans,
                       cell_translator ctrans)
...
private:
  ...
  void emplace_back() override { container_.emplace_back(); }
  ...
  void *pointer() override { return &container_; }

  const void *pointer() const override { return &container_; }
...
template <typename STLContainer>
class shared_container : public basic_container<STLContainer> {
  using value_type = typename STLContainer::value_type;
  using value_translator =
      typename container_translator<value_type>::value_translator;
  using cell_translator =
...
public:
  ...
  shared_container(container_reference cont, value_translator vtrans,
                   cell_translator ctrans)
      : container_(cont), translator_({vtrans, ctrans}) {}
...
template <typename STLContainer>
class shared_immutable_container : public basic_container<STLContainer> {
  using value_type = typename STLContainer::value_type;
  using cell_translator =
      typename container_translator<value_type>::cell_translator;
...
private:
  void clear() override {
    throw std::runtime_error(
        "nana::listbox disallow to remove items because of immutable model");
  }
  ...
  void assign(std::size_t /*pos*/,
              const std::vector<cell> & /*cells*/) override {
    throw std::runtime_error(
        "nana::listbox disallow to remove items because of immutable model");
  }
  ...
  void *pointer() override { return nullptr; }
...
public:
  model_guard(model_interface *model) : model_(model) { model->lock(); }

  model_guard(model_guard &&other) : model_(other.model_) {
  ...
  template <typename STLContainer> STLContainer &container() {
    ...
    if (nullptr == p->pointer())
      throw std::runtime_error(
          "the modal is immutable, please declare model_guard with const");

    return *static_cast<stlcontainer *>(p->pointer());
  }

  template <typename STLContainer> const STLContainer &container() const {
    using stlcontainer = typename std::decay<STLContainer>::type;
...
class standalone_model_container : public model_interface {
public:
  using value_translator = typename container_translator<
      typename STLContainer::value_type>::value_translator;
  using cell_translator = typename container_translator<
      typename STLContainer::value_type>::cell_translator;

  standalone_model_container(STLContainer &&container, value_translator vtrans,
                             cell_translator ctrans)
      : container_(std::move(container), std::move(vtrans), std::move(ctrans)) {
  }

  standalone_model_container(const STLContainer &container,
                             value_translator vtrans, cell_translator ctrans)
  ...
  container_interface *container() noexcept override { return &container_; }
...
template <typename STLContainer, typename Mutex>
class shared_model_container : public model_interface {
public:
  using value_translator = typename container_translator<
      typename STLContainer::value_type>::value_translator;
  using cell_translator = typename container_translator<
  ...
  shared_model_container(STLContainer &container, value_translator vtrans,
                         cell_translator ctrans)
      : container_ptr_(new shared_container<STLContainer>(
            container, std::move(vtrans), std::move(ctrans))) {}

  shared_model_container(const STLContainer &container, cell_translator ctrans)
      : container_ptr_(new shared_immutable_container<STLContainer>(
            container, std::move(ctrans))) {}
...
struct index_pair {
  ...
  bool empty() const noexcept { return (npos == cat); }

  void set_both(size_type n) noexcept { cat = item = n; }

  bool is_category() const noexcept { return (npos != cat && npos == item); }
  ...
  bool operator<(const index_pair &r) const noexcept {
    return (cat < r.cat) || ((cat == r.cat) && (r.item != npos) &&
                             ((item == npos) || (item < r.item)));
  }

  bool operator>(const index_pair &r) const noexcept {
    return (cat > r.cat) || ((cat == r.cat) && (item != npos) &&
                             ((r.item == npos) || (item > r.item)));
  }
};

using index_pairs = ::std::vector<index_pair>;

enum class inline_widget_status { checked, checking, selected, selecting };

using inline_notifier_interface =
    detail::inline_widget_notifier_interface<index_pair, inline_widget_status,
                                             ::std::string>;

// struct essence
//@brief:	this struct gives many data for listbox,
//			the state of the struct does not effect on member
//functions, therefore all data members are public.
struct essence;
...
/// operate with absolute positions and contain only the position but maintain
/// pointers to parts of the real items item_proxy self, it references and
/// iterators are not invalidated by sort()
class item_proxy
    : public ::nana::widgets::detail::widget_iterator<std::input_iterator_tag,
                                                      item_proxy> {
public:
  item_proxy(essence *, const index_pair & = index_pair{npos, npos});

  /// the main purpose of this it to make obvious that item_proxy operate with
  /// absolute positions, and don't get moved during sort()
  static item_proxy from_display(essence *, const index_pair &relative);
  item_proxy from_display(const index_pair &relative) const;

  /// possible use: last_selected_display = last_selected.to_display().item; use
  /// with caution, it get invalidated after a sort()
   ...
   * @param scroll_view Indicates whether to scroll the view to the item. It is
   * ignored if the item is displayed.
  ...
  /// Converts a position of column between display position and absolute
  /// position
  /**
   * @param col The display position or absolute position.
   * @param disp_order Indicates whether the col is a display position or
   * absolute position. If this parameter is true, the col is display position
   * @return absolute position if disp_order is false, display position
   * otherwise.
   */
  ...
  template <typename T> item_proxy &resolve_from(const T &t) {
  ...
  template <typename T> void resolve_to(T &t) const {
    iresolver ires(_m_cells());
    ires >> t;
  }

  template <typename T> T const *value_ptr() const {
  ...
  template <typename T> T &value() const {
    ...
    if (nullptr == p)
      throw std::runtime_error(
          "listbox::item_proxy.value<T>() invalid type of value");
    return *p;
  }
  ...
  template <typename T> item_proxy &value(T &&t) {
...
private:
  ...
  index_pair pos_; // Position of an item, it never represents a category when
                   // item proxy is available.
};
...
public:
  using inline_notifier_interface =
      drawerbase::listbox::inline_notifier_interface;
  template <typename Value>
  using value_translator =
      typename container_translator<Value>::value_translator;
  template <typename Value>
  ...
  /// Append an item at the end of this category using the oresolver to generate
  /// the texts to be put in each column.
  ///
  /// First you have to make sure there is an overload of the operator<<() of
  /// the oresolver for the type of the object used here If a listbox have a
  /// model set, try call append_model instead.
  template <typename T>
  item_proxy
  append(T &&t, ///< Value used by the resolver to generate the texts to be put
                ///< in each column of the item
         bool set_value = false) ///< determines whether to set the object as
                                 ///< the value of this item.
  {
    oresolver ores(ess_);

    // Troubleshoot:
    // If a compiler error that no operator<< overload found for type T occurs,
    // please define a overload operator<<(oresolver&, const T&). If a listbox
    // have a model set, try call append_model instead.
    if (set_value)
  ...
  template <typename T> void append_model(const T &t) {
    nana::internal_scope_guard lock;
    _m_try_append_model(const_virtual_pointer{&t});
    _m_update();
  }

  template <typename Mutex, typename STLContainer, typename ValueTranslator,
            typename CellTranslator>
  void model(STLContainer &&container, ValueTranslator vtrans,
             CellTranslator ctrans) {
    _m_reset_model(
        new standalone_model_container<typename std::decay<STLContainer>::type,
                                       Mutex>(
            std::forward<STLContainer>(container), std::move(vtrans),
            std::move(ctrans)));
  }

  template <typename Mutex, typename STLContainer, typename ValueTranslator,
            typename CellTranslator>
  void shared_model(STLContainer &container, ValueTranslator vtrans,
                    CellTranslator ctrans) {
    _m_reset_model(
        new shared_model_container<typename std::decay<STLContainer>::type,
                                   Mutex>(container, std::move(vtrans),
                                          std::move(ctrans)));
  }
  ...
  void shared_model(const STLContainer &container, CellTranslator ctrans) {
    _m_reset_model(
        new shared_model_container<typename std::decay<STLContainer>::type,
                                   Mutex>(container, std::move(ctrans)));
  }

  model_guard model();

  /// Appends one item at the end of this category with the specifies texts in
  /// the column fields
  ...
  /// Enables/disables the number of items in the category to be displayed
  /// behind the category title
  ...
  /**
   * @param expand Indicates whether to expand or collapse the category. If this
   * parameter is true, it expands the category. If the parameter is false, it
   ...
   * @param from_display_order	If this parameter is true, the method converts
   *a display position to an absolute position. If the parameter is false, the
   *method converts an absolute position to a display position.
   * @return a display position or an absolute position that are depending on
   *from_display_order.
   */
...
private:
  ...
  category_t *cat_{nullptr};
  size_type pos_{0}; ///< Absolute position, not relative to display, and dont
                     ///< change during sort()
};

struct export_options {
  std::string sep = ::std::string{"\t"}, endl = ::std::string{"\n"};
  bool only_selected_items{true}, only_checked_items{false},
...
struct arg_listbox : public event_arg {
...
struct arg_listbox_category : public event_arg {
  drawerbase::listbox::cat_proxy category;

  /// A flag that indicates whether or not to block expansion/shrink of category
...
namespace drawerbase {
namespace listbox {
struct listbox_events : public general_events {
...
struct scheme : public widget_geometrics {
  ...
  color_proxy selection_box{
      static_cast<color_rgb>(0x3399FF)}; ///< Color of selection box border.

  std::shared_ptr<paint::font>
      column_font; ///< Renderer draws column texts with the font if it is not a
                   ///< nullptr.

  /// The max column width which is generated by fit_content is allowed. It is
  /// ignored when it is 0, or a max value is passed to fit_content.
  unsigned max_fit_content{0};

  unsigned min_column_width{20}; ///< def=20 . non counting suspension_width

  unsigned text_margin{
      5}; ///<  def= 5. Additional or extended with added (before) to the text
          ///<  width to determine the cell width. cell_w = text_w + ext_w +1

  unsigned item_height_ex{
      6}; ///< Set !=0 !!!!  def=6. item_height = text_height + item_height_ex
  ...
  ::nana::parameters::mouse_wheel
      mouse_wheel{}; ///< The number of lines/characters to scroll when
                     ///< vertical/horizontal mouse wheel is moved.
};
} // namespace listbox
} // end namespace drawerbase
...
This widget contain a list of \a categories, with in turn contain a list of \a
items. A \a category is a text with can be \a selected, \a checked and \a
expanded to show the \a items. An \a item is formed by \a column-fields, each
corresponding to one of the \a headers. An \a item can be \a selected and \a
checked. The user can \a drag the header to \a resize it or to \a reorganize it.
By \a clicking on one header the list get \a reordered, first up, and then down
alternatively.

1. The resolver is used to resolute an object of the specified type into (or
back from) a listbox item.
3. nana::listbox creates the category 0 by default.
   This is an special category, because it is invisible, while the associated
items are visible. The optional, user-created categories begin at index 1 and
are visible. The member functions without the categ parameter operate the items
that belong to category 0.
4. A sort compare is used for sorting the items. It is a strict weak ordering
comparer that must meet the requirement: Irreflexivity (comp(x, x) returns
false) and Antisymmetry(comp(a, b) != comp(b, a) returns true) A simple example.
...
nana::any*, bool reverse)
        ...
        The listbox supports attaching a customer's object for each item,
therefore the items can be sorted by comparing these customer's object. bool
sort_compare( const std::string&, nana::any* o1, const std::string&, nana::any*
o2, bool reverse)
                {
                        if(o1 && o2) 	//some items may not attach a customer
object.
                        {
                                int * i1 = any_cast<int>(*o1);
                                int * i2 = any_cast<int>(*o2);
                                return (i1 && i2 && (reverse ? *i1 > *i2 : *i1 <
*i2));
...
5. listbox is a widget_object, with template parameters
drawerbase::listbox::trigger and drawerbase::listbox::scheme among others. That
means that listbox have a member trigger_ constructed first and accessible with
get_drawer_trigger() and a member (unique pointer to) scheme_ accessible with
scheme_type& scheme() created in the constructor with
API::dev::make_scheme<Scheme>() which call
API::detail::make_scheme(::nana::detail::scheme_factory<Scheme>()) which call
restrict::bedrock.make_scheme(static_cast<::nana::detail::scheme_factory_base&&>(factory));
...
and which in create is set with: API::dev::set_scheme(handle_, scheme_.get());
which save the scheme pointer in the nana::detail::basic_window member pointer
scheme \todo doc: actualize this example listbox.at(0)... \see
nana::drawerbase::listbox::cat_proxy \see nana::drawerbase::listbox::item_proxy
...
public:
  ...
  /// Returns the category at specified location pos, no bounds checking is
  /// performed.
  cat_proxy operator[](size_type pos);
  const cat_proxy operator[](size_type pos) const;

  /// Returns the item at specified absolute position, no bounds checking is
  /// performed.
  ...
  /// Returns a proxy to the category of the key or create a new one in the
  /// right order
  /**
   * @param key The key of category to find
   * @return A category proxy
   */
  template <typename Key> cat_proxy assoc(Key &&key) {
    using key_type = typename ::nana::detail::type_escape<
        const typename std::decay<Key>::type>::type;

    auto p = std::make_shared<nana::key<key_type, std::less<key_type>>>(
        std::forward<Key>(key));
   ...
   * @return A category proxy
   */
  template <typename Key> cat_proxy assoc_at(Key &&key) {
    using key_type = typename ::nana::detail::type_escape<
        const typename std::decay<Key>::type>::type;
   ...
   */
  template <typename Key> void assoc_erase(Key &&key) {
    using key_type = typename ::nana::detail::type_escape<
        const typename std::decay<Key>::type>::type;
  ...
  void auto_draw(
      bool) noexcept; ///< Set state: Redraw automatically after an operation

  template <typename Function> void avoid_drawing(Function fn) {
  ...
  /// Scrolls the view to show an item specified by absolute position at
  /// top/bottom of the listbox.
  void scroll(bool to_bottom, const index_pair &abs_pos);

  /// Appends a new column with a header text and the specified width at the
  /// end, and return it position
  ///
  ...
  cat_proxy
  append(std::wstring category); ///< Appends a new category to the end
  void append(std::initializer_list<std::string>
                  categories); ///< Appends categories to the end
  void append(std::initializer_list<std::wstring>
                  categories); ///< Appends categories to the end

  /// Access a column at specified position
  /**
   * @param pos Position of column
   * @param disp_order Indicates whether the pos is display position or absolute
   * position.
   ...
   * @except std::out_of_range if !(pos < columns())
   */
  const column_interface &column_at(size_type pos,
                                    bool disp_order = false) const;
  ...
  /// Sort columns in range first_col to last_col inclusive using the values
  /// from a row
  void
  reorder_columns(size_type first_col, size_type last_col, index_pair row,
                  bool reverse,
                  std::function<bool(const std::string &cell1, size_type col1,
                                     const std::string &cell2, size_type col2,
                                     const nana::any *rowval, bool reverse)>
                      comp);
  ...
  void insert_item(index_pair abs_pos, const listbox &rhs,
                   const index_pairs &indexes);

  /// Returns the index pair of the item which contains the specified "screen"
  /// point.
   ...
   * @param return_end Indicates whether to return an end position instead of an
   * empty position if no item is hovered.
   * @return The position of the hovered item. If return_end is true and no item
   * is hovered it returns the position next to the last item of last category.
   */
  index_pair hovered(bool return_end) const;

  /// Returns the absolute position of the column which contains the specified
  /// "screen" point.
  size_type column_from_pos(const point &screen_pos) const;

  void checkable(bool make_checkeable); ///< Display a checkbox at te links of
                                        ///< each item if make_checkeable=true
  index_pairs checked() const; ///< Returns all the items which are checked.

  void
  clear(size_type cat); ///< Removes all the items from the specified category
  ...
  void set_sort_compare(
      size_type col,
      std::function<bool(const std::string &, nana::any *, const std::string &,
                         nana::any *, bool reverse)>
          strick_ordering);
  ...
  size_type
  sort_col() const; ///< return the column currently used to sort items

  /// Eliminate any "column sorting", effectively setting the items in the order
  /// of creation.
  ///
  /// Potentially invalidates any existing reference from display position to
  /// absolute item, that is: after unsort() display offset may point to
  /// different items
  void unsort();
  ...
  index_pairs
  selected() const; ///< Get the absolute indexes of all the selected items

  void show_header(bool);
  bool visible_header() const;
  void move_select(bool upwards); ///< Selects an item besides the current
                                  ///< selected item in the display.

  size_type size_categ() const; ///< Get the number of categories
  size_type
  size_item(size_type cat) const; ///< The number of items in category "cat"

  void enable_single(bool for_selection, bool category_limited);
  void disable_single(bool for_selection);
  bool is_single_enabled(bool for_selection) const
      noexcept; ///< Determines whether the single selection/check is enabled.
  export_options &def_export_options(); ///< return a modifiable reference to
                                        ///< the export options in use
   ...
   */
  listbox &
  category_icon(std::function<void(paint::graphics &graph,
                                   const rectangle &rt_icon, bool expanded)>
                    icon_renderer);

  /// Sets category icons
  /**
   * @param img_expanded An icon displayed in front of category title when the
   * category is expanded.
   * @param img_collapsed An icon displayed in front of category title when the
   * category is collapsed.
   * @return the reference of *this.
   */
  listbox &category_icon(const paint::image &img_expanded,
  ...
  /// Sets a predicate that indicates whether to deselect items when mouse_up is
  /// triggered.
  /**
   * The predicate is called before the listbox attempts to deselect the
   *selected items in the mouse_up event. Other situations, the predicates isn't
   *called, for example, releasing mouse button after user performed a box
   *selection, because listbox doesn't deselect the items during this operation.
   * @param predicate Decides to deselect the items.
   *	The paramater of predicate indicates the mouse button which is
   *releasing. It returns true to deselect the selected items. It returns false
   *to cancel to deselect the selected items.
   */
...
private:
  ...
  nana::any *_m_anyobj(size_type cat, size_type index,
                       bool allocate_if_empty) const override;
  drawerbase::listbox::category_t *
  _m_assoc(std::shared_ptr<nana::detail::key_interface>,
