...
namespace nana {
/// A list box combined with a textbox - the list box should drop down when the
/// user selects the arrow next to the control
class combox;

struct arg_combox : public event_arg {
  combox &widget;
  arg_combox(combox &);
};
...
namespace combox {
struct combox_events : public general_events {
  ...
  template <typename T> T &value() const {
    ...
    if (nullptr == p)
      throw std::runtime_error(
          "combox::item_proxy.value<T>() invalid type of value");
    return *p;
  }

  template <typename T> item_proxy &value(T &&val) {
...
class combox
    : public widget_object<category::widget_tag, drawerbase::combox::trigger,
                           drawerbase::combox::combox_events,
...
public:
  ...
  std::size_t
  option() const; ///< Index of the last selected, from drop-down list, item.
  void option(std::size_t); ///< Select the text specified by index
  ::std::string text(std::size_t) const;
  void erase(std::size_t pos);

  template <typename Key> item_proxy operator[](const Key &kv) {
    typedef typename nana::detail::type_escape<Key>::type key_t;
    std::shared_ptr<nana::detail::key_interface> p(
        new nana::key<key_t, std::less<key_t>>(kv),
        [](nana::detail::key_interface *p) { delete p; });

    return _m_at_key(std::move(p));
  }

  template <typename Key> void erase_key(Key &&kv) {
    typedef typename nana::detail::type_escape<Key>::type key_t;
    std::unique_ptr<nana::detail::key_interface> p(
        new nana::key<key_t, std::less<key_t>>(std::forward<Key>(kv)));
    _m_erase(p.get());
  }

  /// \brief Set user-defined item renderer object.
  /// It is an address therefore the user should not destroy the renderer object
  /// after it is set to the combox. Passing null_ptr cancels the user-defined
  /// renderer object.
  ...
  void image_pixels(
      unsigned); ///< Sets the width of image area. Default is 16 pixels.
private:
