...
struct arg_slider : public event_arg {
  slider &widget;
...
struct scheme_impl : public widget_geometrics {
  /// Colors
...
struct slider_events : public general_events {
  basic_event<arg_slider> value_changed;
};

enum class seekdir { bilateral, forward, backward };
...
public:
  ...
  virtual void background(window, graph_reference, bool transparent,
                          const scheme &) = 0;
  virtual void adorn(window, graph_reference, const data_adorn &,
                     const scheme &) = 0;
  virtual void vernier(window, graph_reference, const data_vernier &,
                       const scheme &) = 0;
  virtual void bar(window, graph_reference, const data_bar &,
                   const scheme &) = 0;
  virtual void slider(window, graph_reference, mouse_action,
                      const data_slider &, const scheme &) = 0;
};
...
class slider
    : public widget_object<category::widget_tag, drawerbase::slider::trigger,
                           drawerbase::slider::slider_events,
                           drawerbase::slider::scheme_impl> {
public:
  using renderer_interface =
      drawerbase::slider::renderer_interface; ///< The interface for customized
                                              ///< renderer.
  using seekdir =
      drawerbase::slider::seekdir; ///< Defines the slider seek direction.
  ...
  void seek(
      seekdir); ///< Define the direction that user can seek by using mouse.
  void vertical(bool);
  bool vertical() const;
...
*/
  ...
  unsigned
  move_step(bool forward); ///< Increase or decrease the value of slider.
  unsigned adorn() const;

  const pat::cloneable<renderer_interface> &
  renderer(); ///< Refers to the current renderer that slider is using.
  void renderer(const pat::cloneable<renderer_interface>
                    &); ///< Set the current renderer.

  void
  vernier(std::function<std::string(unsigned maximum, unsigned cursor_value)>
              provider);
