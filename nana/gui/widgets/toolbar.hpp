...
struct arg_toolbar : public event_arg {
  toolbar &widget;
  std::size_t button;
...
namespace toolbar {
enum class tool_type { button, toggle };
...
public:
  ...
  bool toggle()
      const; ///< Gets the tool toggle state (only if tool style is toggle).
  item_proxy &toggle(bool toggle_state); ///< Sets the tool toggle state (only
                                         ///< if tool style is toggle).
  std::string
  toggle_group() const; ///< Returns the toggle group associated with the tool
                        ///< (only if tool style is toggle).
  item_proxy &
  toggle_group(const ::std::string &group); ///< Adds the tool to a toggle group
                                            ///< (only if tool style is toggle).
...
struct item_type {
  ...
  item_type(const std::string &text, const nana::paint::image &img,
            tool_type type)
      : text(text), image(img), type(type) {}
};

struct toolbar_events : public general_events {
...
class toolbar
    : public widget_object<category::widget_tag, drawerbase::toolbar::drawer,
                           drawerbase::toolbar::toolbar_events> {
public:
  ...
  drawerbase::toolbar::item_proxy
  append(const ::std::string &text,
         const nana::paint::image &img); ///< Adds a control button.
  drawerbase::toolbar::item_proxy
  append(const ::std::string &text); ///< Adds a control button.
  ...
  bool istoggle(
      size_type index) const; ///< Returns true if the tool style is toggle.
  bool toggle(size_type index)
      const; ///< Gets the tool toggle state (only if tool style is toggle).
  void toggle(size_type index,
              bool toggle_state); ///< Sets the tool toggle state (only if tool
                                  ///< style is toggle).
  std::string toggle_group(
      size_type index) const; ///< Returns the toggle group associated with the
                              ///< tool (only if tool style is toggle).
  void
  toggle_group(size_type index,
               const ::std::string &group); ///< Adds the tool to a toggle group
                                            ///< (only if tool style is toggle).

  void textout(size_type index,
               bool show); ///< Show/Hide the text inside the button

  void scale(unsigned s); ///< Sets the scale of control button.

  /// Enable to place buttons at right part. After calling it, every new button
  /// is right aligned.
  void go_right();
