...
public:
  ...
  typedef drawerbase::menu::menu_item_type::event_fn_t
      event_fn_t; ///< A callback functor type. Prototype: `void(item_proxy&)`
   ...
   */
  item_proxy insert(std::size_t pos, std::string text_utf8,
                    const event_fn_t &handler = {});

  void clear(); ///< Erases all of the items.
  /// Closes the menu. It does not destroy the menu; just close the window for
  /// the menu.
  void close();
  ...
  void enabled(
      std::size_t pos,
      bool); ///< Enables or disables the mouse or keyboard input for the item.
  bool enabled(std::size_t pos) const;
  void erase(std::size_t pos); ///< Removes the item
  bool link(std::size_t pos,
            menu &menu_obj); ///< Link a menu to the item as a sub menu.
  menu *
  link(std::size_t pos) const; ///< Retrieves a linked sub menu of the item.
  menu *create_sub_menu(std::size_t pos);
  void popup(window owner, int x,
             int y); ///< Popup the menu at the owner window.
  void popup_await(window owner, int x, int y);
  void answerer(std::size_t index,
                const event_fn_t &); ///< Modify answerer of the specified item.
  void destroy_answer(
      std::function<void()>); ///< Sets an answerer for the callback while the
                              ///< menu window is closing.
  void gaps(const nana::point &); ///< Sets the gap between a menu and its sub
                                  ///< menus.(\See Note4)
  void
  goto_next(bool forward); ///< Moves the focus to the next or previous item.
  bool goto_submen(); ///< Popup the submenu of the current item if it has a sub
                      ///< menu. Returns true if succeeds.
...
detail::popuper menu_popuper(menu &, window owner, const point &,
