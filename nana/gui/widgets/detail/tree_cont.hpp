...
template <typename T> struct tree_node {
  ...
  tree_node(tree_node *owner) : owner(owner), next(nullptr), child(nullptr) {}
...
template <typename UserData> class tree_cont {
...
public:
  ...
  tree_cont() : root_(nullptr) {}

  ~tree_cont() { clear(&root_); }
  ...
  node_type *insert(node_type *node, const std::string &key,
  ...
  template <typename Functor> void for_each(node_type *node, Functor f) const {
    if (nullptr == node)
      node = root_.child;
    int state = 0; // 0: Sibling, the last is a sibling of node
                   // 1: Owner, the last is the owner of node
                   //>= 2: Children, the last is is a child of the node that
                   // before this node.
    while (node) {
  ...
  unsigned child_size_if(const ::std::string &key, PredAllowChild pac,
                         PredAllowNode pan) const {
    auto node = _m_locate(key);
    return (node ? child_size_if<PredAllowChild, PredAllowNode>(*node, pac, pan)
                 : 0);
  }

  template <typename PredAllowChild, typename PredAllowNode>
  unsigned child_size_if(const node_type &node, PredAllowChild pac,
                         PredAllowNode pan) const {
    unsigned size = 0;
  ...
  template <typename PredAllowChild, typename PredAllowNode>
  std::size_t distance_if(const node_type *node, PredAllowChild pac,
                          PredAllowNode pan) const {
    if (nullptr == node)
      return 0;
...
private:
  ...
  struct each_make_node {
    each_make_node(self_type &self) : node(&(self.root_)) {}
  ...
  struct find_key_node {
    find_key_node(const self_type &self) : node(&self.root_) {}
