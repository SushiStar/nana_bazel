 ...
 *		The treebox would have a vertical scrollbar if there are too
 *many nodes to display. It does not have an horizontal scrollbar: the widget
 *will adjust the node's displaying position for fitting.
 */
...
struct scheme : public widget_geometrics {
  color_proxy item_bg_selected{
      static_cast<color_rgb>(0xD5EFFC)}; ///< item selected: background color
  color_proxy item_fg_selected{
      static_cast<color_rgb>(0x99DEFD)}; ///< item selected: foreground color
  color_proxy item_bg_highlighted{
      static_cast<color_rgb>(0xE8F5FD)}; ///< item highlighted: background color
  color_proxy item_fg_highlighted{
      static_cast<color_rgb>(0xD8F0FA)}; ///< item highlighted: foreground color
  color_proxy item_bg_selected_and_highlighted{static_cast<color_rgb>(
      0xC4E8FA)}; ///< item selected and highlighted: background color
  color_proxy item_fg_selected_and_highlighted{static_cast<color_rgb>(
      0xB6E6FB)}; ///< item selected and highlighted: foreground color
  ...
  unsigned crook_size{16};  ///< crook size in pixels (TODO: the function that
                            ///< draw the crook doesn't scale the shape)

  unsigned indent_displacement{18}; ///< children position displacement in
                                    ///< pixels (def=18 (before was 10))
};

typedef widgets::detail::compset<component, node_attribute> compset_interface;
typedef widgets::detail::compset_placer<component, node_attribute, scheme>
...
class item_proxy : public std::iterator<std::input_iterator_tag, item_proxy> {
public:
  ...
  /// Expand/Shrink children of the node, and returns itself.  \todo change to
  /// expand ??
  ...
  /// Makes an action for each sub item recursively, returns the item that stops
  /// the action where action returns false.
  item_proxy visit_recursively(std::function<bool(item_proxy)> action);

  bool operator==(
      const ::std::string &s) const;       ///< Compare the text of node with s.
  ...
  template <typename T> const T *value_ptr() const {
    return any_cast<T>(&_m_value());
  }

  template <typename T> T *value_ptr() { return any_cast<T>(&_m_value()); }
...
struct arg_treebox : public event_arg {
...
namespace drawerbase {
namespace treebox {
struct treebox_events : public general_events {
...
/// \brief  Displays a hierarchical list of items, such as the files and
/// directories on a disk. See also in
/// [documentation](http://nanapro.org/en-us/documentation/widgets/treebox.htm)
class treebox
...
public:
  ...
  /// The interface of treebox compset_placer to define the position of node
  /// components
  typedef drawerbase::treebox::compset_placer_interface
      compset_placer_interface;
  ...
  /// \brief  The construct that creates a widget.
  /// @param wd  A handle to the parent window of the widget being created.
  /// @param r  the size and position of the widget in its parent window
  /// coordinate.
  ...
  {
    get_drawer_trigger().renderer() =
        ::nana::pat::cloneable<renderer_interface>{rd};
    return *this;
  }

  const nana::pat::cloneable<renderer_interface> &
  renderer() const; ///< get user-defined node renderer

  template <typename Placer>
  treebox &placer(const Placer &r) ///< location of a node components
  {
    get_drawer_trigger().placer(
        ::nana::pat::cloneable<compset_placer_interface>(r));
    return *this;
  }
  ...
  /// under some circumstances, it is good to disable the automatic drawing
  /// mode, for example, before adding nodes in a loop, disable the mode
  /// avoiding frequent and useless refresh for better performance, and then,
  /// after the operations, enable the automatic redraw mode again.
  ...
  /// If 'hovered' or 'expanded' are not set, it uses 'normal' state image for
  /// these 2 states. See also in
  /// [documentation](http://nanapro.org/en-us/help/widgets/treebox.htm)
  /// @param id The name of an icon scheme. If the name is not existing, it
  /// creates a new scheme for the name.
  /// @return The reference of node image scheme corresponding with the
  /// specified id.
  node_image_type &icon(const ::std::string &id);

  void icon_erase(const ::std::string &id);

  item_proxy find(const ::std::string
                      &keypath); ///< Find an item through a specified keypath.

  /// Inserts a new node to treebox, but if the keypath exists change and
  /// returns the existing node.
  item_proxy
  insert(const ::std::string &path_key, ///< specifies the node hierarchy
  ...
  /// returns the existing node.
  item_proxy
  insert(item_proxy pos,           ///< the parent item node
         const ::std::string &key, ///< specifies the new node
         ::std::string title ///< title used for displaying in the new node.
  );

  item_proxy erase(item_proxy i); ///< Removes the node at i and return the Item
                                  ///< proxy following the removed node

  void
  erase(const ::std::string &keypath); ///< Removes the node by the key path.

  ::std::string
  make_key_path(item_proxy i,
                const ::std::string &splitter) const; ///< returns the key path
  ...
  /// Enable/disable the interactions (selection, click, ...) on the entire line
  /// of the treebox.
  /**
   * @param enable bool  whether to enable.
   */
