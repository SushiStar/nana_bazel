 ...
 *	 @brief A tabbar contains tab items and toolbox for scrolling, closing,
 *selecting items.
 *
 */
...
template <typename T> struct arg_tabbar : public event_arg {
  tabbar<T> &widget;
  T &value;
...
template <typename T> struct arg_tabbar_click : public arg_tabbar<T> {
...
template <typename T> struct arg_tabbar_mouse : public arg_mouse {
...
template <typename T> struct arg_tabbar_adding : public event_arg {
  arg_tabbar_adding(tabbar<T> &wdg, std::size_t p) : widget(wdg), where(p) {}
...
template <typename T> struct arg_tabbar_removed : public arg_tabbar<T> {
  arg_tabbar_removed(tabbar<T> &wdg, T &v, std::size_t p)
      : arg_tabbar<T>({wdg, v, p}) {}

  mutable bool remove = true; ///< determines whether to remove the item
  mutable bool close_attach_window =
      true; ///< determines whether to close the attached window. It is ignored
            ///< if remove is false
};

namespace drawerbase {
namespace tabbar {
template <typename T> struct tabbar_events : public general_events {
  using value_type = T;
...
public:
  ...
  virtual void background(graph_reference, const nana::rectangle &r,
                          const ::nana::color &bgcolor) = 0;
  virtual void item(graph_reference, const item_t &, bool active, state_t) = 0;
  virtual void close_fly(graph_reference, const nana::rectangle &, bool active,
                         state_t) = 0;
...
class event_agent : public event_agent_interface {
public:
  ...
  void added(std::size_t pos) override {
    if (pos != npos) {
      ...
      tabbar_.events().added.emit(arg_tabbar({tabbar_, tabbar_[pos], pos}),
                                  tabbar_);
    }
  }
  ...
  void activated(std::size_t pos) override {
    if (pos != npos)
      tabbar_.events().activated.emit(arg_tabbar({tabbar_, tabbar_[pos], pos}),
                                      tabbar_);
  }
...
class tabbar
    : public widget_object<category::widget_tag, drawerbase::tabbar::trigger,
                           drawerbase::tabbar::tabbar_events<Type>> {
  typedef drawerbase::tabbar::trigger drawer_trigger_t;

public:
  using value_type =
      Type; ///< The type of element data which is stored in the tabbar.
  using item_renderer =
      drawerbase::tabbar::item_renderer; ///< A user-defined item renderer
  ...
  tabbar() {
    evt_agent_.reset(
        new drawerbase::tabbar::event_agent<value_type, drawer_trigger_t>(
            *this, this->get_drawer_trigger()));
    this->get_drawer_trigger().set_event_agent(evt_agent_.get());
  }

  tabbar(window wd, bool visible) : tabbar() {
  ...
  ~tabbar() { this->get_drawer_trigger().set_event_agent(nullptr); }

  value_type &operator[](std::size_t pos) const {
    return any_cast<value_type &>(
        this->get_drawer_trigger().at_no_bound_check(pos));
  }
  ...
  /// Append a new tab
  tabbar &append(std::string text, window attach_wd,
                 value_type value =
                     {}) // 2x text convertion. maybe better to duplicate code?
  {
    return this->append(static_cast<std::wstring &&>(nana::charset(
                            std::move(text), nana::unicode::utf8)),
                        attach_wd, std::move(value));
  }

  tabbar &append(std::wstring text, window attach_wd, value_type value = {}) {
    if (attach_wd && API::empty_window(attach_wd))
      throw std::invalid_argument("Appending a tab to a tabbar - error: "
                                  "tabbar.attach: invalid window handle");

    this->get_drawer_trigger().insert(::nana::npos, to_nstring(std::move(text)),
                                      std::move(value));
    if (attach_wd)
  ...
  void insert(std::size_t pos, std::string text, value_type value = {}) {
    ...
    this->get_drawer_trigger().insert(pos, to_nstring(std::move(text)),
                                      std::move(value));
    API::update_window(*this);
  }
  ...
  /// Attach a window to a specified tab. When the tab is activated, tabbar
  /// shows the attached window.
  /**
   * @param pos The position of tab to set the attached window.
   * @param attach_wd A handle to the window to be set.
   * @param drop_other Drop the attached windows of other tabs whose attach
   * windows are equal to the parameter attach_wd. If drop_other is true, the
   * other tabs attached windows equal to attach_wd will be dropped.
  ...
  void erase(std::size_t pos) { this->get_drawer_trigger().erase(pos); }
  ...
  void text(std::size_t pos,
            const std::string &
                str) /// Sets the title of the specified item, If pos is
                     /// invalid, the method throws an std::out_of_range object.
  {
    this->get_drawer_trigger().text(pos, to_nstring(str));
  }

  std::string text(std::size_t pos)
      const /// Returns a title of a specified item, If pos is invalid, the
            /// method throws a std::out_of_range object.
  {
...
private:
  std::unique_ptr<
      drawerbase::tabbar::event_agent<value_type, drawerbase::tabbar::trigger>>
      evt_agent_;
};
} // end namespace nana
...
struct events : public general_events {
  basic_event<event_arg> selected;
};

class driver : public drawer_trigger {
public:
  driver();
  ~driver();
...
class tabbar_lite : public widget_object<category::widget_tag,
                                         drawerbase::tabbar_lite::driver,
                                         drawerbase::tabbar_lite::events> {
public:
  tabbar_lite() = default;
