...
public:
  ...
  virtual void render(widget_reference, graph_reference,
                      const nana::rectangle &, const item_interface *,
                      state_t) = 0;
...
class float_listbox : public widget_object<category::root_tag,
                                           drawerbase::float_listbox::trigger> {
  typedef widget_object<category::root_tag, drawerbase::float_listbox::trigger>
      base_type;
...
public:
   ...
   *	@param is_ignore_first_mouse_up	The flost_listbox will be closed when a
   *mouse_up is emitted, this parameter is specified for ignoring the first
   *mouse_up emitting.
   *	@remark	The float_listbox can be pop-upped in a mouse_down event, the
   *next mouse_up may be ignored, otherwise the float_listbox will be closed
