...
struct arg_scroll : public event_arg {
  window window_handle;

  arg_scroll(window wd) : window_handle{wd} {}
};

namespace drawerbase {
namespace scroll {
struct scroll_events : public general_events {
  basic_event<arg_scroll> value_changed;
};

enum class buttons { none, forward, backward, scroll, first, second };

struct metrics_type {
  using size_type = std::size_t;

  size_type peak;  ///< the whole total
  size_type range; ///< how many is shown on a page, that is, How many to scroll
                   ///< after click on first or second
  ...
  buttons what;
  bool pressed;
  size_type scroll_length; ///< the length in pixels of the central button show
                           ///< how many of the total (peak) is shown (range)
  int scroll_pos; ///< in pixels, and correspond to the offset from the very
                  ///< beginning (value)
...
class drawer {
public:
  enum class states { none, highlight, actived, selected };

  using graph_reference = paint::graphics &;
  const static unsigned fixedsize =
      16; // make it part of a new "metric" in the widget_scheme
...
template <bool Vertical> class trigger : public drawer_trigger {
public:
  typedef metrics_type::size_type size_type;

  trigger() : graph_(nullptr), drawer_(Vertical) {}

  const metrics_type &metrics() const { return drawer_.metrics; }
  ...
  void step(size_type s) { drawer_.metrics.step = (s ? s : 1); }
  ...
  bool make_step(bool forward, unsigned multiple) {
    ...
    size_type step =
        (multiple > 1 ? drawer_.metrics.step * multiple : drawer_.metrics.step);
...
private:
  ...
  void detached() override { graph_ = nullptr; }

  void refresh(graph_reference graph) override { drawer_.draw(graph); }
  ...
  void mouse_enter(graph_reference graph,
                   const ::nana::arg_mouse &arg) override {
    drawer_.metrics.what = drawer_.what(graph, arg.pos);
    drawer_.draw(graph);
    API::dev::lazy_refresh();
  }

  void mouse_move(graph_reference graph,
                  const ::nana::arg_mouse &arg) override {
  ...
  void mouse_down(graph_reference graph, const arg_mouse &arg) override {
    if (arg.left_button) {
      ...
      case buttons::scroll:
        ...
        drawer_.metrics.scroll_mouse_offset =
            (Vertical ? arg.pos.y : arg.pos.x) - drawer_.metrics.scroll_pos;
        break;
...
public:
  ...
  /// @param for_less  whether it can be scrolled for a less value (backward or
  /// "up" if true, forward or "down" if false).
  ...
  /// Get the range of the widget (how many is shown on a page, that is, How
  /// many to scroll after click on first or second)
  ...
  ///  \brief Get the step of the sroll widget. The step indicates a variation
  ///  of the value.
  /// @return the step.
  ...
  ///  \brief Increase/decrease values by a step (alternatively by some number
  ///  of steps).
...
/// Provides a way to display an object which is larger than the window's client
/// area.
template <bool Vertical>
class scroll // add a widget scheme?
    : public widget_object<category::widget_tag,
                           drawerbase::scroll::trigger<Vertical>,
                           drawerbase::scroll::scroll_events>,
      public scroll_interface {
  typedef widget_object<category::widget_tag,
                        drawerbase::scroll::trigger<Vertical>>
      base_type;
...
public:
  ...
  scroll(window wd, bool visible = true) {
    this->create(
        wd, rectangle(),
        visible); // add a widget scheme? and take some colors from these wd?
  }
  ...
  ///  \brief Set the step.
  /// @param s  a value for step.
  void step(size_type s) override { return this->get_drawer_trigger().step(s); }
  ...
  window window_handle() const override { return this->handle(); }

  ///  \brief Increase/decrease values by steps as if it is scrolled through
  ///  mouse wheel.
  /// @param forward  it determines whether increase or decrease.
  /// @return true if the value is changed.
  bool make_scroll(bool forward) {
    return this->make_step(
        forward, 3); // set this 3 in the metrics of the widget scheme ?
  }
  ...
  bool make_page_scroll(bool forward) {
    ...
    return this->make_step(forward,
                           static_cast<unsigned>(count > 2 ? count - 1 : 1));
  }
}; // end class scroll
} // end namespace nana
#endif
