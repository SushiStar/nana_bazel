...
struct arg_checkbox : public event_arg {
  checkbox *const widget;

  arg_checkbox(checkbox *wdg) : widget(wdg) {}
};
...
struct events_type : public general_events {
...
class checkbox
    : public widget_object<category::widget_tag, drawerbase::checkbox::drawer,
                           drawerbase::checkbox::events_type,
                           drawerbase::checkbox::scheme> {
public:
  checkbox();
  ...
  void react(
      bool want); ///< Enables the reverse check while clicking on the checkbox.
  bool checked() const;

  /// Checks/unchecks the checkbox
  void check(bool state);

  /// \brief With the radio mode, users make a choice among a set of mutually
  /// exclusive, related options. Users can choose one and only one option.
...
public:
  ...
  ///< Retrieves the index of checked option. It returns radio_group::npos if no
  ///< checkbox is checked.
  std::size_t checked() const;
  std::size_t size() const;

  template <typename Function> void on_clicked(Function &&click_fn) {
    for (auto &e : ui_container_) {
      e.uiobj->events().click(std::move(click_fn));
    }
  }

  template <typename Function> void on_checked(Function &&check_fn) {
    for (auto &e : ui_container_) {
