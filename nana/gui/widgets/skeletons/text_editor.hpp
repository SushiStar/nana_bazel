...
public:
  ...
  struct renderers {
    std::function<void(graph_reference, const nana::rectangle &text_area,
                       const ::nana::color &)>
        background; ///< a customized background renderer
    std::function<void(graph_reference, const ::nana::color &)>
        border; ///< a customized border renderer
  };

  enum class accepts { no_restrict, integer, real };
  ...
  void set_highlight(const ::std::string &name, const ::nana::color &,
                     const ::nana::color &);
  void erase_highlight(const ::std::string &name);
  void set_keyword(const ::std::wstring &kw, const std::string &name,
                   bool case_sensitive, bool whole_word_matched);
  ...
  /// Set the text_editor whether it is line wrapped, it returns false if the
   ...
   * @param enable_cart Indicates whether to show or hide the caret when the
   * text_editor is not editable. It is ignored if enable is false.
   */
   ...
   * @param stay_in_view Indicates whether to adjust the view to make the caret
   * in view. This parameter is ignored if the caret is already in view.
...
private:
  nana::color
  _m_draw_colored_area(paint::graphics &graph,
  ...
  ::nana::point _m_caret_to_coordinate(::nana::upoint pos,
                                       bool to_screen_coordinate = true) const;
  // Screen coordinate or context coordinate(in pixels) to caret,
  ::nana::upoint
  _m_coordinate_to_caret(::nana::point pos,
                         bool from_screen_coordinate = true) const;

  bool _m_pos_from_secondary(std::size_t textline,
                             const nana::upoint &secondary, unsigned &pos);
  bool _m_pos_secondary(const nana::upoint &charpos,
                        nana::upoint &secondary_pos) const;
  ...
  static bool
  _m_resolve_text(const ::std::wstring &,
  ...
  /// Returns the logical position that text starts of a specified line in
  /// x-axis
  int _m_text_x(const text_section &) const;

  void _m_draw_parse_string(const keyword_parser &, bool rtl, ::nana::point pos,
                            const ::nana::color &fgcolor, const wchar_t *,
                            std::size_t len) const;
  //_m_draw_string
  //@brief: Draw a line of string
  void _m_draw_string(int top, const ::nana::color &,
                      const nana::upoint &str_pos, const text_section &,
                      bool if_mask) const;
...
private:
  ...
  struct attributes {
    ...
    bool editable{true};
    bool enable_caret{true}; ///< Indicates whether to show or hide caret when
                             ///< text_editor is not editable
  ...
  struct coordinate {
    nana::upoint caret; // position of caret by text, it specifies the position
                        // of a new character
