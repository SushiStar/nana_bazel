...
// into two parts.
// Formatted tokens: The tokens present in a format block, they form the
// format-syntax. Data tokens: The tokens present a text displaying on the
// screen.
enum class token {
  tag_begin,
  tag_end,
...
public:
  tokenizer(const std::wstring &s, bool format_enabled)
      : iptr_(s.data()), endptr_(s.data() + s.size()),
        format_enabled_(format_enabled) {}

  void push(token tk) { revert_token_ = tk; }
  ...
  const ::std::wstring &idstr() const { return idstr_; }
  ...
  int number() const { return std::stoi(idstr_, nullptr, 0); }
...
private:
  ...
  token _m_token() {
    ...
    if (ch > 0xFF) {
      ...
      ch = *++iptr_;
      while ((iptr_ != endptr_) && (ch > 0xFF) &&
             (false == _m_unicode_word_breakable(iptr_))) {
        idstr_.append(1, ch);

        ch = *++iptr_;
      }

      // When the last _m_unicode_word_breakable returns true, it implies the
      // ch(left character) is not the breakable character. So it belongs to the
      // data.
      idstr_.append(1, ch);
      ++iptr_;
...
struct fblock {
  struct aligns {
    enum t { top, center, bottom, baseline };
  };
  ...
  aligns::t text_align;
  ::nana::color bgcolor; // If the color is not specified, it will be ignored,
                         // and the system will search for its parent.
...
public:
  data_text(const std::wstring &s) : str_(s) {}

private:
  virtual bool is_text() const override { return true; }

  virtual bool is_whitespace() const override { return false; }

  virtual const std::wstring &text() const override { return str_; }
  ...
  virtual const nana::size &size() const override { return size_; }

  virtual std::size_t ascent() const override { return ascent_; }
...
class data_image : public data {
public:
  data_image(const std::wstring &imgpath, const nana::size &sz,
             std::size_t limited)
...
private:
  ...
  virtual bool is_text() const override { return false; }
  ...
  virtual void nontext_render(graph_reference graph, int x, int y) override {
    if (size_ != image_.size())
      image_.stretch(::nana::rectangle{image_.size()}, graph,
                     nana::rectangle(x, y, size_.width, size_.height));
    else
  ...
  virtual std::size_t ascent() const override { return size_.height; }
...
public:
  ...
  void close() {
    for (auto &values : lines_) {
      for (std::deque<value>::iterator u = values.begin(); u != values.end();
           ++u)
        delete u->data_ptr;
    }
  ...
  void parse(const std::wstring &s, bool format_enabled) {
    ...
    while (true) {
      ...
      case token::tag_begin:
        ...
        if (attr_image_.path.size()) {
          ...
          // This fblock just serves the image. So we should restore the
          // pervious fblock
          fstack.pop();
        }
    ...
    for (auto &ln : lines_) {
      ...
      for (auto &e : entities) {
        ...
        // If the pos is not a start position, it indicates the block of bidi
        // entity has been inserted into the ln container. Because the content
        // of a block may be divided into multiple bidi entities.
        if (i == position.cend())
          continue;
...
private:
  void _m_parse_format(tokenizer &tknizer, std::stack<fblock *> &fbstack) {
    ...
    while (true) {
      ...
      case token::target:
        ...
        if (token::string != tknizer.read())
          throw std::runtime_error(
              "error: the value of 'target' should be a string");
  ...
  void _m_data_factory(token tk, const std::wstring &idstr, fblock *fp,
                       std::deque<value> &line) {
    value v;
    v.fblock_ptr = fp;
  ...
  void _m_data_image(fblock *fp, std::deque<value> &line) {
    ...
    v.data_ptr =
        new data_image(attr_image_.path, attr_image_.size, attr_image_.limited);
