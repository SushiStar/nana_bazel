 ...
 *	@description: This class manages the multi-line text and provides some
 *operation on text
 */
...
namespace widgets {
namespace skeletons {
template <typename CharT> class textbase : public ::nana::noncopyable {
public:
  ...
  bool load(const path_type &file) {
    ...
    while (ifs.good()) {
      ...
      text_cont_.emplace_back(
          static_cast<string_type &&>(nana::charset{str_mbs}));
  ...
  bool load(const path_type &file, nana::unicode encoding) {
    ...
    if (ifs.good()) {
      ...
      text_cont_.emplace_back(
          static_cast<string_type &&>(nana::charset{str, encoding}));
      attr_max_.size = text_cont_.back().size();
      attr_max_.line = 0;
    }
  ...
  void store(const path_type &filename, bool is_unicode,
    ...
    if (ofs && text_cont_.size()) {
      ...
      if (is_unicode) {
        const char *le_boms[] = {"\xEF\xBB\xBF", "\xFF\xFE",
                                 "\xFF\xFE\x0\x0"}; // BOM for little-endian
        int bytes = 0;
        switch (encoding) {
        case nana::unicode::utf8:
          bytes = 3;
      ...
      ofs.write(last_mbs.c_str(),
                static_cast<std::streamsize>(last_mbs.size()));
      _m_saved(filename);
    }
  }

  // Triggers the text_changed event.
  // It is exposed for outter classes. For a outter class(eg. text_editor), a
  // changing text content operation may contains multiple textbase operations,
  // therefore, the outter class determines when an event should be triggered.
  //
  // Addtional, using text_changed() method, it is possible to allow a outter
  // class performing some updating operations before triggering text_changed
  // event.
  void text_changed() {
    if (!changed_) {
  ...
  size_type lines() const { return text_cont_.size(); }
...
public:
  ...
  const path_type &filename() const { return filename_; }

  bool edited() const { return changed_; }
  ...
  bool saved() const { return !(changed_ || filename_.empty()); }

private:
  string_type &_m_at(size_type pos) { return *_m_iat(pos); }
