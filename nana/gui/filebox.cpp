...
class filebox_implement : public form {
  struct item_fs {
    std::string name;
    ::tm modified_time;
    bool directory;
    ...
    friend listbox::iresolver &operator>>(listbox::iresolver &ires,
                                          item_fs &m) {
      std::wstring type;
      ires >> m.name >> type >> type;
      m.directory =
          (to_utf8(type) == internationalization()("NANA_FILEBOX_DIRECTORY"));
      return ires;
    }

    friend listbox::oresolver &operator<<(listbox::oresolver &ores,
                                          const item_fs &item) {
      std::wstringstream tm;
      tm << (item.modified_time.tm_year + 1900)
         << '-'; /// \todo : use nana::filesystem_ext:: pretty_file_date
  ...
  private:
    ...
    static std::string
    _m_trans(std::size_t
                 bytes) /// \todo : use nana::filesystem_ext::pretty_file_size
    {
...
public:
  enum class mode {
    ...
    write_file, // choose a filename, it can be specified a new filename which
                // doesn't exist.
...
public:
  filebox_implement(window owner, mode dialog_mode, const std::string &title,
                    bool pick_directory, bool allow_multi_select)
      : form(owner, API::make_center(owner, 630, 440)),
        pick_directory_(pick_directory), mode_(dialog_mode) {
    ...
    path_.create(*this);
    path_.splitstr("/");
    path_.events().selected.connect_unignorable(
        [this](const arg_categorize<int> &) {
          ...
          if (path.size() == 0)
            path = "/"; /// \todo : use nana::filesystem_ext::def_rootstr?
    ...
    filter_.events().key_release.connect_unignorable(
        [this](const arg_keyboard &) { _m_list_fs(); });
    ...
    btn_folder_.events().click.connect_unignorable([this](const arg_click &) {
      ...
      btn_cancel.events().click.connect_unignorable(
          [&fm](const arg_click &) { fm.close(); });
      API::modal_window(fm);
    });
    ...
    ls_file_.events().selected.connect_unignorable(
        [this](const arg_listbox &arg) { _m_select_file(arg.item); });

    ls_file_.set_sort_compare(
        0,
        [](const std::string &a, nana::any *fs_a, const std::string &b,
           nana::any *fs_b, bool reverse) -> bool {
          ...
          while (true) {
            ...
            if ((pos_a != a.npos) && (pos_a == pos_b)) {
              ...
              auto num_a =
                  a.substr(pos_a, end_a != a.npos ? end_a - pos_a : a.npos);
              auto num_b =
                  b.substr(pos_b, end_b != b.npos ? end_b - pos_b : b.npos);
          ...
          return (reverse ? cia.substr(seek_a) > cib.substr(seek_b)
                          : cia.substr(seek_a) < cib.substr(seek_b));
        });
    ls_file_.set_sort_compare(
        2,
        [](const std::string &a, nana::any *anyptr_a, const std::string &b,
           nana::any *anyptr_b, bool reverse) -> bool {
    ...
    ls_file_.set_sort_compare(
        3,
        [](const std::string &, nana::any *anyptr_a, const std::string &,
           nana::any *anyptr_b, bool reverse) -> bool {
    ...
    const char *idstr =
        (mode::open_directory == dialog_mode ? "NANA_FILEBOX_DIRECTORY_COLON"
                                             : "NANA_FILEBOX_FILE_COLON");
    ...
    tb_file_.events().key_char.connect_unignorable(
        [this](const arg_keyboard &arg) {
          allow_fall_back_ = false;
          if (arg.key == nana::keyboard::enter)
            _m_try_select(tb_file_.caption());
        });

    // Don't create the combox for choose a file extension if the dialog is used
    // for picking a directory.
    if (!pick_directory) {
      cb_types_.create(*this);
      cb_types_.editable(false);
      cb_types_.events().selected.connect_unignorable(
          [this](const arg_combox &) { _m_list_fs(); });
    }

    btn_ok_.create(*this);
    btn_ok_.i18n(i18n_eval("NANA_BUTTON_OK_SHORTKEY"));

    btn_ok_.events().click.connect_unignorable(
        [this](const arg_click &) { _m_try_select(tb_file_.caption()); });

    btn_cancel_.create(*this);
    btn_cancel_.i18n(i18n_eval("NANA_BUTTON_CANCEL_SHORTKEY"));

    btn_cancel_.events().click.connect_unignorable(
        [this](const arg_click &) { API::close_window(handle()); });
  ...
  void def_extension(const std::string &ext) { def_ext_ = ext; }
  ...
  void load_fs(const std::string &init_path, const std::string &init_file) {
    ...
    auto filename =
        (pos != init_file.npos ? init_file.substr(pos + 1) : init_file);
...
private:
  void _m_layout() {
    ...
    place_.div(
        "vert"
        "<weight=" +
        std::to_string(text_height) +
        " margin=5 path arrange=[variable,200] gap=5>"
  ...
  void _m_init_tree() {
    // The path in linux is starting with the character '/', the name of root
    // key should be "FS.HOME", "FS.ROOT". Because a key of the tree widget
    // should not be '/'
  ...
  void _m_load_path(const std::string &path) {
    ...
    if (addr_.filesystem.size() &&
        addr_.filesystem[addr_.filesystem.size() - 1] != '/')
      addr_.filesystem += '/';
    ...
    for (fs::directory_iterator i(path); i != end; ++i) {
      ...
      if (ftype == fs::file_type::not_found ||
          ftype == fs::file_type::unknown || ftype == fs::file_type::directory)
        m.bytes = 0;
      else
  ...
  void _m_enter_folder(std::string path) {
    ...
    while (!beg_node.empty() && (beg_node != nodes_.home) &&
           (beg_node != nodes_.filesystem))
      beg_node = beg_node.owner();

    auto head = fs_ext::path_user().native();
    if (path.size() >= head.size() &&
        (path.substr(0, head.size()) == head)) { // This is HOME
      path_.caption("HOME");
    ...
    try {
      ...
      dw.draw([](paint::graphics &graph) {
        ...
        graph.string({static_cast<int>(sz.width - txt_sz.width) / 2,
                      static_cast<int>(sz.height - txt_sz.height) / 2},
  ...
  bool _m_filter_allowed(const std::string &name, bool is_dir,
    ...
    if (is_dir || (nullptr == extension) || extension->empty())
      return true;
  ...
  void _m_list_fs() {
    ...
    auto ext_types =
        cb_types_.anyobj<std::vector<std::string>>(cb_types_.option());
      ...
      if (_m_filter_allowed(fs.name, fs.directory, filter, ext_types)) {
        ...
        if (fs.directory)
          ...
          if (use_image.empty() && (size > 4)) {
            ...
            if ((".exe" == ext4) || (".dll" == ext4))
              use_image = images_.exec;
            else if ((".zip" == ext4) || (".rar" == ext4) || (".bz2" == ext4) ||
                     (".tar" == ext4))
              use_image = images_.package;
            else if (".txt" == ext4)
            ...
            else if ((".jpg" == ext4) || (".png" == ext4) || (".gif" == ext4) ||
                     (".bmp" == ext4))
  ...
  struct folder_creator {
    ...
    void operator()() {
      ...
      if (false == fs::create_directory(fspath)) {
        mb << i18n(
            "NANA_FILEBOX_ERROR_RENAME_FOLDER_BECAUSE_OF_FAILED_CREATION");
        mb();
        return;
      }
...
private:
  void _m_insert_filename(const std::string &name) {
    if (selection_.targets.cend() ==
        std::find(selection_.targets.cbegin(), selection_.targets.cend(), name))
  ...
  bool _m_sync_with_selection() {
    if (_m_has_good_select()) {
      auto selected_items = ls_file_.selected();
      if (selected_items.size() &&
          (selected_items.size() < selection_.targets.size())) {
  ...
  void _m_select_file(const listbox::item_proxy &item) {
    ...
    if (item.selected()) {
      ...
      for (auto i = selection_.targets.cbegin();
           i != selection_.targets.cend();) {
  ...
  void _m_display_target_filenames() {
    ...
    if (selection_.targets.size() == 1) {
      ...
      for (auto i = selection_.targets.crbegin();
           i != selection_.targets.crend(); ++i) {
        std::filesystem::path p{*i};
    ...
    for (auto tar : targets) {
      ...
      if (mode::open_directory == mode_) {
        ...
        if (!is_dir) {
          ...
          if (allow_fall_back_ &&
              (p.filename() == p.parent_path().filename())) {
            ...
            if (fs::file_type::not_found == ftype)
              i18n_idstr =
                  "NANA_FILEBOX_ERROR_DIRECTORY_NOT_EXISTING_AND_RETRY";
        ...
        if (mode::write_file != mode_) {
          if (fs::file_type::not_found == ftype) {
            ...
            if (mode::open_file == mode_)
              ...
              mb << i18n("NANA_FILEBOX_ERROR_DIRECTORY_NOT_EXISTING_AND_RETRY",
                         tar);
            mb();
    ...
    if (kind::filesystem == node.value<kind::t>()) {
      ...
      for (fs::directory_iterator i{path}; i != end; ++i) {
        ...
        if (!child.empty()) {
          ...
          // std::error_code& ec ) noexcept; in C++17
          try {
...
filebox::filebox(window owner, bool open) : impl_(new implement) {
  impl_->owner = owner;
...
filebox::filebox(const filebox &other) : impl_(new implement(*other.impl_)) {}

filebox::~filebox() { delete impl_; }
...
void filebox::owner(window wd) { impl_->owner = wd; }
...
filebox &filebox::add_filter(const std::string &description,
...
const filebox::path_type &filebox::path() const { return impl_->path; }
...
std::vector<filebox::path_type> filebox::show() const {
  ...
  if (!impl_->open_or_save)
    ...
    ofn.Flags = OFN_FILEMUSTEXIST; // In open mode, user can't type name of
                                   // nonexistent file
...

  {
    ...
    if (FALSE == (impl_->open_or_save ? ::GetOpenFileName(&ofn)
                                      : ::GetSaveFileName(&ofn)))
      return targets;
  }
  ...
  filebox_implement fb(
      impl_->owner, (impl_->open_or_save ? mode::open_file : mode::write_file),
      impl_->title, false, impl_->allow_multi_select);
...
folderbox::folderbox(window owner, const path_type &init_path,
                     std::string title)
    : impl_(new implement{owner,
                          fs::weakly_canonical(init_path).make_preferred(),
                          title, false}) {}

folderbox::~folderbox() { delete impl_; }
...
#ifdef NANA_MINGW
static int CALLBACK browse_folder_callback(HWND hwnd, UINT msg, LPARAM lparam,
                                           LPARAM data) {
...
std::vector<folderbox::path_type> folderbox::show() const {
  ...
  if (SUCCEEDED(hr)) {
    ...
    hr = SHCreateItemFromParsingName(impl_->init_path.wstring().c_str(),
                                     nullptr, IID_PPV_ARGS(&init_path));
    if (SUCCEEDED(hr))
      fd->SetFolder(init_path);

    fd->SetOptions(FOS_PICKFOLDERS |
                   (impl_->allow_multi_select ? FOS_ALLOWMULTISELECT : 0));
    fd->Show(reinterpret_cast<HWND>(API::root(
        impl_->owner))); // the native handle of the parent nana form goes here
  ...
  brw.pszDisplayName = display_text; // buffer to receive the display name of
