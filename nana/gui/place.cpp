...
struct badname : place::error {
  explicit badname(::std::string what, const place &plc,
...
public:
  ...
  struct error : std::invalid_argument {
    error(std::string what, const tokenizer &tok)
        ...
        : std::invalid_argument{what + " from tokenizer "}, pos{tok.pos()},
          div_str(tok.divstr_) {}
  ...
  tokenizer(const char *div_text) noexcept : divstr_(div_text), sp_(div_text) {}

  const std::string &idstr() const noexcept { return idstr_; }

  const number_t &number() const noexcept { return number_; }

  std::vector<number_t> &array() noexcept { return array_; }

  repeated_array &&reparray() noexcept { return std::move(reparray_); }
  ...
  token read() {
    ...
    case '[':
      ...
      if (*sp_ == ']') {
        ...
        while (true) {
          ...
          if (token::number != tk && token::variable != tk &&
              token::repeated != tk)

            throw error(
                "invalid array element. Expected a number, variable or repaet",
                *this);
    ...
    if ('_' == *sp_ || std::isalpha(*sp_)) {
      ...
      if ("weight" == idstr_ || "min" == idstr_ || "max" == idstr_ ||
          "width" == idstr_ || "height" == idstr_) {
        auto c3 = idstr_[2], c1 = idstr_[0];
        _m_attr_number_value();
        switch (c3) {
        case 'i':
          return c1 == 'w' ? token::weight : token::height;
        case 'n':
          return token::min_px;
        case 'x':
          return token::max_px;
      ...
      } else if ("arrange" == idstr_ || "hfit" == idstr_ || "vfit" == idstr_ ||
                 "gap" == idstr_) {
        auto ch = idstr_[0];
        _m_attr_reparray();
        switch (ch) {
        case 'a':
          return token::arrange;
        case 'h':
          return token::hfit;
        case 'v':
          return token::vfit;
...
private:
  void _m_attr_number_value() {
    ...
    if (0 == len)
      throw error("the '" + idstr_ +
                      "' requires a number (integer, real or percent)",
                  *this);
  ...
  void _m_attr_reparray() {
    ...
    case token::reparray:
      break;
    default:
      throw error("a (repeated) array is required after '" + idstr + "'",
                  *this);
    }
  }
...
static std::size_t find_idstr(const std::string &text, const char *idstr,
                              std::size_t off = 0) {
  if (!idstr)
    return text.npos; /// ??
...
// Find the text boundary of a field. The parameter start_pos is one of bound
// characters of the field whose bound will be returned. The boundary doesn't
// include the tag characters.
...
struct place::implement {
  /// usefull ??
  struct error : std::invalid_argument {
    error(std::string what, std::string field = "unknown",
          std::string::size_type pos = std::string::npos)

        : std::invalid_argument{what + " from place implementation "}, pos{pos},
  ...
  std::function<void(window, paint::graphics &, nana::mouse_action)>
      split_renderer;
  ...
  // The following functions are defined behind the definition of class
  // division. because the class division here is an incomplete type.
  ~implement();

  void collocate();

  static division *search_div_name(division *start,
...
class place::implement::field_gather : public place::field_interface {
public:
  ...
  field_gather(place *p) noexcept : place_ptr_(p) {}
  ...
  static event_handle erase_element(std::vector<element_t> &elements,
...
private:
  void _m_insert_widget(
      window wd,
      bool to_fasten) /// \todo better errors caption of failed windows, field
  {
    if (API::empty_window(wd))
      throw place::error("Failed to insert an invalid window handle.",
                         *place_ptr_);

    if (API::get_parent_window(wd) != place_ptr_->window_handle())
      throw place::error("Failed to insert a window which is not a child of "
                         "the place-binded window",
                         *place_ptr_);

    // Listen to destroy of a window
    // It will delete the element and recollocate when the window destroyed.
    auto evt = API::events(wd).destroy.connect([this, to_fasten](
                                                   const arg_destroy &arg) {
      if (!to_fasten) {
  ...
  field_interface &operator<<(const char *label_text) override {
    return static_cast<field_interface *>(this)->operator<<(
        agent<label>(label_text ? label_text : ""));
  }

  field_interface &operator<<(std::string label_text) override {
    return static_cast<field_interface *>(this)->operator<<(
        agent<label>(label_text));
  }
...
public:
  ...
  std::map<std::string, std::function<std::unique_ptr<widget>(window)>>
      factories; // factories for dockpane
...
public:
  ...
  division(kind k, std::string &&n) noexcept
      : kind_of_division(k), name(std::move(n)) {}
  ...
  static unsigned calc_number(const place_parts::number_t &number,
                              unsigned area_px, double adjustable_px,
                              double &precise_px) {
  ...
  std::pair<double, double> calc_weight_floor() {
    ...
    if (this->display) {
      ...
      auto const vert_fields =
          (kind::vertical_arrange == this->kind_of_division);
      auto const vert_div =
          (this->div_owner &&
           (kind::vertical_arrange == this->div_owner->kind_of_division));
      ...
      if ((fit_policy::none != this->fit) && this->field) {
        ...
        for (auto &elm : this->field->elements) {
          ...
          if (extent) {
            ...
            max_value =
                (std::max)(max_value, (vert_fields ? extent->second.width
  ...
  void set_display(bool dsp) {
    ...
    if (display) {
      ...
      if (this->div_owner &&
          (kind::switchable == this->div_owner->kind_of_division)) {
  ...
  static double limit_px(const division *div, double px,
                         unsigned area_px) noexcept {
    auto const vert = (div->div_owner && (div->div_owner->kind_of_division ==
                                          kind::vertical_arrange));

    auto weight_floor =
        (vert ? div->run_.weight_floor.second : div->run_.weight_floor.first);
  ...
  nana::rectangle margin_area() const { return margin.area(field_area); }
...
public:
  ...
  // The window parameter is specified for the window which the place object
  // binded.
...
public:
  ...
  repeated_array fit_parameters; // it is ignored when fit is not
                                 // fit_policy::horz or fit_policy::vert
...
class place::implement::div_arrange : public division {
public:
  div_arrange(bool vert, std::string &&name,
              place_parts::repeated_array &&arr) noexcept
      : division((vert ? kind::vertical_arrange : kind::arrange),
                 std::move(name)),
  ...
  void collocate(window wd) override {
    ...
    for (auto &child_ptr : children) /// First collocate child div's !!!
    {
      ...
      if (!child->display) // Ignore the division if the corresponding field is
                           // not displayed.
        continue;
      ...
      {
        if (child->is_percent()) // and calculated for others: if the child div
                                 // is percent - simple take it full
      ...
      if ((!child->is_fixed()) && child->max_px.empty() && is_back(child) &&
          (endpos != area.right()))
        endpos = area.right();

      child_area.w_ref() =
          static_cast<unsigned>((std::max)(endpos - child_area.x(), 0));
      ...
      if (child->kind_of_division == kind::splitter)
        ...
        child->collocate(
            wd); /// The child div have full position. Now we can collocate
                 /// inside it the child fields and child-div.
    }
      ...
      for (auto &el : field->elements) {
        ...
        if (fit_policy::none != this->fit) {
          ...
          if (run_.fit_extents.end() != i) {
            ...
            if (vert)
              move_r.x +=
                  place_parts::differ(area_margined.width, move_r.width) / 2;
            else
              move_r.y +=
                  place_parts::differ(area_margined.height, move_r.height) / 2;
        ...
        if (!moved) {
          px = calc_number(arrange_.at(index), area_px, adjustable_px,
                           precise_px);
          element_r.w_ref() = px;
...
private:
  static std::pair<unsigned, std::size_t>
  _m_calc_fa(const place_parts::number_t &number, unsigned area_px,
             double &precise_px) {
  ...
  _m_fixed_and_adjustable(kind match_kind, unsigned area_px) const noexcept {
    std::pair<unsigned, std::size_t> result;
    if (field && (kind_of_division == match_kind)) {
      ...
      for (decltype(count) i = 0; i < count; ++i) {
        ...
        if (i + 1 < count) {
          ...
          result.first += fa.first;
          // fa.second is ignored for gap, because the it has not the adjustable
          // gap.
        }
      }
    }
  ...
  static double
  _m_find_lowest_revised_division(const std::vector<revised_division> &revises,
                                  double level_px) noexcept {
  ...
  static std::size_t _m_remove_revised(std::vector<revised_division> &revises,
                                       double value,
                                       std::size_t &full_count) noexcept {
    full_count = 0;
  ...
  double _m_revise_adjustable(std::pair<unsigned, std::size_t> &fa,
                              unsigned area_px) {
    ...
    for (auto &child : children) {
      ...
      double min_px = std::numeric_limits<double>::lowest(),
             max_px = std::numeric_limits<double>::lowest();

      if (!child->min_px.empty())
        min_px = child->min_px.get_value(static_cast<int>(area_px));

      auto weight_floor = (this->kind_of_division == kind::arrange
                               ? child->run_.weight_floor.first
                               : child->run_.weight_floor.second);
    ...
    while ((rest_px > 0) && blocks) {
      ...
      double fill_px = 0;
      // blocks may be equal to min_count. E.g, all child divisions have min/max
      // attribute.
      if (blocks > min_count) {
...
public:
  div_grid(std::string &&name, place_parts::repeated_array &&arrange,
           std::vector<rectangle> &&collapses) noexcept
      : division(kind::grid, std::move(name)), arrange_(std::move(arrange)),
  ...
  void revise_collapses() noexcept {
    ...
    for (auto i = collapses_.begin(); i != collapses_.end();) {
      if (i->x >= static_cast<int>(dimension.first) ||
          i->y >= static_cast<int>(dimension.second))
    ...
    for (std::size_t i = 0; i < collapses_.size() - 1; ++i) {
      ...
      for (auto u = i + 1; u != collapses_.size();) {
        auto &z = collapses_[u];
        if (col.is_hit(z.x, z.y) || col.is_hit(z.right(), z.y) ||
            col.is_hit(z.x, z.bottom()) || col.is_hit(z.right(), z.bottom()))
  ...
  void collocate(window) override {
    ...
    auto const gap_size = static_cast<unsigned>(gap.at(0).get_value(
        area.width)); // gap_size is 0 if gap isn't specified

    auto i = field->elements.cbegin();
    auto const end = field->elements.cend();

    // The total gaps must not beyond the bound of the area.
    if ((gap_size * dimension.first < area.width) &&
        (gap_size * dimension.second < area.height)) {
      if (dimension.first <= 1 && dimension.second <= 1) {
          ...
          for (std::size_t v = 0; (v < edge && i != end); ++v, ++i) {
            ...
            if (arr.empty())
              ...
              value = static_cast<decltype(value)>(
                  arr.get_value(static_cast<int>(area.width)));

            unsigned width = (value > uns_block_w ? uns_block_w : value);
            if (width > gap_size)
              width -= gap_size;
            API::move_window(i->handle,
                             rectangle{static_cast<int>(x), static_cast<int>(y),
                                       width, height});
            x += block_w;
          }
          y += block_h;
        ...
        const double block_w =
            int(area.width - gap_size * (dimension.first - 1)) /
            double(dimension.first);
        const double block_h =
            int(area.height - gap_size * (dimension.second - 1)) /
            double(dimension.second);

        std::unique_ptr<char[]> table_ptr{
            new char[dimension.first * dimension.second]};
        ...
        for (std::size_t u = 0; (u < dimension.second && i != end); ++u) {
          auto const block_height_px =
              static_cast<unsigned>(block_h + precise_h);
          ...
          for (std::size_t v = 0; (v < dimension.first && i != end); ++v) {
            ...
            const int pos_x =
                area.x + static_cast<int>(v * (block_w + gap_size));
            const int pos_y =
                area.y + static_cast<int>(u * (block_h + gap_size));
            ...
            if (room.first <= 1 && room.second <= 1) {
              ...
              result_h =
                  static_cast<unsigned>(block_h * room.second + precise_h +
                                        (room.second - 1) * gap_size);
            ...
            API::move_window(i->handle,
                             rectangle{pos_x, pos_y, result_w, result_h});
            ++i;
          }
...
private:
  void
  _m_find_collapse(int x, int y,
                   std::pair<unsigned, unsigned> &collapse) const noexcept {
    for (auto &col : collapses_) {
      if (col.x == x && col.y == y) {
...
class place::implement::div_splitter : public division {
  struct div_block {
    division *div;
    int pixels;
    double scale;

    div_block(division *d, int px) noexcept : div(d), pixels(px) {}
  };

  enum { splitter_px = 4 };

public:
  div_splitter(const place_parts::number_t &init_weight,
               implement *impl) noexcept
  ...
  ~div_splitter() { impl_->splitters.erase(this); }

  void set_renderer(
      const std::function<void(window, paint::graphics &, mouse_action)> &fn,
      bool update) {
...
private:
  void collocate(window wd) override {
    ...
    if (splitter_.empty()) {
      ...
      auto grab_fn = [this](const arg_mouse &arg) {
        ...
        if (event_code::mouse_down == arg.evt_code) {
          ...
          // Use field_area of leaf, not margin_area. Otherwise splitter would
          // be at wrong position
          ...
          // when the splitter is dragged, the place recalculates the left/right
          // fields the weight in percentage, then update position of the
          // splitter field. It may cause deviation that new splitter field
          // position is not same with the position of splitter window after
          // dragging a bit, because the field position is calcuated with
          // left/right fields's weights which are float-point values.
          ...
          const auto total_pixels =
              static_cast<int>(left_pixels_ + right_pixels_);

          auto left_px = std::clamp(static_cast<int>(left_pixels_) + delta, 0,
                                    total_pixels);
          ...
          auto right_px = std::clamp(static_cast<int>(right_pixels_) - delta, 0,
                                     total_pixels);
          ...
          // After the collocating, the splitter keeps the calculated weight of
          // left division, and clear the weight of right division.
    ...
    if (!init_weight_.empty()) {
      ...
      auto right_px =
          static_cast<int>(limit_px(leaf_right, init_weight_.get_value(area_px),
                                    static_cast<unsigned>(area_px)));

      // New position of splitter
      const auto pos =
          std::clamp(static_cast<int>(area_px - right_px - splitter_px),
                     limited_range.x(), limited_range.right());
...
private:
  ...
  void _m_update_div(std::string &div) {
    ...
    std::string::size_type tag_pos{left ? div.find('<', bound.second + 2)
                                        : div.rfind('>', bound.first - 2)};
    if (div.npos == tag_pos)
      throw place::implement::error("please report an issue: the splitter was "
    ...
    auto other_fieldstr =
        div.substr(other_bound.first, other_bound.second - other_bound.first);
    ...
    double percent =
        double((left ? r_left : r_right).w()) / double(r_owner.w());

    fieldstr += " weight=" + std::to_string(percent * 100) + "%";

    // Replaces the 'right' field before 'left' in order to make the bound
    // consistent
    if (left) {
      if (other_fieldstr.length() != (other_bound.second - other_bound.first))
        div.replace(other_bound.first, other_bound.second - other_bound.first,
                    other_fieldstr);
  ...
  rectangle_rotator _m_update_splitter_range() {
    ...
    if (!leaf_left->max_px.empty())
      endpos =
          left_base + static_cast<int>(leaf_left->max_px.get_value(area.w()));

    if (!leaf_right->min_px.empty())
      endpos = (std::min)(
          right_base - static_cast<int>(leaf_right->min_px.get_value(area.w())),
          endpos);

    if (!leaf_right->max_px.empty())
      pos = (std::max)(
          right_base - static_cast<int>(leaf_right->max_px.get_value(area.w())),
          pos);
    ...
    dragger_.target(
        splitter_, area.result(),
        (vert ? nana::arrange::vertical : nana::arrange::horizontal));
...
private:
  ...
  dragger dragger_;
  bool grabbed_{false};
  bool pause_move_collocate_{
      false}; // A flag represents whether do move when collocating.
  place_parts::number_t init_weight_;
};

class place::implement::div_dockpane
    : public division,
      public place_parts::dock_notifier_interface {
public:
  div_dockpane(std::string &&name, implement *impl, direction pane_dir) noexcept
      : division(kind::dockpane, std::move(name)), impl_ptr_{impl} {
    dir = pane_dir;
    this->display = false;
  }
...
private:
  ...
  void notify_move() override {
    ...
    if (!indicator_.docker) {
      ...
      indicator_.docker.reset(
          new form(impl_ptr_->window_handle,
                   {static_cast<int>(host_size.width) / 2 - 16,
                    static_cast<int>(host_size.height) / 2 - 16, 32, 32},
                   form::appear::bald<>()));
      ...
      dw.draw([](paint::graphics &graph) {
        ...
        arrow.draw(graph, colors::light_sky_blue, colors::midnight_blue,
                   {12, 0, 16, 16}, element_state::normal);
    ...
    if (_m_hit_test(true)) // hit test on docker
    {
      if (!indicator_.dock_area) {
        ...
        indicator_.dock_area.reset(
            new panel<true>(impl_ptr_->window_handle, false));
  ...
  void request_close() override {
    ...
    API::at_safe_place(window_handle, [ptr] {
      std::unique_ptr<typename std::remove_pointer<decltype(ptr)>::type> del(
          ptr);
    });
...
private:
  bool _m_hit_test(bool try_docker) const {
    ...
    return rectangle{pos, API::window_size(handle)}.is_hit(
        API::cursor_position());
  }
...
class place::implement::div_dock : public division {
  static const unsigned splitter_px = 5;

  class splitter : public panel<true> {
  public:
    splitter(window wd, ::nana::direction dir, division *dock_dv,
             division *pane_dv)
        : panel<true>(wd, true), dir_(dir), dock_dv_(dock_dv),
          pane_dv_(pane_dv) {
      this->bgcolor(colors::alice_blue);
      this->cursor(is_vert_dir(dir_) ? ::nana::cursor::size_ns
                                     : ::nana::cursor::size_we);
      ...
      auto grab_fn = [this, wd](const arg_mouse &arg) {
        ...
        if (event_code::mouse_down == arg.evt_code) // press mouse button
        {
          ...
          base_px_ = (is_vert ? pane_dv_->field_area.height
                              : pane_dv_->field_area.width);
        } else if (event_code::mouse_move == arg.evt_code) // hover
        {
          ...
          auto delta =
              horz_point(is_vert, API::cursor_position()) - base_pos_.x;
          ...
          auto dock_px = (is_vert ? dock_dv_->field_area.height
                                  : dock_dv_->field_area.width);
  ...
  private:
    ...
    unsigned
        base_px_; // weight of div_dockpane when mouse button is been pressing;
  };
...
public:
  ...
  void collocate(window wd) override {
    ...
    ::nana::size room(area.width - splitter_px * (horz_count - 1),
                      area.height - splitter_px * (vert_count - 1));
    ...
    for (auto &child : children) {
      ...
      if (_m_right(child_dv)) {
        ...
        if (!child_dv->splitter) {
          split =
              new splitter(impl_->window_handle, child->dir, this, child_dv);
      ...
      case ::nana::direction::west:
        ...
        left += weight;
        if (split) {
          split->move(rectangle{child_r.right(), child_r.y, splitter_px,
                                child_r.height});
      ...
      case ::nana::direction::east:
        ...
        child_r.height = static_cast<unsigned>(bottom - top);
        if (split) {
          split->move(rectangle{child_r.x - static_cast<int>(splitter_px),
                                child_r.y, splitter_px, child_r.height});
      ...
      case ::nana::direction::north:
        ...
        top += weight;
        if (split) {
          split->move(rectangle{child_r.x, child_r.bottom(), child_r.width,
                                splitter_px});
      ...
      case ::nana::direction::south:
        ...
        if (split) {
          bottom -= splitter_px;
          split->move(rectangle{child_r.x,
                                child_r.y - static_cast<int>(splitter_px),
                                child_r.width, splitter_px});
      ...
      if (split_range_begin > -0.5)
        split->range(static_cast<int>(split_range_begin),
                     static_cast<int>(split_range_end));
...
class place::implement::div_switchable : public division {
public:
...
void place::implement::collocate() {
  if (root_division && window_handle) {
    ...
    for (auto &field : fields) {
      bool is_show = false;
      if (field.second->attached && field.second->attached->visible &&
          field.second->attached->display) {
        is_show = true;
      ...
      // This is a feature that allows tabbar panels to be fastened to a same
      // field, the collocate() shouldn't break the visibility of panels that
      // are maintained by tabbar.
...
place::implement::division *
place::implement::search_div_name(division *start,
...
static int get_parameter(place_parts::tokenizer &tknizer, std::size_t pos) {
  ...
  throw place_parts::tokenizer::error(
      "the type of the " + std::string{pos_strs[pos]} +
          " parameter for collapse should be integer.",
      tknizer);
}

// implicitly_started indicates whether the field in div-text starts without <
// mark. ignore_duplicate A field is allowed to have same name if its has an
// ancestor which name is same with ignore_duplicate.
auto place::implement::scan_div(place_parts::tokenizer &tknizer,
                                bool implicitly_started,
                                const std::string &ignore_duplicate)
  ...
  token tk = token::eof;
  try {
    for (tk = tknizer.read(); (tk != token::eof && tk != token::div_end);
         tk = tknizer.read()) {
      switch (tk) {
      case token::dock:
        if (token::eof != div_type && token::dock != div_type)
          throw std::invalid_argument(
              "conflict of div type -expected dock type-");
      ...
      case token::splitter:
        ...
        if (!children.empty() &&
            (division::kind::splitter != children.back()->kind_of_division)) {
      ...
      case token::div_start: {
        ...
        if (!children.empty()) {
          ...
          if ((children.back()->kind_of_division == division::kind::splitter) &&
              !div->display)
      ...
      case token::collapse: {
        ...
        ::nana::rectangle col{
            get_parameter(tknizer, 0), get_parameter(tknizer, 1),
        ...
        // Ignore this collapse if its area is less than 2(col.width *
        // col.height < 2)
        if (!col.empty() && (col.width > 1 || col.height > 1) &&
            (col.x >= 0 && col.y >= 0)) {
      ...
      case token::height: {
        ...
        case token::weight:
          weight = n;
          weight_type = token::weight;
          break; // we could detect errors here (redefinitions and duplicates)
        case token::width:
          weight = n;
          weight_type = token::width;
          break;
        case token::height:
          weight = n;
          weight_type = token::height;
          break;
        case token::min_px:
          min_px = n;
          break;
      ...
      case token::identifier:
        ...
        break;
      case token::left:
        div_dir = ::nana::direction::west;
        break;
      case token::right:
        div_dir = ::nana::direction::east;
        break;
      case token::top:
        div_dir = ::nana::direction::north;
        break;
      case token::bottom:
        div_dir = ::nana::direction::south;
        break;
    ...
    if (fields.end() != i) {
      ...
      // the field is attached to a division, it means there is another division
      // with same name.
      if (attached_field->attached) {
        // The fields are allowed to have a same name. E.g.
        // place.div("A <B><C>");
        // place.modify("A", "<B>");  Here the same name B must be allowed,
        // otherwise it throws std::invalid_argument.
    ...
    case token::vert: // "vertical" div
      ...
      for (auto &ch : children)
        if (ch->weigth_type == unmatch)
          throw std::invalid_argument(
              "unmatch vertical-height/horizontal-width between division '" +
              name + "' and children division '" + ch->name);

      div.reset(new div_arrange(token::vert == div_type, std::move(name),
                                std::move(arrange)));
      break;
    case token::grid: {
      std::unique_ptr<div_grid> p(new div_grid(
          std::move(name), std::move(arrange), std::move(collapses)));
    ...
    // 2, max >= min
    if (min_px.is_negative())
      min_px.reset();
    if (max_px.is_negative())
      max_px.reset();
    if ((!min_px.empty()) && (!max_px.empty()) &&
        (min_px.get_value(100) > max_px.get_value(100))) {
      min_px.reset();
      max_px.reset();
    }
    ...
    if ((!min_px.empty()) && (!weight.empty()) &&
        (weight.get_value(100) < min_px.get_value(100)))
      weight.reset();

    if ((!max_px.empty()) && (!weight.empty()) &&
        (weight.get_value(100) > max_px.get_value(100)))
      weight.reset();
    ...
    if (attached_field) {
      // Replaces the previous div with the new div which is allowed to have a
      // same name.
    ...
    if (children.size()) {
      ...
      for (auto &child : children) {
        ...
        if (division::kind::splitter == child->kind_of_division)
          dynamic_cast<div_splitter &>(*child).direction(div_type !=
                                                         token::vert);
      }
      ...
      if (token::dock == div_type) {
        ...
        for (auto &child : children) {
          auto dockpn =
              new div_dockpane(std::move(child->name), this, child->dir);
        ...
        for (int i = static_cast<int>(adjusted_children.size()) - 1; i >= 0;
             --i) {
    ...
    throw error(std::string{"unexpected error: "} + e.what(), name,
                tknizer.pos());
  } catch (...) {
...
void place::implement::check_unique(const division *div) const {
  ...
  check_fn = [&check_fn, &unique, &tmp](const implement::division *div) {
    if (!div->name.empty()) {
      ...
      if (ptr)
        throw std::invalid_argument("place, the name '" + div->name +
                                    "' is redefined.");
      ptr = &tmp;
    }
  ...
  connect_fn = [&connect_fn, this, &docks_to_be_closed](division *div) {
    if (div->name.size()) {
      if (division::kind::dock == div->kind_of_division ||
          division::kind::dockpane == div->kind_of_division) {
...
void place::bind(window wd) {
  if (impl_->window_handle)
    throw error(" bind('" + API::window_caption(wd).substr(0, 80) +
                    "'): it was already bound to another window.",
                *this);

  impl_->window_handle = wd;
  impl_->event_size_handle = API::events(wd).resized.connect_unignorable(
      [this](const arg_resized &arg) {
...
window place::window_handle() const { return impl_->window_handle; }

void place::splitter_renderer(
...
void place::div(std::string div_text) {
  try {
    ...
    throw error("failed to set div('" + div_text +
...
const std::string &place::div() const noexcept { return impl_->div_text; }

// Contributed by dankan1890(PR#156)
enum class update_operation { erase = 0, insert, replace };

static void update_div(std::string &div, const char *field, const char *attr,
...
void place::modify(const char *name, const char *div_text) {
  ...
  try {
    ...
    impl_->connect(
        impl_->root_division.get()); // throws if there is a duplicate name
    ...
    replaced->swap(impl_->tmp_replaced);
    throw error("failed to modify('" + std::string(name) + ", " + div_text +
                    "'): " + e.what(),
                *this, e.field, e.pos);
  } catch (place_parts::tokenizer::error &e) {
    replaced->swap(impl_->tmp_replaced);
  ...
  if ((!p->attached) && impl_->root_division) {
    ...
    if (div) {
      if (div->field && (div->field != p))
        throw error("field(): the division attaches an unexpected field", *this,
                    name);
...
static void update_div(std::string &div, const char *field, const char *attr,
  ...
  if (fieldstr.npos == pos) {
    ...
    if (operation == update_operation::insert)
      ...
      div.replace(bound.first, bound.second - bound.first,
                  std::string(attr) + " " + std::string(field));
  } else {
    ...
    if (operation == update_operation::erase) {
      ...
      if (bound.first + pos > 0) {
        ...
        if ((div[bound.first + pos] == div[bound.first + pos - 1]) &&
            (' ' == div[bound.first + pos]))
...
void place::field_visible(const char *name, bool vsb) {
  ...
  if (div) {
    div->set_visible(vsb);
    update_div(impl_->div_text, name, "invisible",
               !vsb ? update_operation::insert : update_operation::erase);
  }
}
...
void place::field_display(const char *name, bool dsp) {
  ...
  if (div) {
    ...
    update_div(impl_->div_text, name, "undisplayed",
               !dsp ? update_operation::insert : update_operation::erase);
    div->set_display(dsp);
  }
}
...
place &place::dock(const std::string &name, std::string factory_name,
  ...
  if (!factory_name.empty()) {
    if (impl_->dock_factoris.find(factory_name) != impl_->dock_factoris.end())
      throw error("dock() - the specified factory name(" + factory_name +
                      ") already exists",
                  *this, name);
  ...
  auto div = dynamic_cast<implement::div_dockpane *>(
      impl_->search_div_name(impl_->root_division.get(), name));
  if (div) {
...
widget *place::dock_create(const std::string &factory) {
  ...
  if (i == impl_->dock_factoris.end())
    throw std::invalid_argument(
        "nana::place::dock_create - invalid factory name(" + factory +
        ") of dockpane");
...
place::error::error(const std::string &what, const place &plc,
                    std::string field, std::string::size_type pos)

    : std::invalid_argument(
          "from widget '" +
          API::window_caption(plc.window_handle()).substr(0, 80) +
          "'; nana::place error " + what + "' in field '" + field +
          (pos == std::string::npos ? "' "
                                    : "' at position " + std::to_string(pos)) +
          " in div_text:\n" + plc.div()),
      base_what(what),
      owner_caption(API::window_caption(plc.window_handle()).substr(0, 80)),
      div_text(plc.div()), field(field), pos(pos) {}
// end class place
