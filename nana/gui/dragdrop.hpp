...
public:
  ...
  simple_dragdrop(window drag_origin, std::function<bool()> when,
                  window drop_target, std::function<void()> how)
  ...
  /// Condition checker
  /**
   * Sets a condition checker that determines whether the drag-and-drop
   * operation can start. If a condition checker is not set, it always start
   * drag-and-drop operation.
   * @param predicate_fn Unary predicate which returns #true# for starting
   * drag-and-drop operation.
   */
...
public:
  ...
  /// Transferred data
  /**
   * Set a data generator. When drag begins, it is called to generate a data
   * object for transferring.
  ...
  /// Drop handler
  /**
   * The drop handler is called when the drop operation is completed. There are
   *3 parameters for the handler dropped Indicates whether the data is accepted
   *by a target window. executed_action Indicates the action returned by target
   *window. Ignore if dropped is false. data_transferred The data object which
   *is generated by the generator.
   * @param finish_fn The drop handling function.
   */
  void
  drop_finished(std::function<void(bool dropped, dnd_action executed_action,
