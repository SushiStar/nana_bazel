...
struct root_misc {
  ...
  struct condition_rep {
    ...
    basic_window *pressed; // The handle to a window which has been pressed by
                           // mouse left button.
    basic_window *pressed_by_space; // The handle to a window which has been
                                    // pressed by SPACEBAR key.
