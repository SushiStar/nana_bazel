 ...
 *
 *	<Knowledge: 1, 2007-8-17, "Difference between destroy and
 *destroy_handle"> destroy method destroys a window handle and the handles of
 *its children, but it doesn't delete the handle which type is a root window
 *		destroy_handle method just destroys the handle which type is a
 *root window
 *
 */
...
public:
  ...
  basic_window *create_root(basic_window *, bool nested, rectangle,
                            const appearance &, widget *);
  basic_window *create_widget(basic_window *, const rectangle &, bool is_lite,
                              widget *);
  ...
  // destroy_handle
  //@brief:	Delete window handle, the handle type must be a root and a
  //frame.

  // Deletes a window whose category type is a root type or a frame type.
  void destroy_handle(basic_window *);

  void icon(basic_window *, const paint::image &small_icon,
  ...
  //@param pos Position
  //@param ignore_captured A flag indicates whether to ignore redirecting the
  //result to its captured window. If this paramter is true, it returns the
  //window at the position, if the parameter is false, it returns the captured
  //window if the captured window don't ignore children.
  basic_window *find_window(native_window_type root, const point &pos,
                            bool ignore_captured = false);

  // move the wnd and its all children window, x and y is a relatively
  // coordinate for wnd's parent window
  ...
  void capture_window(basic_window *, bool capture,
                      bool ignore_children_if_captured);

  void enable_tabstop(basic_window *);
  basic_window *
  tabstop(basic_window *,
          bool forward) const; // forward means move to next in logic.
...
private:
  ...
  void _m_shortkeys(
      basic_window *, bool with_chlidren,
      std::vector<std::pair<basic_window *, unsigned long>> &keys) const;
