...
enum class visible_state { invisible, visible, displayed };

class caret : public caret_interface {
public:
...
/// Define some constant about tab category, these flags can be combine with
/// operator |
struct tab_type {
  enum t {
...
struct basic_window : public events_holder {
  using container = std::vector<basic_window *>;

  enum class update_state { none, lazy, refreshed, request_refresh };
  ...
  void bind_native_window(native_window_type, unsigned width, unsigned height,
                          unsigned extra_width, unsigned extra_height,
                          paint::graphics &);
  ...
  const basic_window *
  child_caret() const; ///< Returns the child which owns the caret

  bool
  is_draw_through() const; ///< Determines whether it is a draw-through window.
...
public:
  ...
  basic_window
      *root_widget; ///< A pointer refers to the root basic window, if the
                    ///< window is a root, the pointer refers to itself.
  ...
  struct flags_type {
    bool enabled : 1;
    bool dbl_click : 1;
    bool captured : 1; ///< if mouse button is down, it always receive mouse
                       ///< move even the mouse is out of its rectangle
    bool modal : 1;
    bool take_active : 1; ///< If take_active is false, other.active_window
                          ///< still keeps the focus.
    bool refreshing : 1;
    bool destroying : 1;
    bool dropable : 1;   ///< Whether the window has make mouse_drop event.
    bool fullscreen : 1; ///< When the window is maximizing whether it fit for
                         ///< fullscreen.
    bool borderless : 1;
    bool make_bground_declared : 1; ///< explicitly make bground for bground
                                    ///< effects
    bool ignore_menubar_focus : 1;  ///< A flag indicates whether the menubar
                                    ///< sets the focus.
    bool ignore_mouse_focus : 1;    ///< A flag indicates whether the widget
                                    ///< accepts focus when clicking on it
    bool space_click_enabled : 1;   ///< A flag indicates whether enable
                                    ///< mouse_down/click/mouse_up when pressing
                                    ///< and releasing whitespace key.
    bool draggable : 1;
  ...
  struct other_tag {
    struct attr_root_tag {
      ...
      bool lazy_update{
          false}; ///< Indicates whether the window is in lazy-updating mode.

      container update_requesters; ///< Container for lazy-updating requesting
                                   ///< windows.
      container tabstop;
      ...
      std::function<void()>
          draw_through; ///< A draw through renderer for root widgets.
    };

    const category::flags category;
    basic_window
        *active_window; ///< if flags.take_active is false, the active_window
                        ///< still keeps the focus, if the active_window is
                        ///< null, the parent of this window keeps focus.
    paint::graphics glass_buffer; ///< if effect.bground is avaiable. Refer to
                                  ///< window_layout::make_bground.
