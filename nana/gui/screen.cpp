...
namespace nana {
// class display
class real_display : public display {
public:
  real_display() = default; // For requirement of vector

#if defined(NANA_WINDOWS)
  real_display(std::size_t number, const MONITORINFOEX &mi)
      : index_(number), is_primary_(mi.dwFlags & /*MONITORINFOF_PRIMARY*/ 0x1),
        area_(mi.rcMonitor.left, mi.rcMonitor.top,
              mi.rcMonitor.right - mi.rcMonitor.left,
              mi.rcMonitor.bottom - mi.rcMonitor.top),
        workarea_(mi.rcWork.left, mi.rcWork.top,
...
public:
  ...
  std::size_t get_index() const override { return index_; }

  bool is_primary_monitor() const override { return is_primary_; }

  const ::nana::rectangle &area() const override { return area_; }

  const ::nana::rectangle &workarea() const override { return workarea_; }
...
#if defined(NANA_WINDOWS)
  auto w =
      static_cast<size::value_type>(::GetSystemMetrics(SM_CXVIRTUALSCREEN));
  auto h =
      static_cast<size::value_type>(::GetSystemMetrics(SM_CYVIRTUALSCREEN));
  return {w, h};
#else
...
struct screen::implement {
  ...
  void load_monitors() {
    ...
    ::EnumDisplayMonitors(nullptr, nullptr, implement::enum_proc,
                          reinterpret_cast<LPARAM>(&tmp));
    tmp.swap(displays);
  }

  static BOOL __stdcall enum_proc(HMONITOR handle, HDC /*context*/,
                                  LPRECT /*r*/, LPARAM self_ptr) {
...
screen::screen() : impl_(std::make_shared<implement>()) {
  impl_->load_monitors();
}
...
std::size_t screen::count() const { return impl_->displays.size(); }

display &screen::from_point(const point &pos) {
#if defined(NANA_WINDOWS)
  typedef HMONITOR(__stdcall * MonitorFromPointT)(POINT, DWORD);

  MonitorFromPointT mfp = reinterpret_cast<MonitorFromPointT>(
      ::GetProcAddress(::GetModuleHandleA("User32.DLL"), "MonitorFromPoint"));
  if (mfp) {
