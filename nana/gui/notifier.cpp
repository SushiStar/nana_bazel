...
#include "nana/paint/image_accessor.hpp"
...
public:
  ...
  unsigned short register_wd(::nana::notifier *ntf_ptr,
                             native_window_type native_handle,
                             detail::notifier_events *evt_ptr) {
  ...
  void emit(native_window_type wd, unsigned short id,
            const arg_notifier &arg_basic) {
    lock_guard lock(mutex_);
    auto i_wd = wd_table_.find(wd);
    if (i_wd == wd_table_.end())
      return;
...
void notifications_window_proc(HWND wd, WPARAM wparam, LPARAM lparam) {
  ...
  case WM_MOUSEMOVE:
  case WM_MOUSELEAVE:
    arg.evt_code = (WM_MOUSEMOVE == lparam ? event_code::mouse_move
                                           : event_code::mouse_leave);
  ...
  notifications::instance().emit(reinterpret_cast<native_window_type>(wd),
                                 static_cast<unsigned short>(wparam), arg);
}
#endif

// class notifier
notifier::notifier(window wd) : impl_(new implement) {
  impl_->handle = wd;
  ...
  impl_->evt_destroy =
      API::events(wd).destroy.connect([this](const arg_destroy &) { close(); });
  impl_->id = notifications::instance().register_wd(this, impl_->native_handle,
                                                    &impl_->events);
}
...
void notifier::period(std::chrono::milliseconds ms) {
  ...
  if (ms.count() && impl_->icons.size()) {
    auto frame_ms =
        (std::max)(ms.count() / static_cast<long long>(impl_->icons.size()),
...
detail::notifier_events &notifier::events() { return impl_->events; }

window notifier::handle() const { return impl_->handle; }
// end notifier
} // namespace nana
