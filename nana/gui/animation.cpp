...
struct framebuilder {
  ...
  framebuilder(
      std::function<bool(std::size_t, paint::graphics &, nana::size &)> f,
      std::size_t l)
      : length(l), frbuilder(std::move(f)) {}
};

struct frame {
  enum class kind { oneshot, framebuilder };

  frame(paint::image img) : type(kind::oneshot) {
    u.oneshot = new paint::image(std::move(img));
  }

  frame(std::function<bool(std::size_t, paint::graphics &, nana::size &)>
            frbuilder,
        std::size_t length)
  ...
  frame(const frame &r) : type(r.type) {
    switch (type) {
    case kind::oneshot:
...
struct frameset::impl {
  ...
  mutable bool good_frame_by_frmbuilder{
      false}; // It indicates the state of frame.

  impl() : this_frame(frames.end()) {}

  // Render A frame on the set of windows.
  void render_this(std::map<window, output_t> &outs,
                   paint::graphics &framegraph,
                   nana::size &framegraph_dimension) const {
    ...
    case frame::kind::oneshot:
      _m_render(outs, frmobj.u.oneshot->size(),
                [&frmobj](paint::graphics &tar, const nana::rectangle &area) {
                  if (frmobj.u.oneshot->size() == area.dimension())
                    frmobj.u.oneshot->paste(tar, area.position());
                  else
                    frmobj.u.oneshot->stretch(
                        rectangle{frmobj.u.oneshot->size()}, tar, area);
                });
      break;
    case frame::kind::framebuilder:
      good_frame_by_frmbuilder = frmobj.u.frbuilder->frbuilder(
          pos_in_this_frame, framegraph, framegraph_dimension);
      if (good_frame_by_frmbuilder) {
        _m_render(outs, framegraph_dimension,
                  [framegraph_dimension, &framegraph](
                      paint::graphics &tar, const rectangle &area) mutable {
  ...
  // Render a frame on a specified window graph. If this frame is created by
  // framebuilder, it doesn't rebuild the frame.
  void render_this(paint::graphics &graph, const rectangle &area,
                   paint::graphics &framegraph,
                   nana::size &framegraph_dimension) const {
    ...
    case frame::kind::oneshot:
      if (frmobj.u.oneshot->size() == area.dimension())
        ...
        frmobj.u.oneshot->stretch(rectangle{frmobj.u.oneshot->size()}, graph,
                                  area);
      break;
    case frame::kind::framebuilder:
      if (good_frame_by_frmbuilder) {
  ...
  bool eof() const { return (frames.end() == this_frame); }
...
private:
  ...
  void _m_render(std::map<window, output_t> &outs, const nana::size &frame_size,
...
// public:
frameset::frameset() : impl_(std::make_unique<impl>()) {}
...
struct animation::impl {
  ...
  void render_this_frame(paint::graphics &graph, const rectangle &area) {
    if (state.this_frameset != framesets.end())
      state.this_frameset->impl_->render_this(graph, area, framegraph,
                                              framegraph_dimension);
  }

  // Renders current from to all outputs graphics
  void render_this_frame() {
    if (state.this_frameset != framesets.end())
      state.this_frameset->impl_->render_this(outputs, framegraph,
                                              framegraph_dimension);
  }
...
void animation::performance_manager::insert(impl *p) {
  ...
  for (auto thr : threads_) {
    ...
    if (thr->fps == p->fps) {
      if (thr->animations.empty() ||
          (thr->performance_parameter * (1.0 + 1.0 / thr->animations.size()) <=
           43.3)) {
        p->thr_variable = thr;
  ...
  thr->thread = std::make_shared<std::thread>([pthr]() {
    ...
    while (true) {
      ...
      {
        // acquire the isg lock first to avoid deadlock that occured by an event
        // hander which operates the animation object.
      ...
      if (thr->performance_parameter < thr->interval)
        nana::system::sleep(
            static_cast<unsigned>(thr->interval - thr->performance_parameter));
...
animation::animation(std::size_t fps) : impl_(std::make_unique<impl>(fps)) {}

animation::~animation() = default;

animation::animation(animation &&rhs) : impl_(std::move(rhs.impl_)) {
...
void branch(const std::string& name, const frameset& frms,
std::function<std::size_t(const std::string&, std::size_t, std::size_t&)>
condition)
{
...
void animation::pause() { impl_->paused = true; }
...
void animation::output(window wd, const nana::point &pos) {
  ...
  if (nullptr == output.diehard) {
    ...
    API::events(wd).destroy.connect([this](const arg_destroy &arg) {
      std::lock_guard<decltype(impl_->thr_variable->mutex)> lock(
          impl_->thr_variable->mutex);
...
std::size_t animation::fps() const { return impl_->fps; }
// end class animation
