          ...
          for (auto &t : targets_) {
            if (API::is_window_zoomed(t.wd, true) == false) {
              ...
              if (!t.restrict_area.empty())
                _m_check_restrict_area(wdps, API::window_size(t.wd),
                                       t.restrict_area);
...
private:
  static void _m_check_restrict_area(nana::point &pos, const nana::size &size,
                                     const nana::rectangle &restr_area) {
    if ((pos.x > 0) &&
        (static_cast<int>(size.width) + pos.x > restr_area.right()))
    ...
    if ((pos.y > 0) &&
        (static_cast<int>(size.height) + pos.y > restr_area.bottom()))
...
// class dragger
dragger::dragger() : impl_(new dragger_impl_t) {}

dragger::~dragger() { delete impl_; }

dragger::dragger(dragger &&other) : impl_(other.impl_) {
  other.impl_ = nullptr;
}
...
void dragger::target(window wd, const rectangle &restrict_area,
                     nana::arrange arg) {
  impl_->drag_target(wd, restrict_area, arg);
}

void dragger::remove_target(window wd) { impl_->remove_target(wd); }

void dragger::trigger(window tg) { impl_->trigger(tg); }
// end class dragger
} // end namespace nana
