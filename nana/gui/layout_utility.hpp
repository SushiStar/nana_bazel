...
bool overlap(const rectangle &ir, const size &valid_input_area,
             const rectangle &dr, const size &valid_dst_area,
             rectangle &output_src_r, rectangle &output_dst_r);
...
// zoom
//@brief:	Calculate the scaled rectangle by refer dst rectangle, that scale
//factor is same as that between scaled and refer.
void zoom(const rectangle &refer, const rectangle &scaled,
          const rectangle &refer_dst, rectangle &r);

// covered
//@brief:	Tests a rectangle whether it is wholly covered by another.
bool covered(const rectangle &
                 underlying, // 1st rectangle must be placed under 2nd rectangle
