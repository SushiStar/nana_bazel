...
HICON image_accessor::icon(const nana::paint::image &img) {
  auto ico_res =
      dynamic_cast<paint::detail::image_ico_resource *>(img.image_ptr_.get());
  if (ico_res)
...
#else
int image_accessor::icon(const image &) { return 0; }
#endif
...
image::image(const image &rhs) : image_ptr_(rhs.image_ptr_) {}

image::image(image &&r) : image_ptr_(std::move(r.image_ptr_)) {}

image::image(const std::string &file) { open(file); }

image::image(const std::wstring &file) { open(file); }
...
std::shared_ptr<image::image_impl_interface> create_image(const char *buf,
                                                          std::size_t len) {
  if (buf && len >= 8) {
...
#endif
    } else if (*reinterpret_cast<const short *>("BM") ==
               *reinterpret_cast<const short *>(buf))
      return std::make_shared<detail::image_bmp>();
    else if (*reinterpret_cast<const short *>("MZ") ==
             *reinterpret_cast<const short *>(buf))
...
void image::close() noexcept { image_ptr_.reset(); }
...
void image::paste(const rectangle &r_src, graphics &dst,
                  const point &p_dst) const {
  if (image_ptr_) {
    if (dst.empty())
...
void image::stretch(const nana::rectangle &r_src, graphics &dst,
                    const nana::rectangle &r_dst) const {
  if (image_ptr_) {
    if (dst.empty())
      dst.make({r_src.width, r_src.height}); // throws if failed to create
