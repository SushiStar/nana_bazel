...
utf8_Error::utf8_Error(const std::string &msg) : std::runtime_error(msg) {}
#endif
#endif
...
#endif
  {
    utf8_Error(std::string("\nThe const text is not encoded in UTF8: ") + text)
        .emit();
...
#endif
  {
    utf8_Error(std::string("\nThe text is not encoded in UTF8: ") + text)
        .emit();
...
std::wstring to_wstring(std::string_view utf8_str) {
  ...
  return ::nana::charset(std::string{utf8_str.data(), utf8_str.size()},
                         unicode::utf8);
}
#else
...
const std::wstring &to_wstring(const std::wstring &wstr) { return wstr; }
...
#else
std::string to_osmbstr(std::string text_utf8) { return text_utf8; }
#endif
...
detail::native_string_type to_nstring(int n) { return std::to_wstring(n); }

detail::native_string_type to_nstring(double d) { return std::to_wstring(d); }
...
detail::native_string_type to_nstring(int n) { return std::to_string(n); }

detail::native_string_type to_nstring(double d) { return std::to_string(d); }
...
#include "nana/common/verbose_preprocessor.hpp"
#endif
