...
long long stoll(const std::wstring &, std::size_t *pos = nullptr,
                int base = 10);

// Workaround for no implementation of std::stoul in MinGW.
unsigned long stoul(const std::string &, std::size_t *pos = nullptr,
                    int base = 10);
unsigned long stoul(const std::wstring &, std::size_t *pos = nullptr,
                    int base = 10);

// Workaround for no implementation of std::stoull in MinGW.
unsigned long long stoull(const std::string &, std::size_t *pos = nullptr,
                          int base = 10);
unsigned long long stoull(const std::wstring &, std::size_t *pos = nullptr,
                          int base = 10);
} // namespace std
...
// template<   >
std::string put_time /*<char, std::string>*/ (const std::tm *tmb,
                                              const char *fmt);

// Defined in header <ctime>
//	std::size_t strftime(char* str, std::size_t count, const char* format,
// const std::tm* time); template<> std::wstring put_time<wchar_t,
// std::wstring>(const std::tm* tmb, const wchar_t* fmt);
} // namespace std
