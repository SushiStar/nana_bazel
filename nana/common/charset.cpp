 ...
 *	@brief A conversion between unicode characters and multi bytes
 *characters
 *	@contributions
...
/// return a code point (max 16 bits?) and the len in code units of the
/// character at pos
wchar_t char_at(const char *text_utf8, unsigned pos, unsigned *len) {
  ...
  if (uch < 0xE0 && (text_utf8 + 1 <= end)) {
    if (len)
      *len = 2;
    return (wchar_t(uch & 0x1F) << 6) |
           (reinterpret_cast<const unsigned char *>(text_utf8)[1] & 0x3F);
    ...
    return ((((uch & 0xF) << 6) |
             (reinterpret_cast<const unsigned char *>(text_utf8)[1] & 0x3F))
            << 6) |
           (reinterpret_cast<const unsigned char *>(text_utf8)[2] & 0x3F);
    ...
    return ((((((uch & 0x7) << 6) |
             ...
             (reinterpret_cast<const unsigned char *>(text_utf8)[2] & 0x3F))
            << 6) |
           (reinterpret_cast<const unsigned char *>(text_utf8)[3] & 0x3F);
  }
...
wchar_t char_at(const ::std::string &text_utf8, unsigned pos, unsigned *len) {
  ...
  if (uch < 0xE0 && (ptr + 1 <= end)) {
    if (len)
      *len = 2;
    return (wchar_t(uch & 0x1F) << 6) |
           (reinterpret_cast<const unsigned char *>(ptr)[1] & 0x3F);
    ...
    return ((((uch & 0xF) << 6) |
             (reinterpret_cast<const unsigned char *>(ptr)[1] & 0x3F))
            << 6) |
           (reinterpret_cast<const unsigned char *>(ptr)[2] & 0x3F);
    ...
    return ((((((uch & 0x7) << 6) |
             ...
             (reinterpret_cast<const unsigned char *>(ptr)[2] & 0x3F))
            << 6) |
           (reinterpret_cast<const unsigned char *>(ptr)[3] & 0x3F);
  }
...
/// convert wchar C string from ? ANSI code page CP_ACP (windows) or LC_CTYPE c
/// locale (-nix) into utf8 std::string
...
/// convert a char C-string from The system default Windows ANSI code page
/// CP_ACP or from LC_CTYPE c locale (-nix) into utf16 std::wstring
...
/// convert a char C string from The system default Windows ANSI code page
/// CP_ACP or LC_CTYPE c locale (-nix) into utf16 std::string
bool mb2wc(std::string &wcstr, const char *s) {
  ...
  if (chars > 1) {
    ...
    ::MultiByteToWideChar(CP_ACP, 0, s, -1,
                          reinterpret_cast<wchar_t *>(&wcstr[0]), chars - 1);
    // ^ the trick !
  }
#else
  locale_initializer::init();
...
/// playing with the idea - we need a mechanism to set a user selected police -
/// Testing an abstract interface
struct encoding_error_police {
  virtual unsigned long next_code_point(const unsigned char *&current_code_unit,
                                        const unsigned char *end) = 0;
  virtual ~encoding_error_police() = default;
};

/// the current nana default: it is safe - you may want to keep it ! use the
/// other at your risk: mainly for debugging
struct utf8_error_police : public encoding_error_police {
  unsigned long next_code_point(const unsigned char *&current_code_unit,
...
struct utf8_error_police_throw : public encoding_error_police {
  unsigned long next_code_point(const unsigned char *&current_code_unit,
    ...
    utf8_Error(std::string("The text is not encoded in UTF8: ") +
               reinterpret_cast<const char *>(current_code_unit))
        .emit();
    ;
    current_code_unit = end;
    return 0;
...
//		auto def_encoding_error_police =
// std::make_unique<utf8_error_police>();  // the nana default 		auto
// def_encoding_error_police = std::make_unique<utf8_error_police_latin>();
// auto def_encoding_error_police = std::make_unique<utf8_error_police_throw>();
// auto def_encoding_error_police =
// std::make_unique<utf8_error_police_def_char>('X');
auto def_encoding_error_police = std::make_unique<utf8_error_police_system>();

#ifndef STD_CODECVT_NOT_SUPPORTED
class charset_string : public charset_encoding_interface {
public:
  charset_string(const std::string &s) : data_(s) {}

  charset_string(std::string &&s) : data_(std::move(s)) {}
...
private:
  ...
  virtual std::string str() const {
    if (is_unicode_) {
      ...
      switch (utf_x_) {
      case unicode::utf8:
        wcstr = std::wstring_convert<std::codecvt_utf8<wchar_t>>().from_bytes(
            data_);
        break;
      case unicode::utf16:
        wcstr = std::wstring_convert<
                    std::codecvt_utf16<wchar_t, 0x10FFFF, std::little_endian>>()
                    .from_bytes(data_);
        break;
      case unicode::utf32:
        wcstr.append(reinterpret_cast<const wchar_t *>(data_.c_str()),
                     data_.size() / sizeof(wchar_t));
        break;
      }
  ...
  virtual std::string str(unicode encoding) const {
    if (is_unicode_ && (utf_x_ != encoding)) {
      ...
      case unicode::utf8:
        switch (encoding) {
        case unicode::utf16:
          return std::wstring_convert<std::codecvt_utf16<char16_t>, char16_t>()
              .to_bytes(std::wstring_convert<std::codecvt_utf8_utf16<char16_t>,
                                             char16_t>()
                            .from_bytes(data_));
        case unicode::utf32: {
          std::u32string u32str =
              std::wstring_convert<std::codecvt_utf8<char32_t>, char32_t>()
                  .from_bytes(data_);
          return std::string(reinterpret_cast<const char *>(u32str.c_str()),
      ...
      case unicode::utf16:
        ...
        case unicode::utf8:
          return std::wstring_convert<std::codecvt_utf8_utf16<char16_t>,
                                      char16_t>()
              .to_bytes(
                  std::wstring_convert<std::codecvt_utf16<char16_t>, char16_t>()
                      .from_bytes(data_));
        case unicode::utf32: {
          std::u32string u32str =
      ...
      case unicode::utf32:
        ...
        case unicode::utf8:
          return std::wstring_convert<std::codecvt_utf8<char32_t>, char32_t>()
              .to_bytes(std::u32string(
                  reinterpret_cast<const char32_t *>(data_.c_str()),
                  data_.size() / sizeof(char32_t)));
        case unicode::utf16:
          return std::wstring_convert<std::codecvt_utf16<char32_t>, char32_t>()
              .to_bytes(std::u32string(
                  reinterpret_cast<const char32_t *>(data_.c_str()),
                  data_.size() / sizeof(char32_t)));
        default:
    ...
    if (mb2wc(wcstr, data_.c_str())) {
...
#if defined(NANA_WINDOWS)
      {
        ...
        std::u32string utf32str =
            std::wstring_convert<std::codecvt_utf16<char32_t>, char32_t>()
                .from_bytes(bytes, bytes + sizeof(wchar_t) * wcstr.size());
        return std::string(reinterpret_cast<const char *>(utf32str.c_str()),
                           sizeof(char32_t) * utf32str.size());
      }
#elif defined(NANA_POSIX)
        return std::string(reinterpret_cast<const char *>(wcstr.c_str()),
                           sizeof(wchar_t) * wcstr.size());
#else
    ...
    if (is_unicode_) {
      ...
      case unicode::utf16:
        return std::wstring_convert<
            ...
            .from_bytes(data_);
      case unicode::utf32:
        return std::wstring(reinterpret_cast<const wchar_t *>(data_.c_str()),
                            data_.size() * sizeof(wchar_t));
      }
      return {};
    }
...
class charset_wstring : public charset_encoding_interface {
public:
  charset_wstring(const std::wstring &s) : data_(s) {}

  charset_wstring(std::wstring &&s) : data_(std::move(s)) {}
  ...
  virtual std::string str(unicode encoding) const {
...
#if defined(NANA_WINDOWS)
    {
      ...
      std::u32string utf32str =
          std::wstring_convert<std::codecvt_utf16<char32_t>, char32_t>()
              .from_bytes(bytes, bytes + sizeof(wchar_t) * data_.size());
      return std::string(reinterpret_cast<const char *>(utf32str.c_str()),
                         sizeof(char32_t) * utf32str.size());
    }
#elif defined(NANA_POSIX)
      return std::string(reinterpret_cast<const char *>(data_.c_str()),
                         data_.size() * sizeof(wchar_t));
#else
      throw std::runtime_error("Bad charset");
#endif
  ...
  virtual std::wstring wstr() const { return data_; }

  virtual std::wstring &&wstr_move() { return std::move(data_); }
...
/// return the first code point and move the pointer to next character,
/// springing to the end by errors
unsigned long utf8char(const unsigned char *&p, const unsigned char *end) {
  if (p != end) {
    ...
    {
      code =
          ((((((ch & 0x7) << 6) | (p[1] & 0x3F)) << 6) | (p[2] & 0x3F)) << 6) |
          (p[3] & 0x3F);
      p += 4;
...
unsigned long utf16char(const unsigned char *&bytes, const unsigned char *end,
                        bool le_or_be) {
  unsigned long code;
  if (le_or_be) {
...
unsigned long utf32char(const unsigned char *&bytes, const unsigned char *end,
                        bool le_or_be) {
  if (end - bytes >= 4) {
    unsigned long code;
    if (le_or_be)
...
std::string utf32_to_utf8(const std::string &s) {
  ...
  // If there is a BOM, ignore it
  if (s.size() >= 4) {
    if (bytes[0] == 0 && bytes[1] == 0 && bytes[2] == 0xFE &&
        bytes[3] == 0xFF) {
      le_or_be = false;
      bytes += 4;
      utf8str += (char)0xEF;
      utf8str += (char)0xBB;
      utf8str += (char)0xBF;
    } else if (bytes[0] == 0xFF && bytes[1] == 0xFE && bytes[2] == 0 &&
               bytes[3] == 0) {
      le_or_be = true;
      bytes += 4;
      utf8str += (char)0xEF;
...
private:
  ...
  virtual std::wstring wstr() const {
    if (is_unicode_) {
      ...
      return std::wstring(reinterpret_cast<const wchar_t *>(bytes.c_str()),
...
public:
  ...
  virtual std::string str(unicode encoding) const {
    ...
    case unicode::utf8:
#if defined(NANA_WINDOWS)
      return detail::utf16_to_utf8(
          std::string(reinterpret_cast<const char *>(data_.c_str()),
                      data_.size() * sizeof(wchar_t)));
#else
      return detail::utf32_to_utf8(
          std::string(reinterpret_cast<const char *>(data_.c_str()),
                      data_.size() * sizeof(wchar_t)));
#endif
    case unicode::utf16:
      ...
      return std::string(reinterpret_cast<const char *>(data_.c_str()),
                         data_.size() * sizeof(wchar_t));
#else
      return detail::utf32_to_utf16(
          std::string(reinterpret_cast<const char *>(data_.c_str()),
                      data_.size() * sizeof(wchar_t)));
#endif
    case unicode::utf32:
#if defined(NANA_WINDOWS)
      return detail::utf16_to_utf32(
          std::string(reinterpret_cast<const char *>(data_.c_str()),
                      data_.size() * sizeof(wchar_t)));
#else
      return std::string(reinterpret_cast<const char *>(data_.c_str()),
...
charset::charset(charset &&r) : impl_(r.impl_) { r.impl_ = 0; }
...
charset::charset(const std::string &s) : impl_(new detail::charset_string(s)) {}
...
charset::~charset() { delete impl_; }

charset::operator std::string() const { return impl_->str(); }

charset::operator std::string &&() { return impl_->str_move(); }

charset::operator std::wstring() const { return impl_->wstr(); }

charset::operator std::wstring &&() { return impl_->wstr_move(); }
