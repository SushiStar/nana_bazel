...
#include "nana/common/internationalization.hpp"
...
enum class token { msgid, msgstr, string, eof };
  ...
  tokenizer(const std::string &file, bool utf8) {
    ...
    if (ifs) {
      ...
      if (len > 0) {
        ...
        read_ptr_ = data_.get();
        if (utf8 && len > 3) {
          if (static_cast<unsigned char>(read_ptr_[0]) == 0xEF &&
              static_cast<unsigned char>(read_ptr_[1]) == 0xBB &&
              static_cast<unsigned char>(read_ptr_[2]) == 0xBF)
            read_ptr_ += 3;
        }
...
struct data {
  ...
  data() {
    ...
    table["NANA_FILEBOX_ERROR_INVALID_FOLDER_NAME"] =
        "Please input a valid name for the new folder.";
    table["NANA_FILEBOX_ERROR_RENAME_FOLDER_BECAUSE_OF_EXISTING"] =
        "The folder is existing, please rename it.";
    table["NANA_FILEBOX_ERROR_RENAME_FOLDER_BECAUSE_OF_FAILED_CREATION"] =
        "Failed to create the folder, please rename it.";
    table["NANA_FILEBOX_ERROR_INVALID_FILENAME"] =
        "\"%arg0\"\nThe filename is invalid.";
    table["NANA_FILEBOX_ERROR_NOT_EXISTING_AND_RETRY"] =
        "The file \"%arg0\"\n is not existing. Please check and retry.";
    table["NANA_FILEBOX_ERROR_DIRECTORY_NOT_EXISTING_AND_RETRY"] =
        "The directory \"%arg0\"\n is not existing. Please check and retry.";
    table["NANA_FILEBOX_ERROR_DIRECTORY_INVALID"] =
        "The directory \"%arg0\"\n is invalid. Please check and retry.";
    table["NANA_FILEBOX_ERROR_QUERY_REWRITE_BECAUSE_OF_EXISTING"] =
        "The input file is existing, do you want to overwrite it?";
  }
};
...
struct eval_window {
  ...
  eval_window(i18n_eval &&arg) : eval(std::move(arg)) {}
};
...
void set_eval(nana::window wd, i18n_eval &&eval) {
  ...
  if (i == mgr.table.end()) {
    ...
    result.first->second.destroy =
        nana::API::events(wd).destroy.connect([wd](const arg_destroy &) {
...
void internationalization::set_missing(
    std::function<void(const std::string &msgid_utf8)> handler) {
...
void internationalization::_m_replace_args(
    std::string &str, std::vector<std::string> *arg_strs) const {
  ...
  while (true) {
    ...
    if ((pos == str.npos) || (pos != offset + 4)) {
      ...
      std::size_t arg =
          static_cast<std::size_t>(std::stoi(str.substr(offset + 4, arg_n)));
...
void internationalization::_m_fetch_args(std::vector<std::string> &v,
                                         const char *arg) {
  v.emplace_back(arg);
}
...
class i18n_eval::arg_string : public eval_arg {
public:
  arg_string(std::string str) : str_(std::move(str)) {}

  std::string eval() const override { return str_; }
...
class i18n_eval::arg_eval : public eval_arg {
public:
  arg_eval(const i18n_eval &eval) : eval_{eval} {}

  arg_eval(i18n_eval &&eval) : eval_{std::move(eval)} {}
...
// class i18n_eval
i18n_eval::i18n_eval(const i18n_eval &rhs) : msgid_(rhs.msgid_) {
  for (auto &arg : rhs.args_)
    args_.emplace_back(arg->clone());
}

// Workaround for VC2013, because it can't specify a default explicit
// move-constructor
