...
color::color(colors clr)
    : color((static_cast<unsigned>(clr) & 0xFF0000) >> 16,
            (static_cast<unsigned>(clr) & 0xFF00) >> 8,
            static_cast<unsigned>(clr) & 0xFF) {}

color::color(colors clr, double alpha)
    : color((static_cast<unsigned>(clr) & 0xFF0000) >> 16,
            (static_cast<unsigned>(clr) & 0xFF00) >> 8,
            static_cast<unsigned>(clr) & 0xFF, alpha) {}

color::color(color_rgb rgb)
    : r_((static_cast<int>(rgb) >> 16) & 0xFF),
      g_((static_cast<int>(rgb) >> 8) & 0xFF), b_(static_cast<int>(rgb) & 0xFF),
      a_(1.0) {}
...
std::string read_number(std::string &str, std::size_t &pos) {
  ...
  if (str[end] == '.') {
    ...
    if ((decimal_end == str.npos) ||
        (decimal_end == end + 1)) // Because of missing %
      return {};
...
color::color(std::string css_color) : a_(1.0) {
  ...
  if ('#' == css_color[pos]) {
    ...
    auto endpos =
        css_color.find_first_not_of("0123456789abcdefABCDEF", pos + 1);
    ...
    auto n =
        std::stoi(css_color.substr(pos + 1, endpos - pos - 1), nullptr, 16);
  ...
  std::transform(css_color.begin(), css_color.end(), css_color.begin(),
                 [](char ch) {
                   if ('A' <= ch && ch <= 'Z')
  ...
  if (type_end == css_color.npos ||
      ((type_end - pos != 3) &&
       (type_end - pos != 4))) // rgb/hsl = 3, rgba/hsla = 4
    throw std::invalid_argument(excpt_what);
  ...
  if ("rgb" == type_name) {
    pat.assign("(\\d*\\.)?\\d+\\%?");
    i = std::regex_iterator<std::string::iterator>(css_color.begin() + pos,
                                                   css_color.end(), pat);
...
bool color::invisible() const { return (a_ == 0.0); }

pixel_color_t color::px_color() const { return argb(); }
...
const double &color::r() const { return r_; }
const double &color::g() const { return g_; }
const double &color::b() const { return b_; }

const double &color::a() const { return a_; }
...
size::size(value_type width, value_type height)
    : width(width), height(height) {}

bool size::empty() const { return (0 == width || 0 == height); }

bool size::is_hit(const point &pos) const {
  return (0 <= pos.x && pos.x < static_cast<int>(width) && 0 <= pos.y &&
...
bool rectangle::operator==(const rectangle &rhs) const {
  return (width == rhs.width) && (height == rhs.height) && (x == rhs.x) &&
         (y == rhs.y);
}
...
point rectangle::position() const noexcept { return {x, y}; }
...
size rectangle::dimension() const noexcept { return {width, height}; }
...
int rectangle::right() const noexcept { return x + static_cast<int>(width); }

int rectangle::bottom() const noexcept { return y + static_cast<int>(height); }
...
bool rectangle::empty() const { return (0 == width) || (0 == height); }
...
rectangle_rotator::rectangle_rotator(bool rotated,
                                     const ::nana::rectangle &area)
    : rotated_(rotated), area_(area) {}

int rectangle_rotator::x() const { return (rotated_ ? area_.y : area_.x); }

int &rectangle_rotator::x_ref() { return (rotated_ ? area_.y : area_.x); }

int rectangle_rotator::y() const { return (rotated_ ? area_.x : area_.y); }

int &rectangle_rotator::y_ref() { return (rotated_ ? area_.x : area_.y); }
...
int rectangle_rotator::right() const {
  return (rotated_ ? area_.y + static_cast<int>(area_.height)
                   : area_.x + static_cast<int>(area_.width));
}

int rectangle_rotator::bottom() const {
  return (rotated_ ? area_.x + static_cast<int>(area_.width)
                   : area_.y + static_cast<int>(area_.height));
}

const ::nana::rectangle &rectangle_rotator::result() const { return area_; }
