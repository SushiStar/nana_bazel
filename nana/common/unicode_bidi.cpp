 ...
 */
#include "nana/common/unicode_bidi.hpp"
#include "nana/common/c++defines.hpp"
...
static unsigned char charmap_0x0000_0x00C0[192] = {
    BN, BN, BN, BN, BN, BN, BN, BN, BN, S,  B,  S,  WS, B,  BN, BN, BN, BN,
    BN, BN, BN, BN, BN, BN, BN, BN, BN, BN, B,  B,  B,  S,  WS, ON, ON, ET,
    ET, ET, ON, ON, ON, ON, ON, ES, CS, ES, CS, CS, EN, EN, EN, EN, EN, EN,
    EN, EN, EN, EN, CS, ON, ON, ON, ON, ON, ON, L,  L,  L,  L,  L,  L,  L,
    L,  L,  L,  L,  L,  L,  L,  L,  L,  L,  L,  L,  L,  L,  L,  L,  L,  L,
    L,  ON, ON, ON, ON, ON, ON, L,  L,  L,  L,  L,  L,  L,  L,  L,  L,  L,
    L,  L,  L,  L,  L,  L,  L,  L,  L,  L,  L,  L,  L,  L,  L,  ON, ON, ON,
    ON, BN, BN, BN, BN, BN, BN, B,  BN, BN, BN, BN, BN, BN, BN, BN, BN, BN,
    BN, BN, BN, BN, BN, BN, BN, BN, BN, BN, BN, BN, BN, BN, BN, BN, CS, ON,
    ET, ET, ET, ET, ON, ON, ON, ON, L,  ON, ON, BN, ON, ON, ET, ET, EN, EN,
    ON, L,  ON, ON, ON, EN, L,  ON, ON, ON, ON, ON};

t bidi_char_type(wchar_t ch) {
  if (ch <= 0x0FC6) {
    if (ch < 0x00C0)
      return static_cast<bidi_charmap::t>(charmap_0x0000_0x00C0[ch]);
    if (ch == 0x00D7 || ch == 0x00F7)
      return ON;
    if (ch <= 0x02B8)
      return L; // N = 449
    if (ch <= 0x02BA)
    ...
    if (0x058F == ch)
      return ET;
    if (0x05BE == ch || 0x05C0 == ch || 0x05C3 == ch || 0x05C6 == ch)
      return R;
    if (ch <= 0x05C7)
      return NSM;
    if (ch <= 0x05F4)
    ...
    if (ch <= 0x0815)
      return R; // N = 28
    if (0x081A == ch || 0x0824 == ch || 0x0828 == ch)
      return R;
    if (ch <= 0x082D)
      return NSM; // N = 5
    if (ch <= 0x0858)
    ...
    if (0x0AF1 == ch)
      return ET;
    if (0x0B01 == ch || 0x0B3C == ch || 0x0B3F == ch)
      return NSM;
    if (ch <= 0x0B40)
      return L;
    if (ch <= 0x0B44)
    ...
    if (ch <= 0x0B63)
      return NSM; // N = 2
    if (0x0B82 == ch || 0x0BC0 == ch || 0x0BCD == ch)
      return NSM;
    if (ch <= 0x0BF2)
      return L; // N = 35
    if (0x0BF9 == ch)
  ...
  if (ch <= 0x1FFE) {
    ...
    if (ch <= 0x1CC7)
      return L; // N = 141
    if (0x1CD3 == ch || 0x1CE1 == ch)
      return L;
    if (ch <= 0x1CE8)
      return NSM; // N = 7
    if (0x1CED == ch || 0x1CF4 == ch)
      return NSM;
    if (ch <= 0x1DBF)
      return L; // N = 203
    if (ch <= 0x1DFF)
...
// class unicode_bidi
std::vector<unicode_bidi::entity> unicode_bidi::reorder(const char_type *str,
                                                        std::size_t len) {
  levels_.clear();
...
unsigned unicode_bidi::_m_paragraph_level(const char_type *begin,
                                          const char_type *end) {
...
void unicode_bidi::_m_push_entity(const char_type *begin, const char_type *end,
                                  unsigned level, bidi_char bidi_char_type) {
...
std::vector<unicode_bidi::entity>::iterator
unicode_bidi::_m_search_first_character() {
...
void unicode_bidi::_m_resolve_weak_types() {
  ...
  // W1. Examine each nonspacing mark, and change the type of the NSM to the
  // type of the previous character. W2. Search backward from each instance of a
  // European number until the first strong type(R, L, AL, or sor) is found If
  // an AL is found, change the type of the European Number to Arabic number.
  // W3. Change all ALs to R.

  // The three phases could be combined as one process. Because these phases are
  // standalone. Generally, the directional char type of sor is taken on current
  // level.
  bidi_char prev =
      (level_of_run & 1 ? bidi_char::R : bidi_char::L); // sor is either L or R
  bool change_european_number = false;

  for (auto i = begin_character; i != end; ++i) {
    if (level_of_run != i->level) {
      level_of_run = i->level;
      prev = (level_of_run & 1 ? bidi_char::R
                               : bidi_char::L); // sor is either L or R
    }
  ...
  // to all European numbers. W6. Otherwise, separators and terminators change
  // to Other Neutral.
  auto etpos = end; // Indicates the head of the sequence of European
                    // Terminators
  bool head = true;
  for (auto i = begin_character; i != end; ++i) {
    switch (i->bidi_char_type) {
    case bidi_char::ES:
      ...
      if ((false == head) && (i != last)) {
        if ((bidi_char::EN == (i - 1)->bidi_char_type) &&
            (bidi_char::EN == (i + 1)->bidi_char_type))
    ...
    case bidi_char::CS:
      ...
      if ((false == head) && (i != last)) {
        ...
        if (((i - 1)->bidi_char_type == right_one) &&
            (bidi_char::AN == right_one || bidi_char::EN == right_one))
  ...
  // W7. Search backward from each instance of a European number until the first
...
void unicode_bidi::_m_resolve_neutral_types() {
  ...
  // numbers act as if they were R in terms of their influence on neutrals.
  ...
  for (auto i = begin_character; i != end; ++i) {
    ...
    if (_m_bidi_category(i->bidi_char_type) == bidi_category::neutral) {
      if (begin_neutral == end) {
        begin_neutral = i;
        left =
            (head_of_run ? (level_of_run & 1 ? bidi_char::R : bidi_char::L)
                         : (i - 1)->bidi_char_type); // Head ? sor : prev_char
    ...
    } else if (begin_neutral != end) // This element is not a neutral
    {
      ...
      bidi_char target =
          (left == right
               ? left
               : (begin_neutral->level & 1 ? bidi_char::R : bidi_char::L));
...
void unicode_bidi::_m_resolve_implicit_levels() {
  ...
  // levels. I2. For all characters with an odd (right-to-left) embedding
...
void unicode_bidi::_m_reordering_resolved_levels(
    std::vector<entity> &reordered) {
  reordered = levels_;

  // First find the highest_level for resolution, because the resolution is from
  // highest level.
...
unicode_bidi::bidi_category
unicode_bidi::_m_bidi_category(bidi_char bidi_char_type) {
  return static_cast<unicode_bidi::bidi_category>(
      static_cast<int>(bidi_char_type) & 0xF000);
}
...
unicode_bidi::bidi_char unicode_bidi::_m_char_dir(char_type ch) {
  ...
  if (type < bidi_charmap::B)
    return static_cast<bidi_char>(static_cast<int>(type - bidi_charmap::PDF) +
                                  0x1000);

  return static_cast<bidi_char>(static_cast<int>(type - bidi_charmap::B) +
                                0x2000);
}
// end class unicode_bidi

std::vector<unicode_bidi::entity> unicode_reorder(const wchar_t *text,
                                                  std::size_t length) {
...
unicode_character_type unicode_char_type(unsigned long ch) {
  if ((0x0600 <= ch && ch <= 0x0603) ||
      (0x06DD == ch || 0x070F == ch || 0x17B4 == ch || 0x17B5 == ch) ||
      (0x200C <= ch && ch <= 0x200F) || (0x202A <= ch && ch <= 0x202E) ||
      (0x2060 <= ch && ch <= 0x2063) || (0x206A <= ch && ch <= 0x206F) ||
      (0xFEFF == ch) || (0xFFF9 <= ch && ch <= 0xFFFB) ||
      (0x1D173 <= ch && ch <= 0x1D17A) || (0xE0001 == ch) ||
      (0xE0020 <= ch && ch <= 0xE007F))
    return unicode_character_type::format;

  if ((0x30A1 <= ch && ch <= 0x30FA) || (0x30FC <= ch && ch <= 0x30FF) ||
      (0x31F0 <= ch && ch <= 0x31FF) || (0xFF66 <= ch && ch <= 0xFF9F))
    return unicode_character_type::katakana;

  if (('A' <= ch && ch <= 'Z') || ('a' <= ch && ch <= 'z') ||
      (0x00AA == ch || 0x00B5 == ch || 0x00BA == ch) ||
      (0x00C0 <= ch && ch <= 0x00D6) || (0x00D8 <= ch && ch <= 0x00F6) ||
      (0x00F8 <= ch && ch <= 0x0236) || (0x0250 <= ch && ch <= 0x02C1) ||
      // Hebrew
      (0x05BB <= ch && ch <= 0x05BD) || (0x05BF == ch) ||
      ((0x05C1 <= ch && ch <= 0x05C4) && (0x05C3 != ch)) ||
      (0x05D0 <= ch && ch <= 0x05EA) || (0x05F0 <= ch && ch <= 0x05F3) ||
      // Arabic
      (0x0610 <= ch && ch <= 0x0615) || (0x0621 <= ch && ch <= 0x063A) ||
      (0x0640 <= ch && ch <= 0x0657) || (0x066E <= ch && ch <= 0x06D3) ||
      (0x06D5 <= ch && ch <= 0x06DC) || (0x06E1 <= ch && ch <= 0x06E8) ||
      (0x06ED == ch || 0x06EF == ch) || (0x06FA <= ch && ch <= 0x06FC) ||
      (0x06FF == ch))
    return unicode_character_type::aletter;

  if ('\'' == ch || 0x00AD == ch || 0x00B7 == ch || 0x05F4 == ch ||
      0x2019 == ch || 0x2027 == ch)
  ...
  if (('0' <= ch && ch <= '9') || (0x0660 <= ch && ch <= 0x0669) ||
      (0x06F0 <= ch && ch <= 0x06F9))
...
bool unicode_wordbreak(wchar_t left, wchar_t right) {
  ...
  case unicode_character_type::katakana:
    return !(unicode_character_type::format == r_type) ||
           (unicode_character_type::katakana == r_type);
  case unicode_character_type::aletter:
  case unicode_character_type::numeric:
    return !((unicode_character_type::format == r_type) ||
             (unicode_character_type::aletter == r_type) ||
             (unicode_character_type::numeric == r_type));
  }
  return true;
}
} // end namespace nana
