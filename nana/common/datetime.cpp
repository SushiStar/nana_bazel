...
::nana::date::value to_dateval(const std::tm &t) {
  return {static_cast<unsigned>(t.tm_year + 1900),
          static_cast<unsigned>(t.tm_mon + 1),
          static_cast<unsigned>(t.tm_mday)};
}

::nana::time::value to_timeval(const std::tm &t) {
  return {static_cast<unsigned>(t.tm_hour), static_cast<unsigned>(t.tm_min),
          static_cast<unsigned>(t.tm_sec)};
}
...
namespace nana {
// class date
void date::set(const std::tm &t) { value_ = to_dateval(t); }

date::date() : value_(to_dateval(localtime())) {}

date::date(const std::tm &t) : value_(to_dateval(t)) {}

date::date(int year, int month, int day)
    : value_({static_cast<unsigned>(year), static_cast<unsigned>(month),
...
bool date::operator==(const date &r) const {
  return (r.value_.year == value_.year && r.value_.month == value_.month &&
          r.value_.day == value_.day);
}

bool date::operator!=(const date &r) const { return !(this->operator==(r)); }
...
int date::day_of_week() const {
  return day_of_week(static_cast<int>(value_.year),
                     static_cast<int>(value_.month),
                     static_cast<int>(value_.day));
}

int date::day_of_week(int year, int month, int day) {
  // w=y+[y/4]+[c/4]-2c+[26(m+1)/10]+d-1;
  // the Jan and Feb of Every year are treated as the 13th/14th month of last
  // year.
  ...
  int w = year + (year / 4) + (century / 4) - (2 * century) +
          (26 * (month + 1) / 10) + day - 1;
...
const date::value &date::read() const { return this->value_; }
...
unsigned date::month_days(const unsigned year, const unsigned month) {
  if (month != 2) {
    constexpr std::array<unsigned, 12> days_in_month = {31, 0,  31, 30, 31, 30,
                                                        31, 31, 30, 31, 30, 31};
...
// class time
void time::set(const std::tm &t) { value_ = to_timeval(t); }

time::time() : value_(to_timeval(localtime())) {}

time::time(const std::tm &t) : value_(to_timeval(t)) {}
...
const time::value &time::read() const { return value_; }
// end class time
} // end namespace nana
