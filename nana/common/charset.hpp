...
namespace nana {
namespace utf {
/// Attempt to get a pointer to a character of UTF-8 string by a specified
/// character index.
/// @param text_utf8 A string encoded as UTF-8.
/// @param pos The unicode character index.
/// @returns A pointer to the unicode character. It returns a null if pos is out
/// of range.
...
/// @param text_utf8 A string encoded as UTF-8.
/// @param pos The unicode character index.
/// @param len A unsigned pointer to receive the number of bytes it takes in
/// UTF-8 encoded. If len is a nullptr, it is ignored.
...
2. Send the string in text to the socket as UTF-8.
    ...
    std::string utf8str =
nana::charset(textbox.caption()).to_bytes(nana::unicode::utf8);
...
public:
  ...
  charset(const std::string &,
          unicode); ///< Attempt to convert a unicode string in byte sequence.
  charset(std::string &&,
          unicode); ///< Attempt to convert a unicode string in byte sequence.
  ...
  std::string to_bytes(
      unicode) const; ///< Converts the string to a unicode in bytes sequenece
                      ///< width a specified unicode transformation format.
private:
