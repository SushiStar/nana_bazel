 ...
 *	@description:	An implementation of a data exchange mechanism through
 *Windows Clipboard, X11 Selection.
 */
...
bool dataexch::set(const nana::paint::graphics &g, native_window_type owner) {
  ...
  HDC hDC = ::GetDC(NULL);
  if (::GetDIBits(hDC, (HBITMAP)g.pixmap(), 0, 1, NULL, (BITMAPINFO *)&bmi,
                  DIB_RGB_COLORS) == 0) {
    ::ReleaseDC(NULL, hDC);
    return false;
  }
  ...
  // Bitmaps are huge, so to avoid unnegligible extra copy, this routine does
  // not use private _m_set method.
  ...
  if (gmem) {
    ...
    p += header_size;
    // many programs do not support bottom-up DIB, so reversing row order is
    // needed.
    for (int y = 0; y < bmi.bmiHeader.biHeight; ++y) {
      memcpy(p, pbuffer.raw_ptr(bmi.bmiHeader.biHeight - 1 - y),
             bytes_per_line);
      p += bytes_per_line;
    }
...
// #elif defined(NANA_X11)
  ...
  throw std::logic_error("dataexch::set(const paint::graphics&, "
                         "native_window_type owner) not implemented yet.");
...
void dataexch::get(std::string &text_utf8) {
  ...
  if (res) {
    ...
    std::wstring wstr;
    wstr.reserve(size);
    wstr.append(reinterpret_cast<wchar_t *>(res),
                reinterpret_cast<wchar_t *>(res) + size);
    ...
    text_utf8.append(reinterpret_cast<char *>(res),
                     reinterpret_cast<char *>(res) + size);
...
void dataexch::get(std::wstring &str) {
  ...
  if (res) {
    ...
    str.reserve(size);
    str.append(reinterpret_cast<wchar_t *>(res),
               reinterpret_cast<wchar_t *>(res) + size);
...
bool dataexch::_m_set(format fmt, const void *buf, std::size_t size,
  ...
  if (::OpenClipboard(reinterpret_cast<HWND>(owner))) {
    if (::EmptyClipboard()) {
      ...
      switch (fmt) {
      case format::text:
        data_format = CF_UNICODETEXT;
        break;
      case format::pixmap:
        data_format = CF_BITMAP;
        break;
      }
  ...
  switch (fmt) {
  case format::text:
    atom_type = spec.atombase().utf8_string;
    break;
  default:
    return false;
  }
...
void *dataexch::_m_get(format fmt, size_t &size) {
  ...
  if (requester) {
    ...
    switch (fmt) {
    case format::text:
      atom = spec.atombase().utf8_string;
      break;
    default:
      return nullptr;
    }
