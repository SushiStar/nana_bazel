...
static void posix_open_url(const char *url_utf8) {
...

  {
    ...
    auto string_cpy_buff =
        std::make_unique<char[]>(full_path.size() + 1 + url_utf8_len + 1);
...
#if defined(NANA_WINDOWS)
  if (::ShellExecute(0, L"open", nana::to_wstring(url_utf8).c_str(), 0, 0,
    ...
    // initialized under WinXP.
    nana::detail::platform_spec::co_initializer co_init;
    ::ShellExecute(0, L"open", nana::to_wstring(url_utf8).c_str(), 0, 0,
                   SW_SHOWNORMAL);
  }
#elif defined(NANA_POSIX)
