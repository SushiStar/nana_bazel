...
public:
  ...
  directory_only_iterator(Arg &&arg, Args &&...args)
      : directory_iterator(arg, std::forward<Args>(args)...) {
    find_first();
  }
...
class regular_file_only_iterator : public std::filesystem::directory_iterator {
  ...
  regular_file_only_iterator &find_first() {
    while (((*this) != directory_iterator{}) &&
           !is_regular_file((**this).status()))
      this->directory_iterator::operator++();
    return (*this);
  }
...
public:
  ...
  template <typename Arg, typename... Args>
  regular_file_only_iterator(Arg &&arg, Args &&...args)
      : directory_iterator(std::forward<Arg>(arg),
...
inline regular_file_only_iterator
begin(regular_file_only_iterator iter) noexcept {
  return iter;
}

inline regular_file_only_iterator
end(const regular_file_only_iterator &) noexcept {
  return {};
}
...
bool modified_file_time(const std::filesystem::path &p,
                        struct tm &); ///< extention ?
