...
#include "nana/common/datetime.hpp"
#include <shlobj.h>
#elif defined(NANA_POSIX)
#include "nana/common/charset.hpp"
#include <cstdio>
#include <cstring>
#include <dirent.h>
#include <errno.h>
...
filesystem_error::filesystem_error(const std::string &msg, const path &path1,
                                   std::error_code err)
    : std::system_error(err, msg), path1_(path1) {}

filesystem_error::filesystem_error(const std::string &msg, const path &path1,
                                   const path &path2, std::error_code err)
    : std::system_error(err, msg), path1_(path1), path2_(path2) {}

const path &filesystem_error::path1() const noexcept { return path1_; }

const path &filesystem_error::path2() const noexcept { return path2_; }
...
// class file_status
file_status::file_status(file_type ft, perms prms) : value_{ft}, perms_{prms} {}

file_type file_status::type() const { return value_; }

void file_status::type(file_type ft) { value_ = ft; }

perms file_status::permissions() const { return perms_; }

void file_status::permissions(perms prms) { perms_ = prms; }
// end filestatus

// class path
int path::compare(const path &p) const { return pathstr_.compare(p.pathstr_); }

/// true if the path is empty, false otherwise. ??
bool path::empty() const noexcept { return pathstr_.empty(); }
...
path path::extension() const {
  ...
  if ((pos == pathstr_.npos) || (pathstr_[pos] != P) ||
      (pos + 1 == pathstr_.size()))
    return path();
...
path path::root_name() const {
  ...
  if (pathstr_.npos != pos) {
    ...
    if (pathstr_.npos != pos) {
      if ((is_directory_separator(pathstr_[0]) &&
           is_directory_separator(pathstr_[1]))
#ifdef NANA_WINDOWS
          || (pathstr_[pos - 1] == ':')
#endif
      )
...
std::size_t root_directory_pos(const path::string_type &str) {
#ifdef NANA_WINDOWS
  // case "c:/"
  if (str.size() > 2 && str[1] == ':' && is_directory_separator(str[2]))
    return 2;
#endif

  // case "//"
  if (str.size() == 2 && is_directory_separator(str[0]) &&
      is_directory_separator(str[1]))
    return path::string_type::npos;

#ifdef NANA_WINDOWS
  // case "\\?\"
  if (str.size() > 4 && is_directory_separator(str[0]) &&
      is_directory_separator(str[1]) && str[2] == '?' &&
  ...
  // case "//net {/}"
  if (str.size() > 3 && is_directory_separator(str[0]) &&
      is_directory_separator(str[1]) && !is_directory_separator(str[2])) {
    auto pos = str.find_first_of(separators, 2);
    return pos < str.size() ? pos : str.npos;
  }

  // case "/"
  if (str.size() > 0 && is_directory_separator(str[0]))
    return 0;
...
path path::root_directory() const {
  ...
  return pos == string_type::npos ? path() : path(pathstr_.substr(pos, 1));
}
...
path path::parent_path() const { return {nana_fs::parent_path(pathstr_)}; }
...
const path::value_type *path::c_str() const { return native().c_str(); }

const path::string_type &path::native() const { return pathstr_; }

path::operator string_type() const { return native(); }

std::string path::string() const { return to_osmbstr(to_utf8(pathstr_)); }

std::wstring path::wstring() const { return to_wstring(pathstr_); }
...
{
        auto str = pathstr_;
        std::replace(str.begin(), str.end(), '\\', '/');  // uppss ...  revise
this !!!!! return to_utf8(str);
}*/
...
path path::lexically_normal() const {
  ...
  for (auto itr(start); itr != stop; ++itr) {
    ...
    if (itr->native().size() == 1 && (itr->native())[0] == '.' &&
        itr != start && itr != last)
      continue;

    // ignore a name and following ".."
    if (!temp.empty() && itr->native().size() == 2 &&
        (itr->native())[0] == '.' && (itr->native())[1] == '.') // dot dot
    {
...
directory_entry::directory_entry(const filesystem::path &p) : path_{p} {}

// modifiers
void directory_entry::assign(const nana_fs::path &p) { path_ = p; }
...
// observers
file_status directory_entry::status() const { return nana_fs::status(path_); }
...
const nana_fs::path &directory_entry::path() const { return path_; }
...
#endif
  if (false == end_) {
    find_ptr_ = std::shared_ptr<find_handle>(new find_handle(handle),
                                             inner_handle_deleter());
    handle_ = handle;
  }
}
...
bool not_found_error(int errval) {
  ...
  static_assert(
      false,
      "Only Windows and Unix are supported now (Mac OS is experimental)");
#endif
}
...
#if defined(NANA_WINDOWS)
  return (FALSE != ::RemoveDirectoryW(p.c_str())) ||
         not_found_error(static_cast<int>(::GetLastError()));
#elif defined(NANA_POSIX)
...
// rm_dir_recursive
//@brief: remove a directory, if it is not empty, recursively remove it's
// subfiles and sub directories
template <typename CharT> bool rm_dir_recursive(const CharT *dir) {
  std::vector<directory_iterator::value_type> files;
  std::basic_string<CharT> path = dir;
  path += '\\';

  std::copy(directory_iterator(dir), directory_iterator(),
...
file_status status(const path &p, std::error_code &ec) {
  ...
  return file_status{(FILE_ATTRIBUTE_DIRECTORY & attr) ? file_type::directory
                                                       : file_type::regular,
                     perms::all};
#elif defined(NANA_POSIX)
  ...
  auto prms = static_cast<perms>(path_stat.st_mode &
                                 static_cast<unsigned>(perms::mask));
...
std::uintmax_t file_size(const path &p, std::error_code &ec) noexcept {
  ...
  GetFileSizeEx_fptr_t get_file_size_ex =
      reinterpret_cast<GetFileSizeEx_fptr_t>(::GetProcAddress(
          ::GetModuleHandleA("Kernel32.DLL"), "GetFileSizeEx"));
  if (get_file_size_ex) {
    HANDLE handle = ::CreateFile(p.c_str(), GENERIC_READ, FILE_SHARE_READ, 0,
...
path absolute(const path &p, std::error_code & /*err*/) { return absolute(p); }
...
path canonical(const path &p, std::error_code *err) {
  ...
  if (stat.type() == file_type::not_found) {
    ...
    err->assign(static_cast<int>(std::errc::no_such_file_or_directory),
                std::generic_category());
    return result;
  } else if (local_ec) {
    if (nullptr == err)
      throw(filesystem_error("nana::filesystem::canonical", source, local_ec));
    *err = local_ec;
    return result;
  }
...
path canonical(const path &p) { return canonical(p, nullptr); }

path canonical(const path &p, std::error_code &err) {
  return canonical(p, &err);
}

bool try_throw(int err_val, const path &p, std::error_code *ec,
               const char *message) {
  if (0 == err_val) {
    if (ec)
      ec->clear();
    ...
    if (nullptr == ec)
      throw(filesystem_error(
          message, p, std::error_code(err_val, std::generic_category())));
    else
...
path weakly_canonical(const path &p, std::error_code *err) {
  ...
  while (!head.empty()) {
    ...
    if (head_status.type() == file_type::unknown) {
      if (try_throw(static_cast<int>(std::errc::invalid_argument), head, err,
                    "nana::filesystem::weakly_canonical"))
        return path{};
    }
  ...
  if (try_throw(tmp_err.value(), head, err,
                "nana::filesystem::weakly_canonical"))
    return path();
...
path weakly_canonical(const path &p) { return weakly_canonical(p, nullptr); }
...
bool exists(const std::filesystem::path &p) { return exists(status(p)); }
...
namespace filesystem {
#if defined(_MSC_VER) &&                                                       \
    ((_MSC_VER < 1900) || ((!defined(_MSVC_LANG)) || (_MSVC_LANG < 201703)))
...
path canonical(const path &p, std::error_code *err) {
  ...
  if (stat.type() == file_type::not_found) {
    if (nullptr == err)
      throw(filesystem_error(
          "nana::filesystem::canonical", source,
          error_code(static_cast<int>(errc::no_such_file_or_directory),
                     generic_category())));
    err->assign(static_cast<int>(errc::no_such_file_or_directory),
                generic_category());
    return result;
  } else if (local_ec) {
    if (nullptr == err)
...
bool try_throw(int err_val, const path &p, std::error_code *ec,
  ...
  if (0 == err_val) {
    ...
    if (nullptr == ec)
      throw(filesystem_error(message, p,
...
path weakly_canonical(const path &p, std::error_code *err) {
  ...
  while (!head.empty()) {
    ...
    if (head_status.type() == file_type::unknown) {
      if (try_throw(static_cast<int>(errc::invalid_argument), head, err,
                    "nana::filesystem::weakly_canonical"))
        return path{};
    }
    if (head_status.type() != file_type::not_found)
...
path lexically_normal(path p) {
  ...
  for (auto itr(start); itr != stop; ++itr) {
    ...
    // ignore a name and following ".."
    if (!p.empty() && itr->native().size() == 2 && (itr->native())[0] == '.' &&
        (itr->native())[1] == '.') // dot dot
    {
      ...
      if (lf.size() > 0 &&
        ...
        p.remove_filename();
        auto next = itr;
        if (p.empty() && ++next != stop && next == last &&
            last->string() == ".") {
          p /= ".";
        }
        continue;
...
path weakly_canonical(const path &p, std::error_code *err) {
  ...
  while (!head.empty()) {
    ...
    if (head_status.type() == file_type::unknown) {
      if (detail::try_throw(static_cast<int>(errc::invalid_argument), head, err,
                            "nana::filesystem::weakly_canonical"))
        return path{};
    }
    if (head_status.type() != file_type::not_found)
  ...
  head = canonical(head, tmp_err);
  if (detail::try_throw(tmp_err.value(), head, err,
                        "nana::filesystem::weakly_canonical"))
    return path();
  return tail.empty() ? head
                      : (tail_has_dots // optimization: only normalize if tail
