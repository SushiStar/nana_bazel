...
// struct task_signal
struct pool::task_signal : task {
  task_signal() : task(task::signal) {}
...
public:
  impl(std::size_t thr_number) {
    ...
    for (; thr_number; --thr_number) {
...
#if defined(NANA_WINDOWS)
      pto->handle = (HANDLE)::_beginthreadex(
          0, 0,
      ...
      ::pthread_create(
          &(pto->handle), 0,
          reinterpret_cast<void *(*)(void *)>(&impl::_m_thr_starter), pto);
#endif
      ...
      case task::signal:
        while (true) {
          bool finished = true;
          {
            std::lock_guard<decltype(mutex_)> lock(mutex_);
            for (auto thr : container_.threads) {
              if ((thr != pto) && (state::run == thr->thr_state) &&
                  (thr->timestamp <= pto->timestamp)) {
                finished = false;
                break;
              }
            }
...
pool::pool(unsigned thread_number)
    : impl_(new impl(thread_number ? thread_number
                                   : std::thread::hardware_concurrency())) {}
#else
pool::pool(unsigned thread_number) : impl_(new impl(0)) {}
#endif

pool::pool(pool &&other) : pool() { std::swap(impl_, other.impl_); }
...
void pool::wait_for_signal() { impl_->wait_for_signal(); }

void pool::wait_for_finished() { impl_->wait_for_finished(); }

void pool::_m_push(task *task_ptr) { impl_->push(task_ptr); }
// end class pool
