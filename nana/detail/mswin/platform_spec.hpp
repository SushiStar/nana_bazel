...
namespace detail {
// struct messages
//@brief:	This defines some messages that are used for remote thread
// invocation. 			Some Windows APIs are window-thread-dependent, the
// operation in other thread 			must be posted to its own
// thread.
struct messages {
  struct caret {
    int x;
  ...
  enum {
    ...
    // Execute a function in a thread with is associated with a specified native
    // window
    affinity_execute,
...
public:
  ...
  void keep_window_icon(native_window_type, const paint::image &sml_icon,
