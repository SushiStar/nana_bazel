...
#include "nana/detail/platform_abstraction.hpp"
...
#include "nana/detail/posix/msg_dispatcher.hpp"
...
// class conf
conf::conf(const char *file) { ifs_.open(file); }
...
std::string conf::value(const char *key) {
  ...
  while (ifs_.good()) {
    ...
    if ((kpos != str.npos) &&
        ((kpos == 0) || (str.substr(0, kpos) == std::string(kpos, ' ')))) {
      size_t aspos = str.find("=", kpos + len);
      if (aspos != str.npos) {
        if ((aspos == kpos + len) || (str.substr(kpos + len, aspos) ==
                                      std::string(aspos - kpos - len, ' '))) {
...
struct caret_rep {
  ...
  caret_rep(native_window_type wd) : window{wd} {}
...
class timer_runner {
  ...
  // The delay delection is used for storing a timer id when the timer is
  // deleted in a timer's event handler function. If the timer is deleted
  // directly in timer's event handler function, it will cause a crash because
  // the deletion operation invalidates iterator. According to ISO C++
  // 2011, 23.2.4 9 the erase members shall invalidate only iterators and
  // references to the erased elements(timer_group::timers is an associative
  // container), although the iterator can be moved to next before calling the
  // timer handler function, the delay deletion is still required. Becuase a
  // timer which is erased in another timer's handler function happens to be
  // refereneced by the "next" iterator.
  struct timer_group {
    bool proc_entered{
        false}; // This flag indicates whether the timers are going to do event.
    std::set<const timer_core *> timers;
    std::vector<const timer_core *> delay_deleted;
  };

public:
  timer_runner() : is_proc_handling_(false) {}

  void set(const timer_core *handle, std::size_t interval,
           handler_type handler) {
  ...
  bool is_proc_handling() const { return is_proc_handling_; }
  ...
  bool kill(const timer_core *handle) {
    ...
    if (i != holder_.end()) {
      ...
      if (ig !=
          threadmap_
              .end()) // Generally, the ig should not be the end of threadmap_
      {
  ...
  if (bgcolor_rgb != current_color_) {
    ...
    case 16:
      col = ((((col >> 16) & 0xFF) * 31 / 255) << 11) |
            ((((col >> 8) & 0xFF) * 63 / 255) << 5) | (col & 0xFF) * 31 / 255;
      break;
    }
    ...
    // assumes the xft_fgcolor is not assigned in update_color. There is a
    // situation that causes a bug.
    //
    // update_text_color ( if fgcolor_rgb = A, then current_color = A and
    // xft_fgcolor = A) update_color (if bgcolor_rgb = B, then current_color = B
    // and xft_fgcolor is still A) update_text_color ( if fgcolor_rgb = B, then
    // current_color = B, xft_fgcolor is still A)
...
int X11_fatal_handler(Display *) { return 0; }
...
platform_spec::platform_spec()
  ...
  atombase_.wm_delete_window =
      ::XInternAtom(display_, "WM_DELETE_WINDOW", False);
  atombase_.net_frame_extents =
      ::XInternAtom(display_, "_NET_FRAME_EXTENTS", False);
  atombase_.net_wm_state = ::XInternAtom(display_, "_NET_WM_STATE", False);
  atombase_.net_wm_state_skip_taskbar =
      ::XInternAtom(display_, "_NET_WM_STATE_SKIP_TASKBAR", False);
  atombase_.net_wm_state_fullscreen =
      ::XInternAtom(display_, "_NET_WM_STATE_FULLSCREEN", False);
  atombase_.net_wm_state_maximized_horz =
      ::XInternAtom(display_, "_NET_WM_STATE_MAXIMIZED_HORZ", False);
  atombase_.net_wm_state_maximized_vert =
      ::XInternAtom(display_, "_NET_WM_STATE_MAXIMIZED_VERT", False);
  atombase_.net_wm_state_modal =
      ::XInternAtom(display_, "_NET_WM_STATE_MODAL", False);
  atombase_.net_wm_name = ::XInternAtom(display_, "_NET_WM_NAME", False);
  atombase_.net_wm_window_type =
      ::XInternAtom(display_, "_NET_WM_WINDOW_TYPE", False);
  atombase_.net_wm_window_type_normal =
      ::XInternAtom(display_, "_NET_WM_WINDOW_TYPE_NORMAL", False);
  atombase_.net_wm_window_type_utility =
      ::XInternAtom(display_, "_NET_WM_WINDOW_TYPE_UTILITY", False);
  atombase_.net_wm_window_type_dialog =
      ::XInternAtom(display_, "_NET_WM_WINDOW_TYPE_DIALOG", False);
...
Display *platform_spec::open_display() { return display_; }
...
void platform_spec::lock_xlib() { xlib_locker_.lock(); }

void platform_spec::unlock_xlib() { xlib_locker_.unlock(); }
...
Colormap &platform_spec::colormap() { return colormap_; }
...
const atombase_tag &platform_spec::atombase() const { return atombase_; }

// There are three members make_owner(), get_owner() and remove(),
// they are maintain a table to discribe the owner of windows because of the
// feature in X11, the owner of top level window must be RootWindow.
void platform_spec::make_owner(native_window_type owner,
...
void platform_spec::read_keystate(XKeyEvent &xkey) { xkey = this->key_state_; }
...
void platform_spec::caret_open(native_window_type wd,
  ...
  if (nullptr == addr) {
    ...
    if (addr->input_method) {
      XIMStyles *imstyle;
      ::XGetIMValues(addr->input_method, XNQueryInputStyle, &imstyle, nullptr,
                     nullptr);
      if (imstyle) {
        if (imstyle->count_styles) {
          addr->input_font = 0;
          XVaNestedList preedit_attr = ::XVaCreateNestedList(
              0, XNSpotLocation, &(addr->input_spot), nullptr);
          XVaNestedList status_attr = ::XVaCreateNestedList(
              0, XNAreaNeeded, &(addr->input_status_area), nullptr);
          XIMStyle *style_end =
              imstyle->supported_styles + imstyle->count_styles;
          ...
          if (has_status) {
            addr->input_context = ::XCreateIC(
                addr->input_method, XNInputStyle,
                (XIMPreeditPosition | XIMStatusArea), XNPreeditAttributes,
                preedit_attr, XNStatusAttributes, status_attr, XNClientWindow,
                reinterpret_cast<Window>(wd), nullptr);
          } else
            addr->input_context = 0;

          if ((addr->input_context == 0) && has_preedit) {
            addr->input_context =
                ::XCreateIC(addr->input_method, XNInputStyle,
                            (XIMPreeditPosition | XIMStatusNothing),
                            XNPreeditAttributes, preedit_attr, XNClientWindow,
                            reinterpret_cast<Window>(wd), nullptr);
          }

          if (addr->input_context) {
            XVaNestedList attr = ::XVaCreateNestedList(
                0, XNAreaNeeded, &(addr->input_status_area), XNClientWindow,
                reinterpret_cast<Window>(wd), nullptr);
            ::XGetICValues(addr->input_context, XNStatusAttributes, attr,
                           nullptr);
            ::XFree(attr);
          } else
            addr->input_context = ::XCreateIC(
                ...
                reinterpret_cast<Window>(wd), XNFocusWindow,
                reinterpret_cast<Window>(wd), nullptr);

          if (addr->input_context) {
            // Make the IM event filter.
            ::XGetICValues(addr->input_context, XNFilterEvents,
                           &(addr->input_context_event_mask), nullptr);
            XWindowAttributes attr;
            ::XGetWindowAttributes(display_, reinterpret_cast<Window>(wd),
                                   &attr);
            XSetWindowAttributes new_attr;
            new_attr.event_mask =
                (attr.your_event_mask | addr->input_context_event_mask);
            ::XChangeWindowAttributes(display_, reinterpret_cast<Window>(wd),
                                      CWEventMask, &new_attr);

            ::XSetICValues(addr->input_context, XNResetState, XIMPreserveState,
                           nullptr);
          }
          ::XFree(preedit_attr);
          ::XFree(status_attr);
...
void platform_spec::caret_close(native_window_type wd) {
  bool is_end_routine = false;
  {
    ...
    if (i != caret_holder_.carets.end()) {
      ...
      if (addr->input_context) {
        ...
        set_error_handler();
        XWindowAttributes attr;
        if (BadWindow != ::XGetWindowAttributes(
                             display_, reinterpret_cast<Window>(wd), &attr)) {
          if ((attr.your_event_mask & addr->input_context_event_mask) ==
              addr->input_context_event_mask) {
            XSetWindowAttributes new_attr;

            // Don't remove the KeyPress and KeyRelease mask(0x3), otherwise the
            // window will not receive Keyboard events after destroying caret
            new_attr.event_mask = (attr.your_event_mask &
                                   ~(addr->input_context_event_mask & (~0x3)));
            ::XChangeWindowAttributes(display_, reinterpret_cast<Window>(wd),
                                      CWEventMask, &new_attr);
          }
        }
  ...
  if (is_end_routine && (caret_holder_.thr != nullptr) &&
      (caret_holder_.thr->joinable())) {
...
void platform_spec::caret_pos(native_window_type wd, const point &pos) {
  ...
  if (addr && addr->input_context) {
    ...
    list = ::XVaCreateNestedList(0, XNSpotLocation, &spot, XNForeground, 0,
                                 XNBackground, 0, (void *)0);
...
bool platform_spec::caret_update(native_window_type wd,
...
void platform_spec::set_timer(const timer_core *handle, std::size_t interval,
                              void (*timer_proc)(const timer_core *)) {
...
void platform_spec::msg_dispatch(
    std::function<propagation_chain(const msg_packet_tag &)> msg_filter_fn) {
  msg_dispatcher_->dispatch(msg_filter_fn);
}

void *platform_spec::request_selection(native_window_type requestor, Atom type,
                                       size_t &size) {
  if (requestor) {
...
void platform_spec::write_selection(native_window_type owner, Atom type,
                                    const void *buf, size_t bufsize) {
  platform_scope_guard psg;
  ::XSetSelectionOwner(display_, XA_PRIMARY, reinterpret_cast<Window>(owner),
                       CurrentTime);
  ::XSetSelectionOwner(display_, atombase_.clipboard,
                       reinterpret_cast<Window>(owner), CurrentTime);
  ::XFlush(display_);
  if (XA_STRING == type || atombase_.utf8_string == type) {
    ...
    utf8str->append(reinterpret_cast<const char *>(buf),
...
// Icon Storage
const nana::paint::graphics &
platform_spec::keep_window_icon(native_window_type wd,
...
bool platform_spec::register_dragdrop(native_window_type wd,
                                      x11_dragdrop_interface *ddrop) {
  platform_scope_guard lock;
...
std::size_t platform_spec::dragdrop_target(native_window_type wd, bool insert,
                                           std::size_t count) {
  ...
  if (insert) {
    ...
    if (1 == new_val) {
      int dndver = 5;
      ::XChangeProperty(display_, reinterpret_cast<Window>(wd),
                        atombase_.xdnd_aware, XA_ATOM, sizeof(int) * 8,
                        PropModeReplace,
                        reinterpret_cast<unsigned char *>(&dndver), 1);
    }
  } else {
    ...
    if (0 == new_val) {
      ...
      ::XDeleteProperty(display_, reinterpret_cast<Window>(wd),
                        atombase_.xdnd_aware);
    } else
      i->second = new_val;
  }
  return new_val;
...
//		1 = msg_dispatcher dispatches the msg_packet_tag that modified
// by
//_m_msg_filter 		2 = msg_dispatcher should ignore the msg,
// because the XEvent is processed by _m_msg_filter
int platform_spec::_m_msg_filter(XEvent &evt, msg_packet_tag &msg) {
  ...
  if (KeyPress == evt.type || KeyRelease == evt.type) {
    auto menu_wd = bedrock.get_menu(
        reinterpret_cast<native_window_type>(evt.xkey.window), true);
    if (menu_wd)
    ...
    if (evt.xselection.property) {
      ...
      ::XGetWindowProperty(self.display_, evt.xselection.requestor,
                           evt.xselection.property, 0, 0, 0, AnyPropertyType,
      ...
      if (evt.xselection.property == self.atombase_.clipboard) {
        ...
        if (self.selection_.items.size()) {
          ...
          if (bytes_left > 0 && (type == im->type)) {
            ...
            if (Success ==
                ::XGetWindowProperty(self.display_, evt.xselection.requestor,
                                     evt.xselection.property, 0, bytes_left, 0,
        ...
        ::XSendEvent(self.display_, self.xdnd_.wd_src, False, NoEventMask,
                     &respond);
    ...
    // Skip if it is requested by XDND, it will be processed by dragdrop's
    // xdnd_protocol
    ...
    if (self.atombase_.targets == evt.xselectionrequest.target) {
      ...
      ::XChangeProperty(
          self.display_, evt.xselectionrequest.requestor,
          evt.xselectionrequest.property, XA_ATOM, 32, 0,
          reinterpret_cast<unsigned char *>(atoms.size() ? &atoms[0] : 0),
          static_cast<int>(atoms.size()));
    } else if (XA_STRING == evt.xselectionrequest.target ||
               self.atombase_.utf8_string == evt.xselectionrequest.target) {
      std::string str;
      if (self.selection_.content.utf8_string)
      ...
      ::XChangeProperty(
          self.display_, evt.xselectionrequest.requestor,
          evt.xselectionrequest.property, evt.xselectionrequest.target, 8, 0,
          reinterpret_cast<unsigned char *>(
              str.size() ? const_cast<std::string::value_type *>(str.c_str())
                         : 0),
    ...
    if (self.atombase_.xdnd_enter == evt.xclient.message_type) {
      const Atom *atoms =
          reinterpret_cast<const Atom *>(&(evt.xclient.data.l[2]));
      unsigned long len = 3;
      ...
      if (evt.xclient.data.l[1] & 1) {
        ...
        ::XGetWindowProperty(self.display_, self.xdnd_.wd_src,
                             self.atombase_.xdnd_typelist, 0, 0, False, XA_ATOM,
                             &type, &format, &len, &bytes_left, &data);

        if (bytes_left > 0) {
          ::XGetWindowProperty(self.display_, self.xdnd_.wd_src,
                               self.atombase_.xdnd_typelist, 0, bytes_left,
                               False, XA_ATOM, &type, &format, &len,
                               &bytes_left, &data);
...
#define DEBUG_XdndDirectSave
      ...
      if (XdndDirectSave) {
        ...
        ::XGetWindowProperty(self.display_, self.xdnd_.wd_src, XdndDirectSave,
                             0, 0, False, XA_ATOM, &type, &format, &len,
                             &bytes_left, &data);

        if (bytes_left > 0) {
          ::XGetWindowProperty(self.display_, self.xdnd_.wd_src, XdndDirectSave,
                               0, bytes_left, False, type, &type, &format, &len,
                               &bytes_left, &data);
        }
      }
#endif
      ...
      if (self.xdnd_.good_type != None) {
        Window child;
        ::XTranslateCoordinates(self.display_, self.root_window(),
                                evt.xclient.window, x, y, &self.xdnd_.pos.x,
                                &self.xdnd_.pos.y, &child);

        auto wd = bedrock.wd_manager().find_window(
            reinterpret_cast<native_window_type>(evt.xclient.window),
            self.xdnd_.pos);
        if (wd && wd->flags.dropable) {
          // Cache the time stamp in XdndPosition, and the time stamp must be
          // passed to XConvertSelection for requesting selection
      ...
      ::XConvertSelection(self.display_, self.atombase_.xdnd_selection,
                          self.xdnd_.good_type, self.atombase_.xdnd_selection,
