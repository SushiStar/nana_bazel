...
#include "nana/paint/truetype.hpp"
#include <set>
...
#include "nana/detail/text_reshaping.hpp"
...
public:
  ...
  void open(const std::string &font_desc, const std::set<std::string> &langs) {
    ...
    for (auto &lang : langs) {
      ...
      XftResult res;
      auto match_pat =
          ::XftFontMatch(disp_, ::XDefaultScreen(disp_), pat, &res);

      if (match_pat) {
        char *sf;
        if (XftResultTypeMismatch !=
            ::XftPatternGetString(match_pat, "family", 0, &sf)) {
  ...
  int draw(::XftDraw *xftdraw, ::XftColor *xftcolor, ::XftFont *xft, int x,
           int y, const wchar_t *str, std::size_t len) {
    ...
    // The RTL and shaping should be handled manually, because the libXft and X
    // doesn't support these language features.
    std::wstring rtl;
    ...
    for (auto &e : ents) {
      ...
      if (unicode_bidi::is_text_right(e)) {
        auto restr =
            nana::reshaping::arabic::reshape(std::wstring{e.begin, e.end});
      ...
      while (true) {
        // Scan the string until the character which font is not same with the
        // font of the first character where the scan begins.
        auto preferred = _m_scan_fonts(xft, p, size, glyph_indexes.get());
        x += _m_draw(xftdraw, xftcolor, preferred.first, x, y, p,
                     preferred.second, glyph_indexes.get());
  ...
  std::unique_ptr<unsigned[]> glyph_pixels(::XftFont *xft, const wchar_t *str,
                                           std::size_t len) {
    ...
    _m_reorder_reshaping(
        s, false,
        [&, xft, str](const wchar_t *p, std::size_t size,
                      const wchar_t *pstr) mutable {
          while (true) {
            auto preferred = _m_scan_fonts(xft, p, size, glyph_indexes.get());

            _m_glyph_px(preferred.first, p, preferred.second,
                        glyph_indexes.get(), pbuf + (pstr - str));
    ...
    _m_reorder_reshaping(
        ...
        [&, xft, str](const wchar_t *p, std::size_t size,
          ...
          while (true) {
            ...
            extent.width += _m_extents(preferred.first, p, preferred.second,
                                       glyph_indexes.get());

            if (preferred.first->ascent + preferred.first->descent >
                static_cast<int>(extent.height))
              extent.height =
                  preferred.first->ascent + preferred.first->descent;
...
private:
  /// @param reverse Indicates whether to reverse the string, it only reverse
  /// the RTL language string.
  ...
  int _m_draw(::XftDraw *xftdraw, ::XftColor *xftcolor, ::XftFont *xft, int x,
              int y, const wchar_t *str, std::size_t len,
              const FT_UInt *glyph_indexes) {
    int const init_x = x;
  ...
  // Tab is a invisible character
  unsigned _m_extents(::XftFont *xft, const wchar_t *const str,
                      const std::size_t len, const FT_UInt *glyph_indexes) {
    unsigned pixels = 0;
    auto p = str;
    auto const end = str + len;
  ...
  // Tab is a invisible character
  void _m_glyph_px(::XftFont *xft, const wchar_t *str, std::size_t len,
                   const FT_UInt *glyph_indexes, unsigned *pxbuf) {
    auto const end = str + len;
  ...
  std::pair<::XftFont *, std::size_t>
  _m_scan_fonts(::XftFont *xft, const wchar_t *str, std::size_t len,
                FT_UInt *const glyphs) const {
    auto preferred = xft;
...
public:
  fallback_manager() : langs_(_m_split_lang("ar,hi,zh-cn,zh-tw,ja,ko,th")) {}
  ...
  std::shared_ptr<fallback_fontset>
  make_fallback(const std::string &font_desc) {
...
#ifdef NANA_USE_XFT
  internal_font(const path_type &ttf, const std::string &font_family,
                double font_size, const font_style &fs,
                native_font_type native_font,
                std::shared_ptr<fallback_fontset> fallback)
      : ttf_(ttf), family_(font_family), size_(font_size), style_(fs),
        native_handle_(native_font), fallback_(fallback) {}
#else
  internal_font(const path_type &ttf, const std::string &font_family,
                double font_size, const font_style &fs,
                native_font_type native_font)
...
public:
  ...
  native_font_type native_handle() const override { return native_handle_; }

#ifdef NANA_USE_XFT
  fallback_fontset *fallback() const { return fallback_.get(); }
#endif
private:
...
#ifdef NANA_USE_XFT
void nana_xft_draw_string(::XftDraw *xftdraw, ::XftColor *xftcolor,
                          font_interface *ft, const nana::point &pos,
                          const wchar_t *str, std::size_t len) {
  auto fallback = static_cast<internal_font *>(ft)->fallback();
  if (nullptr == fallback)
    return;

  auto xft = reinterpret_cast<XftFont *>(
      static_cast<internal_font *>(ft)->native_handle());
  fallback->draw(xftdraw, xftcolor, xft, pos.x, pos.y, str, len);
}

nana::size nana_xft_extents(font_interface *ft, const wchar_t *str,
                            std::size_t len) {
  auto fallback = static_cast<internal_font *>(ft)->fallback();
  if (nullptr == fallback)
    return {};
...
nana_xft_glyph_pixels(font_interface *ft, const wchar_t *str, std::size_t len) {
  auto fallback = static_cast<internal_font *>(ft)->fallback();
  if (nullptr == fallback)
    return {};
...
double platform_abstraction::font_default_pt() {
  ...
  auto pt = std::abs(metrics.lfMessageFont.lfHeight) * 72.0 /
            ::GetDeviceCaps(desktop, LOGPIXELSY);
...
platform_abstraction::default_font(const ::std::shared_ptr<font> &new_font) {
  auto &r = platform_storage();
  if (new_font) {
    auto f = r.font;
    if (new_font != r.font)
...
static std::shared_ptr<platform_abstraction::font>
font_factory(::std::string font_family, double size_pt,
  ...
  // Make sure the length of font family less than LF_FACESIZE which is defined
  // by Windows
  ...
  auto font_height =
      -static_cast<LONG>(size_pt * ::GetDeviceCaps(hDC, LOGPIXELSY) / 72);
  ...
  std::string pat_str =
      '-' + std::to_string(size_pt ? size_pt
                                   : platform_abstraction::font_default_pt());
  if (fs.weight < 400)
    pat_str += ":light";
  ...
  int missing_count;
  char *defstr;
  XFontSet fd = ::XCreateFontSet(display_, const_cast<char *>(pat_str.c_str()),
                                 &missing_list, &missing_count, &defstr);
#endif
#endif
  ...
  if (fd) {
    ...
    return std::make_shared<internal_font>(
        std::move(ttf), std::move(font_family), size_pt, fs,
        reinterpret_cast<native_font_type>(fd), fallback);
#else
    return std::make_shared<internal_font>(
        std::move(ttf), std::move(font_family), size_pt, fs,
        reinterpret_cast<native_font_type>(fd));
#endif
  }
  return {};
}
...
::std::shared_ptr<platform_abstraction::font>
platform_abstraction::make_font(const std::string &font_family, double size_pt,
                                const font::font_style &fs) {
  return font_factory(font_family, size_pt, fs, {});
}

::std::shared_ptr<platform_abstraction::font>
platform_abstraction::make_font_from_ttf(const path_type &ttf, double size_pt,
                                         const font::font_style &fs) {
  ...
  if (try_add) {
    if (1 == ++(fc[ttf.string()])) {
      ::FcConfigAppFontAddFile(
          nullptr, reinterpret_cast<const FcChar8 *>(ttf.string().c_str()));
    }
  } else {
...
#ifdef NANA_WINDOWS
  auto hdc = ::GetDC(nullptr);
  auto dots = static_cast<unsigned>(
      ::GetDeviceCaps(hdc, (x_requested ? LOGPIXELSX : LOGPIXELSY)));
  ::ReleaseDC(nullptr, hdc);
  return dots;
#else
