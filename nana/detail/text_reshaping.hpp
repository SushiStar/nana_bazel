...
unsigned short letters[][4] = {
    ...
    {0xFEEF, 0xFBE8, 0xFBE9,
     0xFEF0}, // ARABIC LETTER (UIGHUR KAZAKH KIRGHIZ)? ALEF MAKSURA
...
bool harakat(wchar_t letter) {
  return (0x0610 <= letter && letter <= 0x061A) ||
         (0x064B <= letter && letter <= 0x065F) || (0x0670 == letter) ||
...
int form_index(wchar_t letter) {
  static unsigned short ranges[][2] = {
      {0x0621, 0x063A}, {0x0640, 0x064A}, {0x0671, 0x0671}, {0x0677, 0x0677},
      {0x0679, 0x067B}, {0x067E, 0x0680}, {0x0683, 0x0684}, {0x0686, 0x0688},
      {0x068C, 0x068E}, {0x0691, 0x0691}, {0x0698, 0x0698}, {0x06A4, 0x06A4},
      {0x06A6, 0x06A6}, {0x06A9, 0x06A9}, {0x06AD, 0x06AD}, {0x06AF, 0x06AF},
      {0x06B1, 0x06B1}, {0x06B3, 0x06B3}, {0x06BA, 0x06BB}, {0x06BE, 0x06BE},
      {0x06C0, 0x06C1}, {0x06C5, 0x06C9}, {0x06CB, 0x06CC}, {0x06D0, 0x06D0},
      {0x06D2, 0x06D3}, {ZWJ, ZWJ}};

  if ((letter < 0x0621) || (0x06D3 < letter && letter != ZWJ))
    return -1;

  int base = 0;
  for (std::size_t i = 0; i < sizeof(ranges) / sizeof(unsigned short) / 2;
       ++i) {
...
std::wstring reshape(const std::wstring &text) {
  ...
  const int no_form = -1;
  const int isolated_form =
      use_unshaped_instead_of_isolated ? unshaped : isolated;
  ...
  for (auto letter : text) {
    if (harakat(letter)) {
      ...
      continue;
    } else if (((TATWEEL == letter) && delete_tatweel) ||
               (ZWJ == letter && !support_zwj)) {
      continue;
    }
    ...
    if ((forms.back() == no_form) || (!connect_before(letter)) ||
        (!connect_after(output.back())) ||
    ...
    if (support_zwj && (output.size() > 1) &&
        (output[output.size() - 2] == ZWJ))
