...
#include "nana/common/detail/posix/msg_packet.hpp"
...
public:
  ...
  msg_dispatcher(Display *disp) : display_(disp) {
    proc_.event_proc = 0;
    proc_.timer_proc = 0;
    proc_.filter_proc = 0;
  }
  ...
  void set(timer_proc_type timer_proc, event_proc_type event_proc,
  ...
  void insert(Window wd) {
...

    {
      ...
      std::map<thread_t, thread_binder *>::iterator i =
          table_.thr_table.find(tid);
    ...
    if (start_driver && proc_.event_proc && proc_.timer_proc) {
      // It should start the msg driver, before starting it, the msg driver must
      // be inactive.
      if (thrd_) {
        is_work_ = false;
        thrd_->join();
      ...
      is_work_ = true;
      thrd_ = std::unique_ptr<std::thread>(
          new std::thread([this]() { this->_m_msg_driver(); }));
    }
  }
  ...
  void erase(Window wd) {
    ...
    if (i != table_.wnd_table.end()) {
      ...
      if (thr->window.size()) {
        // Make a cleanup msg packet to infor the dispatcher the window is
        // closed.
        msg_packet_tag msg;
  ...
  void dispatch(Window modal) {
    ...
    // Test whether the thread is registered for window, and retrieve the queue
    // state for event
  ...
  template <typename MsgFilter> void dispatch(MsgFilter msg_filter_fn) {
    auto tid = nana::system::this_thread_id();
    msg_packet_tag msg;
    int qstate;
      ...
      int pending;
      {
        ...
        if (pending) {
          ...
          if (KeyRelease == event.type) {
            // Check whether the key is pressed, because X will send KeyRelease
            // when pressing and holding a key if auto repeat is on.
            char keymap[32];
            ::XQueryKeymap(display_, keymap);

            if (keymap[event.xkey.keycode / 8] &
...
private:
  ...
  // return@ it returns true if the queue is not empty, otherwise the wait is
  // timeout.
  bool _m_wait_for_queue(thread_t tid) {
    ...
    // Waits for notifying the condition variable, it indicates a new msg is
    // pushing into the queue.
    std::unique_lock<decltype(thr->mutex)> lock(thr->mutex);
    return (thr->cond.wait_for(lock, std::chrono::milliseconds(10)) !=
