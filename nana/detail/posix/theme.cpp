...
std::string find_value(std::ifstream &ifs, const std::string &category,
...
std::string find_gnome_theme_name() {
  try {
    ...
    for (fs::directory_iterator i{"/usr/share/glib-2.0/schemas"}, end; i != end;
         ++i) {
      auto filename = i->path().filename().string();
      if (filename.size() > 17 &&
          filename.substr(filename.size() - 17) == ".gschema.override") {
        auto priority = gschema_override_priority(filename);
        if (priority < 0)
          continue;

        auto i =
            std::find_if(overrides.cbegin(), overrides.cend(),
                         [priority](const std::string &ovrd) {
...
public:
  icon_theme(const std::string &name)
      : theme_name_(name), ifs_("/usr/share/icons/" + name + "/index.theme") {
  ...
  std::string find(const std::string &name, std::size_t size_wanted) const {
    ...
    if (closer_dir.empty()) {
      ...
      // Avoid recursively traverse directory for hicolor if current theme name
      // is hicolor
...
theme::theme()
    : path_("/usr/share/icons/"), ifs_("/usr/share/icons/default/index.theme") {
}
...
std::string theme::icon(const std::string &name,
                        std::size_t size_wanted) const {
  // Lookup in cache
