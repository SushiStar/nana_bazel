...
#include "nana/detail/posix/theme.hpp"
...
public:
  enum class xdnd_status_state {
    normal,
    position,
    drop, // Use the 'accept' flag of XdndStatus when mouse has
          // released(XdndDrop has been sent).
    status_ignore
  };

  xdnd_protocol(Window source)
      : spec_(nana::detail::platform_spec::instance()), source_(source) {
    auto disp = spec_.open_display();
    detail::platform_scope_guard lock;
    ::XSetSelectionOwner(disp, spec_.atombase().xdnd_selection, source,
                         CurrentTime);
  ...
  void mouse_leave() { _m_xdnd_leave(); }

  bool mouse_release() { return _m_xdnd_drop(); }

  Atom executed_action() const { return executed_action_; }
  ...
  bool client_message(const ::XClientMessageEvent &xclient) {
    ...
    if (atombase.xdnd_status == xclient.message_type) {
      if (xdnd_status_state::position != xstate_ &&
          xdnd_status_state::drop != xstate_)
        return false;
      ...
      if (xclient.data.l[1] & 0x2) {
        rectangle rct{static_cast<int>(xclient.data.l[2] >> 16),
  ...
  void selection_request(const ::XSelectionRequestEvent &xselectionrequest,
    ...
    if (spec_.atombase().xdnd_selection == xselectionrequest.selection) {
      ...
      if (xselectionrequest.target == spec_.atombase().text_uri_list) {
        if (data.files.size()) {
          ...
          ::XChangeProperty(
              spec_.open_display(), xselectionrequest.requestor,
              xselectionrequest.property, xselectionrequest.target, 8,
              PropModeReplace,
              reinterpret_cast<unsigned char *>(&uri_list.front()),
              uri_list.size() + 1);
      ...
      ::XSendEvent(spec_.open_display(), xselectionrequest.requestor, False, 0,
                   &evt);
...
private:
  bool _m_xdnd_enter(Window wd) {
    ...
    target_ = wd;
    _m_client_msg(spec_.atombase().xdnd_enter, (xdnd_ver << 24),
                  spec_.atombase().text_uri_list, XA_STRING);
  ...
  void _m_xdnd_position(const nana::point &pos, Atom requested_action) {
    ...
    _m_client_msg(spec_.atombase().xdnd_position, 0, position, CurrentTime,
                  requested_action);
  }
...
private:
  ...
  void _m_client_msg(Atom xdnd_atom, long data1, long data2, long data3,
                     long data4 = 0) {
  ...
  //@return the XDND version. If the specified window does not have property
  // XdndAware, it returns 0
  int _m_xdnd_aware(Window wd) {
    ...
    ::XGetWindowProperty(spec_.open_display(), wd, spec_.atombase().xdnd_aware,
                         0, 4, False, XA_ATOM, &actual, &format, &count,
                         &remaining, &data);
  ...
  void _m_cursor(bool accepted) {
    ::XDefineCursor(spec_.open_display(), source_,
                    (accepted ? cursor_.dnd_move : cursor_.dnd_none));
  }
