...
#include "nana/detail/platform_abstraction_types.hpp"
#include <functional>
#include <map>
#include <vector>
...
/// class timer_core
/**
 * Platform-spec only provides the declaration for intrducing a handle type, the
 * definition of timer_core is given by gui/timer.cpp
 */
class timer_core;
...
public:
  ...
  bool caret_update(native_window_type, nana::paint::graphics &root_graph,
                    bool is_erase_caret_from_root_graph);
  ...
  Window grab(Window);
  void set_timer(const timer_core *, std::size_t interval,
                 void (*timer_proc)(const timer_core *tm));
  ...
  // X Selections
  void *request_selection(native_window_type requester, Atom type,
                          size_t &bufsize);
  void write_selection(native_window_type owner, Atom type, const void *buf,
                       size_t bufsize);

  // Icon storage
  //@biref: The image object should be kept for a long time till the window is
  // closed, 			the image object is release in remove() method.
  const nana::paint::graphics &keep_window_icon(native_window_type,
                                                const nana::paint::image &);

  bool register_dragdrop(native_window_type, x11_dragdrop_interface *);
  std::size_t dragdrop_target(native_window_type, bool insert,
                              std::size_t count);
